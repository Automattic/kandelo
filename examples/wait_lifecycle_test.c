 ...
 * struct rusage layouts, so create a real child through Kandelo's non-forking
 * posix_spawn path.  A shell child stops itself, then waits on an inherited
 * pipe after SIGCONT so the parent cannot lose the continued event to a fast
 * exit before inspecting it.
 */
static int test_memory64_wait_layouts(void)
{
    ...
    char script[128];
    int script_len = snprintf(script, sizeof(script),
        "kill -STOP $$; read _ <&%d; exit 7", gate[0]);
    if (script_len < 0 || (size_t)script_len >= sizeof(script)) {
        fprintf(stderr, "memory64 child script was truncated\n");
        close(gate[0]);
        close(gate[1]);
        return -1;
    }
