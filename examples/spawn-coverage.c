 ...
 *
 * Asserts that PATH search via libc's `access(X_OK)` finds a regular,
 * executable program in the guest VFS. The Vitest harness stages the
 * declared `/usr/bin/hello` exec target into that VFS, which is the same
 * guest-visible state that POSIX path resolution observes.
 */
