...
/* Load the bundled font at `px_size` LOGICAL pixels. Glyphs are rasterized
 * at px_size * wpk_scale(), captured at load time, so load fonts only after
 * the scale is set. Valid px_size is [4, 256];
...
/* Logical width of a UTF-8 string in this font (for layout before
 * wpk_text). */
int wpk_text_width(struct wpk_font *f, const char *utf8);

/* Render a UTF-8 string with (x, y) as the logical BASELINE (not the
 * top-left), alpha-blended into the surface. */
void wpk_text(struct wpk_surface *s, struct wpk_font *f,
              int x, int y, const char *utf8, wpk_color color);

/* Font metrics in LOGICAL pixels at the loaded size. height = ascent +
 * descent; ascent = baseline above the origin. Used for row/label layout. */
