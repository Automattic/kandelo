...
# 3. my_gethwaddr: Enable Linux code path for wasm32 (SIOCGIFCONF + SIOCGIFHWADDR)
HWADDR_FILE="$SRC_DIR/mysys/my_gethwaddr.c"
if ! grep -q '__wasm32__' "$HWADDR_FILE" 2>/dev/null; then
    echo "  Patching mysys/my_gethwaddr.c (enable MAC address retrieval for wasm32)..."
    sed -i '' 's/defined(__linux__) || defined(__sun) || defined(_WIN32)/defined(__linux__) || defined(__sun) || defined(_WIN32) || defined(__wasm32__)/' "$HWADDR_FILE"
    sed -i '' 's/#elif defined(_AIX) || defined(__linux__) || defined(__sun)/#elif defined(_AIX) || defined(__linux__) || defined(__sun) || defined(__wasm32__)/' "$HWADDR_FILE"
fi
