...
if [ ! -f "$LIBYAML_DIR/lib/libyaml.a" ]; then
    ...
    if [ ! -d "$LIBYAML_SRC" ]; then
        curl --retry 5 --retry-delay 2 --retry-all-errors -fsSL "https://pyyaml.org/download/libyaml/yaml-${LIBYAML_VERSION}.tar.gz" \
...
if [ ! -d "$SRC_DIR" ]; then
    ...
    curl --retry 5 --retry-delay 2 --retry-all-errors -fsSL "https://cache.ruby-lang.org/pub/ruby/${RUBY_MAJOR_MINOR}/${TARBALL}" \
        -o "/tmp/${TARBALL}"
    mkdir -p "$SRC_DIR"
