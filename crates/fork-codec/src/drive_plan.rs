...
//!                     | DRIVE_OP_STATIC_ROOT (3) | 4 (retired, see below)
...
/// exnref's reachable reference payloads were already allocated or published
/// by earlier steps). Mirrors the JS
...
// `op` 4 is RETIRED. It was DRIVE_OP_EXTERNREF_TRANSIT, which re-rooted a
// captured host `externref` into the child's anyref transit through a host
// `resolve_externref` import. A fork no longer carries a raw host externref
// (capture refuses one with `EOPNOTSUPP`, stage E2), so no plan emits it, and
// the injected `fm_drive_execute` shim has no branch for it. The number stays
// unused so the ops after it keep their values.
...
/// state (funcref/GC) by reading the values the reconstruction steps
...
/// takes NO argument (`() -> ()`; the instrumenter emits `wpk_fork_unwind_end`
/// with an empty signature — see `fork_instrument::runtime::emit_end_fn`), so the
/// injected shim `call_indirect`s it through a distinct `() -> ()` type, reading
/// neither `arg` nor `recipe`. `slot` is `base(activation) +
/// DRIVE_SLOT_UNWIND_END`. Being `>= DRIVE_OP_RESTORE` it is excluded from the
/// reconstruction drive-step counter (a control state-flip, not a reference
/// reconstruction). Mirrors the host loop that called `wpk_fork_unwind_end()` per
...
/// shape as [`DRIVE_OP_UNWIND_END`] — a NO-argument `() -> ()` guest export (the
/// instrumenter emits `wpk_fork_rewind_end` with an empty signature via
/// `fork_instrument::runtime::emit_end_fn`) — so the injected shim
/// `call_indirect`s it through the SAME `() -> ()` type, reading neither `arg`
/// nor `recipe`. `slot` is `base(activation) + DRIVE_SLOT_REWIND_END`. Being
/// `>= DRIVE_OP_RESTORE` it is excluded from the reconstruction drive-step
...
/// Drive-table slot offset the host binds `wpk_fork_unwind_end` into, and a
/// `DRIVE_OP_UNWIND_END` step's `slot` field points at (see
/// `DRIVE_SLOT_REWIND_BEGIN`). Distinct from the op tag (`DRIVE_OP_UNWIND_END` =
/// 9) so it never collides with the ALLOC/FILL/EXN/RESTORE/FINISH/REWIND/ABORT
/// offsets.
pub const DRIVE_SLOT_UNWIND_END: u32 = 7;
/// Drive-table slot offset the host binds `wpk_fork_rewind_end` into, and a
/// `DRIVE_OP_REWIND_END` step's `slot` field points at (see
/// `DRIVE_SLOT_UNWIND_END`). Distinct from every other offset so a single drive
/// table can hold an activation's begin AND end targets simultaneously.
pub const DRIVE_SLOT_REWIND_END: u32 = 8;
/// Drive-table slot offset the host binds `wpk_fork_abort_end` into, and a
...
/// Drive-table slot the host binds the guest's `__wpk_fork_ref_exn_throw_recipe`
/// into: the export that RAISES a captured exception with that guest's own tag.
///
/// The one thing about an exception a module cannot do for another module. A
/// replay reconstructing an `exnref` has to re-enter wasm THROWING, and the tag
/// belongs to whichever activation's codec declared it -- so the throw must
/// happen inside that activation, not in the module and not in a JavaScript
/// import (a JS `throw` crosses back as a foreign exception with the wrong
/// tag). Binding the export here lets the module `call_indirect` it exactly as
/// it drives allocate/fill/materialize, with the owner read out of its own
/// decoded graph.
///
/// NOT a `DRIVE_OP_*`: no plan step names it. The module calls it directly when
/// a guest asks it to re-throw a recipe whose tag that guest does not own,
...
impl<'a, H: DrivePlanHints> PlanWalk<'a, H> {
    ...
    /// funcref / static-root emit nothing HERE. They are reconstructed outside
    /// the allocate/fill walk: the funcref shim, and — for a static root — the
    /// DRIVE_OP_STATIC_ROOT step emitted in Phase 0 (before this walk), so the
    /// reference is already published into the transit by the time an edge reads
    /// it, exactly as the JS `materializeTypedGraph` publishes them first.
    fn ensure_identity(&mut self, recipe_id: u32) -> Result<(), Errno> {
        match self.node(recipe_id)? {
            ...
            | ReferenceRecipeNode::StaticRoot { .. } => Ok(()),
...
/// / exnref / typed-GC — no static-root) on this path, so a
...
///    every immutable static root, publishing into the anyref transit at slot
///    `recipe + 1` BEFORE any allocate/fill so an immutable constructor, a
///    `_gc_fill` edge, or the guest's own per-frame decode reads the canonical
///    rooted identity.
...
/// injected funcref shim. A graph carries no host `externref` node at all: a
/// fork that holds one is refused at capture (`EOPNOTSUPP`).
///
...
/// Append one `DRIVE_OP_UNWIND_END` step per `activation` — the module-owned
/// capture-SEAL drive that replaces the host loop calling `wpk_fork_unwind_end()`
/// on each activation before serializing the sealed journal. UNLIKE the
/// rewind/abort begin steps this drives a NO-argument guest export, so `recipe`
/// and `arg` are both 0 (the injected shim reads neither). Emitted in the
/// caller's activation order (ascending id, matching the host's former
/// per-activation seal loop). ONLY valid for a COMPLETE capture — every frame
/// committed — so `wpk_fork_unwind_end` moves each activation cleanly from
/// `UNWINDING` to `NORMAL`; a partial/aborted capture never uses this (the host
/// seals it for abort without an unwind-end drive).
pub fn append_unwind_end_steps(steps: &mut Vec<DriveStep>, activations: &[u32]) {
    for &activation in activations {
        steps.push(DriveStep {
            op: DRIVE_OP_UNWIND_END,
            slot: drive_table_base(activation) + DRIVE_SLOT_UNWIND_END,
            recipe: 0,
            arg: 0,
        });
    }
...
/// caller's activation order (ascending id), matching the host's former
/// per-activation finish loop. Valid only once every inherited frame has been
/// replayed (the guest is at `REWINDING` / `ABORT_UNWINDING`), so the end flip
/// moves each activation cleanly back to `NORMAL`.
pub fn append_replay_end_steps(steps: &mut Vec<DriveStep>, activations: &[u32], abort: bool) {
    let (op, slot_off) = if abort {
        (DRIVE_OP_ABORT_END, DRIVE_SLOT_ABORT_END)
    } else {
        (DRIVE_OP_REWIND_END, DRIVE_SLOT_REWIND_END)
    };
    for &activation in activations {
        steps.push(DriveStep {
            op,
            slot: drive_table_base(activation) + slot_off,
            recipe: 0,
            arg: 0,
        });
    }
...
mod tests {
    ...
    #[test]
    fn append_replay_end_steps_emits_one_void_step_per_activation() {
        // Non-abort: rewind-end at the REWIND_END slot, argument-free.
        let mut steps = Vec::new();
        append_replay_end_steps(&mut steps, &[0, 2], false);
        assert_eq!(
            steps,
            vec![
                DriveStep {
                    op: DRIVE_OP_REWIND_END,
                    slot: drive_table_base(0) + DRIVE_SLOT_REWIND_END,
                    recipe: 0,
                    arg: 0,
                },
                DriveStep {
                    op: DRIVE_OP_REWIND_END,
                    slot: drive_table_base(2) + DRIVE_SLOT_REWIND_END,
                    recipe: 0,
                    arg: 0,
                },
            ]
        );
        // Abort: abort-end at the ABORT_END slot. Distinct op AND slot so a drive
        // table can hold both a rewind-end and an abort-end target per activation.
        let mut abort_steps = Vec::new();
        append_replay_end_steps(&mut abort_steps, &[0], true);
        assert_eq!(
            abort_steps[0],
            DriveStep {
                op: DRIVE_OP_ABORT_END,
                slot: drive_table_base(0) + DRIVE_SLOT_ABORT_END,
                recipe: 0,
                arg: 0,
            }
        );
        assert_ne!(DRIVE_SLOT_REWIND_END, DRIVE_SLOT_ABORT_END);
        // Every end op takes the injected shim's void branch (op >= UNWIND_END).
        assert!(DRIVE_OP_REWIND_END >= DRIVE_OP_UNWIND_END);
        assert!(DRIVE_OP_ABORT_END >= DRIVE_OP_UNWIND_END);
    }

    #[test]
    fn append_unwind_end_steps_emits_one_void_step_per_activation() {
        let mut steps = Vec::new();
        append_unwind_end_steps(&mut steps, &[0, 2]);
        assert_eq!(steps.len(), 2);
        // Activation 0: slot = base(0) + DRIVE_SLOT_UNWIND_END, no argument.
        assert_eq!(
            steps[0],
            DriveStep {
                op: DRIVE_OP_UNWIND_END,
                slot: drive_table_base(0) + DRIVE_SLOT_UNWIND_END,
                recipe: 0,
                arg: 0,
            }
        );
        // Activation 2: slot rides its own base slice; still argument-free.
        assert_eq!(
            steps[1],
            DriveStep {
                op: DRIVE_OP_UNWIND_END,
                slot: drive_table_base(2) + DRIVE_SLOT_UNWIND_END,
                recipe: 0,
                arg: 0,
            }
        );
    ...
    #[test]
    fn plain_struct_over_funcref_allocs_then_fills() {
        // struct(0) -> funcref(1). A funcref leaf needs no drive step (the
        // injected funcref shim reconstructs it), so the plan is the struct's
        // allocate + fill alone.
        let nodes = vec![
    ...
    #[test]
    fn struct_array_cycle_over_funcref_allocates_all_then_fills() {
        // The CYCLIC graph from reference_replay's tests, with MUTABLE fields (no
        // constructor deps): struct(0) <-> array(1), both reaching funcref(2).
        // The aliased funcref leaf needs no step; allocate-all-first breaks the
        // cycle: ALLOC 0, ALLOC 1, FILL 0, FILL 1.
        let nodes = vec![
    ...
    #[test]
    fn exnref_over_funcref_payload_materializes() {
        // exnref(1) whose payload is funcref(0). The funcref payload needs no
        // step, so the exnref emits ONE EXN step in its owner activation.
        let nodes = vec![
