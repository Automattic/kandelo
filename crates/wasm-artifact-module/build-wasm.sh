...
if [[ "${1:-}" == "--verify-fresh" ]]; then
  ...
  if ! assert_side_module_tier_copy_matches "wasm-artifact-module" "$REPO_ROOT" "wasm_artifact_module32.wasm"; then
    exit 1
  fi
...
stage_side_module_tier_copy "$REPO_ROOT" "wasm_artifact_module32.wasm"
