...
mod wasm {
    ...
    // -- Host-seeded resume-slot catalogs -------------------------------------
    //
    // Resume-slot PARITY (D5 §"Other couplings" 1): the guest imports the
    // module's `__wpk_fork_resume_table`, and `resume_peek` returns an index
    // INTO that table, so the module's `ResumeSlotTable` numbering MUST match
    ...
    // (`CatalogCell`, 256 KiB of BSS on every fork-capable thread) -- beside
    // the per-activation store, with `register_activation_slots` falling
    // through per-activation -> process-wide -> committed ordinals. The split
    // ran backwards against measurement: the largest shipped catalogs
    // (php-fpm 19,190, php 19,026, node/spidermonkey 16,555) all sat on the
    // fixed static, while the per-activation store, whose largest extension
    // is 7,750, was the growable one. One store, one entry, one rule now.
    ...
    // catalog the floor could not hold, which nothing ever released. php's
    // main activation (19,026 ordinals) spilled on every process start.

    /// Every `(ordinal, slot)` an activation holds lives in ONE arena record,
    // under `REC_KIND_RESUME_ASSIGNMENT`, allocated when that activation
    // registers and released when it unregisters.
    //
    // WHAT THIS REPLACED, and why the shape changed rather than just the
    // storage: a single `[[u32; 3]; 65_536]` index -- `[activation_id, ordinal,
    // slot]` per coordinate, 786,432 bytes of BSS -- shared by every
    // activation, with a live prefix and a swap-remove compaction. That array
    // was subtracted from EVERY fork-capable thread's mmap window whether the
    // thread ever forked or not.
    //
    // It could not be a per-activation record until `resume_slot_of` went. That
    // was the one reader needing `(activation, ordinal)` as a lookup KEY, and
    // it died with `fm_resume_slots` op 0 when the guest's own shim took over
    // thunk placement. The three readers left are all "everything belonging to
    // activation A" or "append", which one contiguous record serves with no
    // walk at all: the duplicate-activation scan that used to walk the whole
    // live prefix (up to 47,757 entries for php, once per registration) is now
    // `arena_find`'s directory lookup, and the per-ordinal append that
    // re-loaded the live count every iteration is one bounded write into a
    // record sized up front.
    //
    // THERE IS NO CAP ANY MORE. `RESUME_SLOT_CAP = 65_536` bounded the fixed
    // array and produced `E2BIG` for a catalog that would overrun it. A chunk
    // that cannot be mapped now fails with `channel_mmap`'s truthful
    // `ENOMEM`/`EAGAIN` instead of an `E2BIG` for a boundary that no longer
    // exists.

    /// THE FREE SET IS DERIVED, NOT RECORDED, and that is a measured constraint
    // rather than a preference. See `resume_free_set` below for what it is and
    // `free_bits_mark`'s deleted mapping path for what happened when it was
    // stored.
    //
    // WHAT WAS TRIED, IN ORDER, because two of the three are written up
    // elsewhere as settled and neither is:
    //
    //  1. A fixed `[u64; 1024]` bitmap in BSS. 8 KiB reserved on every
    //     fork-capable thread, and its extent was one slot narrower than the
    //     numbering it indexed.
    //  2. A chain of chunks mapped on demand, marked at FREE. Reverted once on
    //     the argument that a vfork child's exit teardown can never syscall;
    //     that argument was then refuted by measurement, for the HEALTHY child
    //     exit it was written about.
    //  3. This. Because the refutation's scope was not the whole path: when a
    //     vfork borrower is killed by an external fatal signal, the kernel
    //     contains the address space and completes the parent's blocked vfork
    //     with `EINTR`. The exit teardown that follows calls `fm_resume_slots`
    //     op 1, and a `channel_mmap` from THERE never gets an answer -- the
    //     worker parks in `memory_atomic_wait32` and the whole program hangs.
    //     `host/test/vfork-lifecycle-guest.test.ts`'s "contains a
    ...
    ///
    /// The watermark is also the only bound `resume_unregister_impl` has left to
    /// check a slot against, and that bound has a second job -- see the note
    /// there on the use-after-free it detects.
    static RESUME_NEXT_SLOT: AtomicU32 = AtomicU32::new(1);

    /// Visit every live record of `kind`, as `(payload address, byte length)`.
    ///
    /// The directory is the only index of live activations, so this is the only
    /// way to ask a whole-arena question. It is a walk, not a maintained set,
    /// for the reason Task 1 gave when it deleted the maintained directory
    ...
    /// The slots below the watermark that no live activation holds, as a bitmap.
    ///
    /// # Why this is derived rather than maintained
    ///
    /// See the note above `RESUME_NEXT_SLOT`: recording the free set AT FREE
    /// needs somewhere to record it, and every version of "somewhere" either
    /// reserves BSS on every fork-capable thread or maps a chunk from a path
    /// that a killed vfork borrower's teardown reaches, where a channel syscall
    /// never returns. Derived, the free set costs no storage at all between
    /// registrations and the free path becomes pure record removal.
    ///
    /// # What it costs, stated where it can be checked
    ///
    /// O(watermark + live slots) per REGISTRATION -- not per allocation, which
    /// would be quadratic in an activation's size and php registers 19,026
    /// ordinals at once. Registration happens once per worker plus once per
    /// `dlopen`, so a dlopen-heavy program pays O(registrations x live slots):
    /// 100 `dlopen`s over php's 19,026 slots is about 1.9 million bit
    /// operations spread across 100 calls. The measured dlopen fixture's
    /// largest activation is 86 slots.
    ///
    /// The scratch vector is bump-heap and lives only for the registration that
    /// built it. `reset_bump_heap` runs at four points during a FORK, and this
    /// runs at catalog-seed time, so nothing resets under it -- and even if it
    /// did, nothing outlives the call to be clobbered.
    fn resume_free_set() -> alloc::vec::Vec<u64> {
        let watermark = RESUME_NEXT_SLOT.load(Ordering::Relaxed);
        let words = (watermark as usize).div_ceil(64);
        let mut bits = alloc::vec![u64::MAX; words];
        if words == 0 {
            return bits;
        }
        // Slot 0 is the reserved sentinel and is never free.
        bits[0] &= !1u64;
        // And nothing at or above the watermark exists yet.
        let tail = watermark as usize % 64;
        if tail != 0 {
        ...
        arena_for_each_record(REC_KIND_RESUME_ASSIGNMENT, |_, at, byte_len| {
            for index in 0..(byte_len / RESUME_ASSIGNMENT_RECORD_BYTES) {
                ...
                if slot != 0 && slot < watermark {
                    bits[slot as usize / 64] &= !(1u64 << (slot as usize % 64));
                }
            }
        });
        bits
    ...
    /// Take the smallest slot the set still holds, clearing it.
    ///
    ...
    /// slots out of it. That is measured, not argued: an earlier version of
    /// those tests asserted the free list's STORAGE and stayed green while a
    /// perturbed module handed out slots around 1,000,001.
    fn resume_take_free(bits: &mut [u64], cursor: &mut usize) -> Option<u32> {
        while *cursor < bits.len() {
            let word = bits[*cursor];
            if word == 0 {
                *cursor += 1;
                continue;
            }
            let bit = word.trailing_zeros();
            bits[*cursor] = word & (word - 1);
            return Some((*cursor as u32) * 64 + bit);
        }
        None
    }

    /// Assign this activation's slots from its seeded catalog, into ONE arena
    /// record.
    ///
    /// The catalog is the activation's own, seeded by the entry that calls
    /// this; there is no other source (see `register_activation_slots`).
    ///
    ...
    ///
    /// # THIS ALLOCATES, so a worker with no syscall channel cannot register
    ///
    /// The record is arena storage, and the arena maps its chunks with
    /// `channel_mmap`. A module whose `fm_set_format` was given no channel base
    /// therefore gets `EINVAL` from `channel_base()` here rather than a
    /// silently empty registration. That is the truthful answer -- a module
    /// with no channel genuinely cannot own storage -- and it is why
    ...
    fn resume_register_impl(activation_id: u32) -> Result<u32, Errno> {
        ...
        // `libneeded-provider.so` in `fork-from-dlopen-side-module-e2e` seeds
        // an EMPTY resume catalog, and mistaking that for "never registered"
        // already cost a real fork (see `resume_unregister_impl`). An empty
        // record says "registered, holding nothing"; no record says "never
        // registered". Those are different facts and the arena holds both.
        // BUILT BEFORE THE RECORD, so the walk sees only activations that
        // really hold slots. This activation's record is zeroed at allocation,
        // and a zero slot is skipped anyway, but building first means the
        // answer does not depend on knowing that.
        let mut free = resume_free_set();
        let mut cursor = 0usize;
        let at = arena_alloc(
            activation_id,
            REC_KIND_RESUME_ASSIGNMENT,
            sorted.len() * RESUME_ASSIGNMENT_RECORD_BYTES,
        )?;
        let mut assigned = 0u32;
        for (index, ordinal) in sorted.iter().enumerate() {
            let slot = match resume_take_free(&mut free, &mut cursor) {
                Some(slot) => slot,
                None => {
                    let fresh = RESUME_NEXT_SLOT.load(Ordering::Relaxed);
                    RESUME_NEXT_SLOT.store(fresh + 1, Ordering::Relaxed);
                    fresh
                }
            };
            let record = at + (index * RESUME_ASSIGNMENT_RECORD_BYTES) as u64;
            arena_set_u32(record, *ordinal);
            arena_set_u32(record + 4, slot);
            assigned += 1;
        }
        Ok(assigned)
    }
    ...
    fn resume_unregister_impl(activation_id: u32, lenient: bool) -> Result<u32, Errno> {
        ...
        // `release_identity_activation` and `arena_release_activation` ONLY
        // from its `Ok` arm, so an `Err` on a never-registered activation would
        // leak every arena record and identity entry that activation owns. And
        // reading "zero slots" as "never registered" already cost a real fork
        // (see this function's doc comment). No record, or a record with no
        // pairs, both answer 0 and both still run the release.
        let Some((at, byte_len)) = arena_find(activation_id, REC_KIND_RESUME_ASSIGNMENT)
        else {
            return Ok(0);
        };
        let pairs = byte_len / RESUME_ASSIGNMENT_RECORD_BYTES;

        // PASS 1: null, mutating nothing.
        for index in 0..pairs {
            let record = at + (index * RESUME_ASSIGNMENT_RECORD_BYTES) as u64;
            resume_null_via_injector(arena_u32(record + 4), lenient);
        }

        // PASS 2: CHECK. There is nothing to free -- dropping the record IS
        // the free, because the free set is derived from the live records -- so
        // what is left is the bound the old `free_bits_mark` carried, kept for
        // the job it turned out to have.
        //
        ...
        // is here because the thing it caught turned out to be real.
        //
        // The first attempt made `host/test/vfork-lifecycle-guest.test.ts`
        // report `fm_resume_slots failed with errno 22` from a vfork child's
        // exit teardown, on a run that was otherwise entirely healthy, and the
        // bound was removed rather than guessed at. Instrumenting the host to
        // read the watermark at every `fm_*` call answered it: the watermark
        // was 0. No writer produces 0 -- `set_format_impl` stores 1 and the
        // allocator stores `fresh + 1`, which is at least 2 -- so 0 is not a
        // reset, it is a ZEROED PAGE. The borrowed (vfork) child had
        ...
        // module teardown ABOVE that munmap and drops its handles below it.
        //
        ...
        // memory is gone. Removing it puts that corruption back to silent.
        //
        ...
        for index in 0..pairs {
            let record = at + (index * RESUME_ASSIGNMENT_RECORD_BYTES) as u64;
            let slot = arena_u32(record + 4);
            if (slot == 0 || slot >= watermark) && failure.is_none() {
                failure = Some(Errno::EINVAL);
            }
        }

        // THE RECORD GOES EITHER WAY, for the same reason the entries used to:
        ...
        match failure {
            ...
            None => Ok(pairs as u32),
        }
    }

    // WHAT USED TO BE HERE: `resume_slot_of(activation_id, ordinal)`, a linear
    // scan of the old shared slot index answering "which slot did this ONE
    ...
    ///
    /// ASCENDING BY ORDINAL BY CONSTRUCTION. `resume_register_impl` sorts once
    /// and writes the pairs in that order into a record that is never compacted
    /// -- where the old shared index swap-removed on every free, so an
    /// activation's entries came back in an order that depended on the worker's
    /// `dlclose` history. `host/test/fork-resume-slot-bitmap.test.ts` asserts
    /// the order, because it has never been asserted anywhere and the consumer
    /// that sorts could stop.
    fn resume_assignment_of(activation_id: u32) -> Option<alloc::vec::Vec<(u32, u32)>> {
        let (at, byte_len) = arena_find(activation_id, REC_KIND_RESUME_ASSIGNMENT)?;
        let pairs = byte_len / RESUME_ASSIGNMENT_RECORD_BYTES;
        if pairs == 0 {
            return None;
        }
        let mut out = alloc::vec::Vec::with_capacity(pairs);
        for index in 0..pairs {
            let record = at + (index * RESUME_ASSIGNMENT_RECORD_BYTES) as u64;
            out.push((arena_u32(record), arena_u32(record + 4)));
        }
        Some(out)
    }
    ...
    /// for the reason the activation catalog's old floor gave: 64 KiB of BSS is
    /// reserved whether a program forks or not, and reserving the worst case
    /// -- 65,536 records, 512 KiB -- would take that much out of every guest's
    /// mmap window to serve the one program that needs it.
    ///
    /// # THE NUMBER IS INHERITED, NOT DERIVED, AND PHP CROSSES IT
    ///
    /// 8,192 is the old activation-catalog floor's record count carried over
    ...
    fn publish_resume_assignment_impl(activation_id: u32) -> Result<(u32, u32), Errno> {
        ...
        // ASCENDING BY ORDINAL, and it now arrives that way. The old shared
        // index compacted by moving the LAST live entry into the hole a free
        // made, so after any `dlclose` an activation's entries were no longer
        // in the order they were assigned, and a published order that depended
        // on a worker's dlclose history would make the recorded placement
        // baseline irreproducible for a reason that has nothing to do with
        // where thunks land. A per-activation record is written once, in sorted
        // order, and never compacted -- so this sort is now a BELT, not the
        // braces. It stays because the ordering is a published contract and one
        // line is cheap; `host/test/fork-resume-slot-bitmap.test.ts` asserts the
        // order so that the guarantee has a test rather than only a sort.
    ...
    /// seeded one for it.
    ///
    /// A view into the record's payload in guest memory, valid until the
    /// record is released or replaced; every caller consumes it before the
    /// next module call.
    fn activation_catalog(activation_id: u32) -> Option<&'static [u32]> {
        let (at, byte_len) = arena_find(activation_id, REC_KIND_RESUME_CATALOG)?;
        guest_u32s(at, byte_len / 4).ok()
    }

    /// Register `activation_id`'s resume targets into `table`.
    ///
    /// ADOPT the worker-lifetime assignment when there is one. It was made at
    /// catalog-seed time, before any fork, and it is the numbering the host
    /// placed its thunks by -- re-deriving it here is what let the two drift
    /// after a `dlclose` (see the allocator's comment above). An activation
    /// whose assignment is EMPTY has no record to adopt and registers from its
    /// (empty) catalog instead, which is the same answer.
    ///
    /// WHAT THIS NO LONGER DOES: fall through to a process-wide catalog, then
    /// to the activation's distinct committed ordinals, for a worker that
    /// seeded nothing. Both arms are gone, together: removing only the
    /// process-wide arm left the committed-ordinal one unreachable-but-present,
    /// and removing only that one turned "never seeded" into a silent
    /// no-slots registration.
    ...
    // defect that broke an earlier chunked slot index, and it is
    ...
    fn begin_replay_impl() -> Result<(), Errno> {
        ...
        let mem = unsafe { mem_ref() };
        // Register each activation's resume targets from its own seeded
        // catalog (adopting the assignment made at seed time).
        let ForkModule {
            activations, table, ..
        } = st;
        for (activation_id, act) in activations.iter_mut() {
            let driver = RewindDriver::attach(mem, act.module_buffer, &act.format)?;
            register_activation_slots(table, *activation_id)?;
    ...
    fn build_child_replay_module(
        ...
        // Register the seed activation's resume slots from its own seeded
        // catalog (adopting the assignment made at seed time).
        let mut table = ResumeSlotTable::new();
        register_activation_slots(&mut table, activation_id)?;
    ...
    fn add_activation_borrowed_child_replay_impl(
        ...
        register_activation_slots(&mut st.table, activation_id)?;
    ...
    const FM_STATS_HIGH_FIELDS: [u32; 5] = [
        ...
        // 103 stays RESERVED. It was claimed by this plan for the resume
        // free-slot bitmap's chunk-chain length, and there is no chain: the
