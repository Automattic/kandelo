...
# substrate linked (rootfs.rs, SffsWriter, sffs_container).
#
# Usage:
#   scripts/dev-shell.sh bash crates/sffs-module/build-wasm.sh
#   scripts/dev-shell.sh bash crates/sffs-module/build-wasm.sh --verify-fresh
#
# Artifact: target/wasm32-unknown-unknown/release/sffs_module.wasm
# Staged as: local-binaries/sffs_module32.wasm
...
# Naming runtime-core explicitly as well as sffs-module: `cargo_closure_paths`
...
SFFS_MODULE_CLOSURE_CRATES="sffs-module,runtime-core"
...
closure_sha() {
  cargo run -q -p xtask --target "$HOST_TRIPLE" -- workspace-closure-sha \
    ...
    --recipe crates/sffs-module/build-wasm.sh
}

build_key_path() {
  echo "$REPO_ROOT/local-binaries/sffs_module${1}.wasm.build-key"
}

shrink_module() {
  local wasm="$1"
  if ! command -v wasm-opt >/dev/null 2>&1; then
    echo "sffs-module: wasm-opt is unavailable, so $wasm cannot be" \
  ...
  echo "sffs-module: wasm-opt -Oz ${before} -> ${after} bytes" >&2
}
...
verify_no_imports() {
  ...
  if ! command -v wasm-objdump >/dev/null 2>&1; then
    echo "sffs-module: wasm-objdump is unavailable, so the zero-import" \
  ...
  if [[ "$imports" != "0" ]]; then
    echo "sffs-module: $wasm declares $imports import(s), but the module" \
  ...
  echo "sffs-module: $wasm imports nothing (contract holds)" >&2
}

if [[ "${1:-}" == "--verify-fresh" ]]; then
  current_sha="$(closure_sha)"
  artifact="local-binaries/sffs_module32.wasm"
  ...
  if [[ ! -f "$artifact" ]]; then
    echo "sffs-module: $artifact is missing; build it with" \
      "'bash crates/sffs-module/build-wasm.sh'." >&2
    exit 1
  fi
  if [[ ! -f "$key_path" ]]; then
    echo "sffs-module: $artifact carries no build-key stamp ($key_path is" \
      "missing); rebuild so freshness can be verified." >&2
    exit 1
  fi
  staged_sha="$(cat "$key_path")"
  if [[ "$staged_sha" != "$current_sha" ]]; then
    echo "sffs-module: $artifact is stale: it was built for closure key" \
      "$staged_sha, but the current source tree" \
      "($SFFS_MODULE_CLOSURE_CRATES) resolves to $current_sha. Rebuild with" \
      "'bash crates/sffs-module/build-wasm.sh'." >&2
    exit 1
  fi
  echo "sffs-module: $artifact matches current source ($current_sha)" >&2
  exit 0
fi

echo "== building sffs-module (standalone module, wasm32) ==" >&2
RUSTFLAGS="${MODULE_RUSTFLAGS[*]}" \
  cargo build --release -p sffs-module --target wasm32-unknown-unknown \
    -Z build-std=core,alloc

WASM32="target/wasm32-unknown-unknown/release/sffs_module.wasm"
...
cp "$WASM32" "$REPO_ROOT/local-binaries/sffs_module32.wasm"
printf '%s\n' "$FRESH_CLOSURE_SHA" > "$(build_key_path 32)"
echo "staged sffs_module32.wasm -> local-binaries/ (build-key $FRESH_CLOSURE_SHA)" >&2
