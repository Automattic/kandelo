...
/// On wasm, calls the host import. On native (tests), returns 0.
...
// ---------------------------------------------------------------------------
// Wasm memory intrinsics (abstraction over wasm32/wasm64)
// ---------------------------------------------------------------------------
...
/// Grow Wasm memory by `pages` pages. Returns previous size or usize::MAX on failure.
#[cfg(target_arch = "wasm64")]
#[inline]
...
mod wasm {
    ...
    /// memory just enough to cover the allocation.
    struct BumpAlloc {
        cursor: AtomicUsize,
    }
    ...
    unsafe impl GlobalAlloc for BumpAlloc {
        unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
            ...
            loop {
                ...
                if cur == 0 {
                    let base = unsafe { &__heap_base as *const usize as usize };
                ...
                let current_bytes = crate::wasm_memory_size() * 65536;
                if new_cursor > current_bytes {
                    let needed_pages = (new_cursor - current_bytes + 65535) / 65536;
                    let prev = crate::wasm_memory_grow(needed_pages);
                    if prev == usize::MAX {
