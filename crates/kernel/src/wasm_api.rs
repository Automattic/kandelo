...
/// Execute a new program. Returns 0 once the kernel and host have accepted the
/// replacement, or negative errno on failure.
///
/// The reusable kernel Wasm must return through its shadow-stack epilogue even
/// though POSIX `execve` does not return to the old guest image. The host owns
/// stopping that guest Worker after this transaction succeeds.
#[unsafe(no_mangle)]
pub extern "C" fn kernel_execve(path_ptr: *const u8, path_len: u32) -> i32 {
    ...
    match result {
        Ok(()) => {
            // Preserve the one-shot binding contract for direct export users;
            // kernel_handle_channel clears the same binding again on return.
            unsafe { &mut *PROCESS_TABLE.0.get() }.clear_current_tid_binding();
            0
        }
        Err(e) => -(e as i32),
...
/// Same return contract as kernel_execve.
#[unsafe(no_mangle)]
