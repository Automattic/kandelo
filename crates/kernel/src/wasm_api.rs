...
pub extern "C" fn kernel_sendmsg(fd: i32, msg_ptr: *const u8, flags: u32) -> i32 {
    ...
    if result > 0 && ancillary_fds.as_ref().is_some_and(|fds| !fds.is_empty()) {
        // Push the already-retained ownership records to the socket's send pipe.
                    ...
                    if let Some(sock) = proc.sockets.get(sock_idx) {
                        if let Some(send_idx) = sock.send_buf_idx {
                            ...
                            if let Some(pipe) = pipe {
                                pipe.push_ancillary(ancillary_fds.take().unwrap());
                            }
                        }
                    }
                }
...
pub extern "C" fn kernel_recvmsg(fd: i32, msg_ptr: *mut u8, flags: u32) -> i32 {
    ...
    if result > 0 {
        ...
        if let Some(in_flight) = popped {
            let required_data_len = in_flight.len() * 4;
            let required_cmsg_len = 12 + required_data_len;
            let required_cmsg_space = (required_cmsg_len + 3) & !3;
            // Validate the complete control buffer before transferring any
            // queued references into receiver-side descriptors.
            if control_ptr != 0 && required_cmsg_space <= control_len {
                let new_fds = syscalls::install_scm_rights_fds(proc, in_flight);
