...
//! ```
//!
//! The host never re-enters wasm while a `dlopen` import frame is live. That
//! is already an act-queue protocol driven from the guest side, which is why
//! this port is a driver rather than a straight-line function: the same shape
//! serves a synchronous native embedder, a JS generator, and libc's staged
//! loader without any of them changing the others' semantics.
//!
//! # Driving a plan
//!
//! ```text
...
//! loop {
//!     match plan.step(&mut linker)? {
//!         PlanStep::Act(act)   => { let r = executor.perform(act)?; plan.resume(r)?; }
//!         PlanStep::Host(req)  => { let r = host.perform(req)?;     plan.resume(r)?; }
//!         PlanStep::Call(call) => { guest.invoke(call)?;            plan.resume(ActResult::Done)?; }
//!         PlanStep::Finished   => break,
//!     }
//! }
//! let library = plan.finish(&mut linker)?;
//! ```
//!
//! Every `step` is total: a plan driven out of order returns
//! [`DylinkError::UnexpectedActSequence`] instead of misbehaving.
...
use crate::act::{
    ActResult, BindingValue, GlobalId, ImportPlan, InstanceExport, InstanceId, LinkAct, ModuleId,
    ModuleSource, PointerWidth, TableValue, TagId, WasmValue,
};
use crate::error::{DylinkError, DylinkResult};
use crate::got::{decide_got_cell, GotCell, GotInit, GotPlacement, GotRequest, GotTable, UnresolvedPolicy};
...
use crate::wasm::{
    ...
    ImportType, ModuleShape, ValType,
};
...
pub enum HostRequest {
    ...
    /// Release an allocation during rollback. Answered with
    /// [`ActResult::Done`].
    ...
    /// the function as an activation+ordinal recipe.
...
pub struct LinkerConfig {
    ...
    /// True when a fork activation coordinator is available. An ABI-43 side
    /// module cannot be loaded without one.
    ...
    /// Whether the memory is shared (required for a borrowed replay).
...
/// The process's linker: scope, GOT, handles and executor id allocation.
...
pub struct Linker {
    ...
    next_instance: u32,
    next_tag: u32,
    /// The process-owned exception tags, shared by the main image and every
    /// side module. C++ exceptions crossing a side-module call require tag
    /// IDENTITY as well as a matching payload type, so these must not be
    /// allocated per `dlopen`.
...
pub struct LinkPlan {
    ...
    got_cursor: usize,
    /// The final ordered binding list, one per import declaration.
    bindings: Option<Vec<crate::act::ImportBinding>>,
    ...
    pending_got_writes: Vec<(GotKind, String, GlobalId, WasmValue, GotPlacement)>,
    got_write_cursor: usize,

    tls_base: Option<u64>,
    owned_table_entries: alloc::collections::BTreeSet<u64>,
    provider_dependencies: alloc::collections::BTreeSet<String>,
...
struct PublishSite {
    name: String,
    kind: ExternKind,
    /// For a function: the slot it will occupy.
    table_index: Option<u64>,
    /// For a data symbol: its relocated address.
    address: Option<u64>,
}
...
impl LinkPlan {
    ...
    pub fn begin(linker: &mut Linker, request: LoadRequest) -> DylinkResult<Self> {
        ...
        // Fork-artifact admission. These are K12's contracts evaluated at the
        // loader's call site; the planner enforces the ones that decide whether
        // the object may be loaded at all.
        let fork_instrumented = shape_is_fork_instrumented(&shape);
        ...
        if !fork_instrumented && shape_has_partial_fork_instrumentation(&shape) {
        ...
        // fresh allocation, which in a fork child would choose new addresses and
        // corrupt every copied relocation.
        ...
        Ok(LinkPlan {
            ...
            publish: Vec::new(),
            publish_cursor: 0,
            pending_got_writes: Vec::new(),
            got_write_cursor: 0,
            tls_base: None,
            owned_table_entries: alloc::collections::BTreeSet::new(),
    ...
    /// The ordered import bindings, available once the plan has reached
    /// [`Phase::Instantiate`]. Exposed for the differential harness.
    pub fn bindings(&self) -> Option<&[crate::act::ImportBinding]> {
    ...
    /// the caller nulls the slots this plan claimed and lets the next
    /// successful archive entry record the resulting exact base. The returned
    /// requests release process mappings and the prepared activation.
    ...
    /// Slots this plan made addressable, so a rollback can null them.
    pub fn rollback_table_range(&self, linker: &Linker) -> Option<(u64, u64)> {
        let start = self.scope_snapshot_table_length();
        let end = linker.scope.table_length();
        (end > start).then_some((start, end - start))
    }

    fn scope_snapshot_table_length(&self) -> u64 {
        // The snapshot is opaque by design; the plan recorded the base it grew
        // from as `table_base - table_pad`.
        self.table_base.saturating_sub(self.table_pad)
    }
    ...
    pub fn step(&mut self, linker: &mut Linker) -> DylinkResult<PlanStep> {
        ...
        loop {
            match self.phase {
                Phase::Compile => {
                    self.pending = Some(Pending::Compile);
                    return Ok(PlanStep::Act(LinkAct::Compile {
                        module: self.module,
                        source: self.source.clone(),
                    }));
                }
                Phase::Memory => {
                    if let Some(step) = self.memory_step(linker)? {
                        return Ok(step);
                    }
                }
                Phase::Table => {
                    if let Some(step) = self.table_step(linker)? {
                        return Ok(step);
                    }
                }
                Phase::BaseGlobals => {
                    if let Some(step) = self.base_global_step(linker)? {
                        return Ok(step);
                    }
                }
                Phase::Tags => {
                    if let Some(step) = self.tag_step(linker)? {
                        return Ok(step);
                    }
                }
                Phase::Got => {
                    if let Some(step) = self.got_step(linker)? {
                        return Ok(step);
                    }
                }
                Phase::Activation => {
                    if self.fork_instrumented && self.activation_id.is_none() {
                        self.pending = Some(Pending::PrepareActivation);
                        return Ok(PlanStep::Host(HostRequest::PrepareActivation {
                            library: self.request.name.clone(),
                ...
                Phase::Instantiate => {
                    ...
                    self.pending = Some(Pending::Instantiate);
                    return Ok(PlanStep::Act(LinkAct::Instantiate {
                        module: self.module,
                ...
                Phase::Register => {
                    if let Some(activation) = self.activation_id {
                        self.pending = Some(Pending::RegisterActivation);
                        return Ok(PlanStep::Host(HostRequest::RegisterActivation {
                            library: self.request.name.clone(),
                            activation,
                            instance: self.instance,
                        }));
                ...
                Phase::Exports => {
                    self.pending = Some(Pending::ReadExports);
                    return Ok(PlanStep::Act(LinkAct::ReadExports { instance: self.instance }));
                }
                Phase::Bootstrap => {
                    if self.runs_stage(InitializationStage::Bootstrap) {
                        self.pending = Some(Pending::Stage(InitializationStage::Bootstrap));
                        return Ok(PlanStep::Call(self.staged_call(InitializationStage::Bootstrap)));
                    }
                ...
                Phase::Relocate => {
                    if let Some(step) = self.relocate_step()? {
                        return Ok(step);
                    }
                }
                Phase::Publish => {
                    if let Some(step) = self.publish_step(linker)? {
                        return Ok(step);
                    }
                }
                Phase::Relocations => {
                    if self.runs_stage(InitializationStage::Relocations) {
                        self.pending = Some(Pending::Stage(InitializationStage::Relocations));
                        return Ok(PlanStep::Call(
                            self.staged_call(InitializationStage::Relocations),
                        ));
                    }
                    self.phase = Phase::Constructors;
                }
                Phase::Constructors => {
                    if self.runs_stage(InitializationStage::Constructors) {
                        self.pending = Some(Pending::Stage(InitializationStage::Constructors));
                        return Ok(PlanStep::Call(
                            self.staged_call(InitializationStage::Constructors),
                        ));
                    }
    ...
    pub fn resume(&mut self, linker: &mut Linker, result: ActResult) -> DylinkResult<()> {
        ...
        match pending {
            ...
            Pending::Allocate => {
                ...
                self.pending = Some(Pending::Zero);
            }
            Pending::Adopt(index) => {
                result.expect_done()?;
                let next = index + 1;
                if next < self.archived_allocations().len() {
                    self.pending = Some(Pending::Adopt(next));
                }
            }
            ...
            Pending::BaseGlobal(which) => {
                result.expect_done()?;
                match which {
                    BaseGlobal::Memory => {}
                    BaseGlobal::Table => self.phase = Phase::Tags,
                }
            }
            Pending::Tag(slot) => {
                result.expect_done()?;
            ...
            Pending::PrepareActivation => {
                let activation = u32::try_from(result.expect_index()?)
                    .map_err(|_| DylinkError::ActResultMismatch { expected: "Index" })?;
                if activation == 0 {
                    return Err(DylinkError::ActResultMismatch { expected: "Index" });
                }
            ...
            Pending::RelocatedGlobal(index) => {
                ...
                let address = relocation
                    .raw
                    .as_u64()
                    .checked_add(relocation.base)
                    .ok_or(DylinkError::AllocationEscapesMemory {
                        library: self.request.name.clone(),
                    })?;
                self.relocated
                    .insert(relocation.name.clone(), SymbolValue::Data { address });
            ...
            Pending::ExportTableGrow(index) => {
                ...
                self.pending = Some(Pending::ExportTableWrite(index));
            }
            Pending::ExportTableWrite(index) => {
                result.expect_done()?;
                self.pending = Some(Pending::ExportJournal(index));
            }
            Pending::ExportJournal(index) => {
                result.expect_done()?;
                let site = self.publish[index].clone();
                if let Some(slot) = site.table_index {
                    linker.scope.record_function_slot(self.instance, &site.name, slot);
                }
                self.publish_cursor = index + 1;
            }
            Pending::GotWrite => {
                result.expect_done()?;
                self.got_write_cursor += 1;
            }
        }
        Ok(())
    }
    ...
    fn archived_allocations(&self) -> &[DylinkAllocation] {
        self.request
            .replay
            .as_ref()
            .map(|replay| replay.allocations.as_slice())
            .unwrap_or(&[])
    }

    fn memory_step(&mut self, linker: &mut Linker) -> DylinkResult<Option<PlanStep>> {
        if let Some(pending) = self.pending.clone() {
            return Ok(match pending {
                Pending::Zero => {
                    self.pending = Some(Pending::Zero);
                    Some(PlanStep::Act(LinkAct::ZeroMemory {
                        address: self.memory_base,
                        length: self.metadata.memory_size,
                    }))
                }
                Pending::Adopt(index) => {
                    let allocation = self.archived_allocations()[index].clone();
                    self.pending = Some(Pending::Adopt(index));
                    Some(PlanStep::Host(HostRequest::AdoptMapping {
                        library: self.request.name.clone(),
                        allocation,
                    }))
                }
        ...
        if self.memory_placement == MemoryPlacement::None && self.metadata.memory_size > 0 {
            let replay_base = self.request.replay.as_ref().map(|replay| replay.memory_base);
            self.memory_placement = plan_memory(
                &self.request.name,
            ...
            match self.memory_placement.clone() {
                MemoryPlacement::None => {}
                MemoryPlacement::Allocate { size, align } => {
                    self.pending = Some(Pending::Allocate);
                    return Ok(Some(PlanStep::Host(HostRequest::AllocateMemory {
                        library: self.request.name.clone(),
                        size,
                        align,
                    })));
                ...
                MemoryPlacement::BumpHeap { base, end, grow_pages } => {
                    ...
                    if grow_pages > 0 {
                        self.pending = Some(Pending::GrowMemory);
                        linker.config.memory_bytes += grow_pages * 65_536;
                        return Ok(Some(PlanStep::Act(LinkAct::GrowMemory {
                            delta_pages: grow_pages,
                        })));
                    }
                    self.pending = Some(Pending::Zero);
                    return Ok(Some(PlanStep::Act(LinkAct::ZeroMemory {
                        address: base,
                        length: self.metadata.memory_size,
                    })));
                }
                MemoryPlacement::Replay { base } => {
                    self.memory_base = base;
                    let allocations = self.archived_allocations().to_vec();
                    ...
                    if !self.allocations.is_empty() {
                        self.pending = Some(Pending::Adopt(0));
                        let allocation = self.allocations[0].clone();
                        return Ok(Some(PlanStep::Host(HostRequest::AdoptMapping {
                            library: self.request.name.clone(),
                            allocation,
                        })));
                    }
        ...
        } else if self.metadata.memory_size == 0 && !self.archived_allocations().is_empty() {
    ...
    fn table_step(&mut self, linker: &mut Linker) -> DylinkResult<Option<PlanStep>> {
        if self.table_base == 0 && self.table_pad == 0 {
            ...
            if placement.pad > 0 {
                self.pending = Some(Pending::TablePad);
                return Ok(Some(PlanStep::Act(LinkAct::GrowTable { delta: placement.pad })));
            }
        }
        if self.metadata.table_size > 0 && self.owned_table_entries.is_empty() {
            self.pending = Some(Pending::TableReserve);
            return Ok(Some(PlanStep::Act(LinkAct::GrowTable {
                delta: self.metadata.table_size,
            })));
        }
    ...
    fn base_global_step(&mut self, linker: &mut Linker) -> DylinkResult<Option<PlanStep>> {
        if self.memory_base_global.is_none() {
            let global = linker.got.allocate_global();
            self.memory_base_global = Some(global);
            self.pending = Some(Pending::BaseGlobal(BaseGlobal::Memory));
            return Ok(Some(PlanStep::Act(LinkAct::NewGlobal {
                global,
        ...
        if self.table_base_global.is_none() {
            let global = linker.got.allocate_global();
            self.table_base_global = Some(global);
            self.pending = Some(Pending::BaseGlobal(BaseGlobal::Table));
            return Ok(Some(PlanStep::Act(LinkAct::NewGlobal {
                global,
                ty: self.width.val_type(),
                mutable: false,
    ...
    fn tag_step(&mut self, linker: &mut Linker) -> DylinkResult<Option<PlanStep>> {
        if self.needs_longjmp_tag && self.longjmp_tag.is_none() {
            let tag = linker.allocate_tag();
            self.longjmp_tag = Some(tag);
            self.pending = Some(Pending::Tag(TagSlot::Longjmp));
            return Ok(Some(PlanStep::Act(LinkAct::NewTag {
                tag,
        ...
        if self.needs_cpp_tag && self.cpp_exception_tag.is_none() {
            ...
            self.pending = Some(Pending::Tag(TagSlot::CppException));
            return Ok(Some(PlanStep::Act(LinkAct::NewTag {
                tag,
                parameters: alloc::vec![self.width.val_type()],
            })));
    ...
    fn got_step(&mut self, linker: &mut Linker) -> DylinkResult<Option<PlanStep>> {
        while self.got_cursor < self.got_imports.len() {
            ...
            // exactly as the TypeScript `Proxy` returns the same Global for a
            ...
            let resolved_table_index = resolved.as_ref().and_then(|resolved| match &resolved.value {
            ...
            if self.request.replay.is_some() && kind == GotKind::Func && replay_value.is_none() {
                return Err(DylinkError::MissingSavedGotValue {
                    library: self.request.name.clone(),
                    symbol: symbol.clone(),
                });
            }
            ...
            // An existing shared cell is reused; a shared cell that resolves
            // better now is rewritten by the publish phase, not here.
            ...
            self.pending = Some(Pending::GotGlobal(index));
            return Ok(Some(PlanStep::Act(LinkAct::NewGlobal {
                global,
                ty: self.width.val_type(),
                mutable: true,
    ...
    fn build_bindings(
        &mut self,
        linker: &Linker,
    ) -> DylinkResult<Vec<crate::act::ImportBinding>> {
    ...
    fn bind_env_import(
        ...
        if let Some(resolved) = linker.scope.scoped_symbol(&self.dependency_scope, &decl.name) {
            return Ok(match resolved.value {
                ...
                SymbolValue::Data { .. } => BindingValue::Export {
                    instance: crate::scope::MAIN_INSTANCE,
    ...
    fn runs_stage(&self, stage: InitializationStage) -> bool {
        let replay = self.request.replay.as_ref();
        ...
        // pointers twice and clobber post-startup state. An IN-FLIGHT replay
        // still yields the full sequence so the archived selector can stop at
        // the exact guest call being resumed.
        if replay.is_some_and(|replay| replay.initialization_stage.is_none()) {
            return false;
        }
        match stage {
    ...
    fn plan_tls(&mut self) -> DylinkResult<()> {
        let tls_size = self.global_export_value("__tls_size");
        ...
        let Some(tls_size) = tls_size.map(WasmValue::as_u64) else {
            if self.request.replay.as_ref().and_then(|replay| replay.tls_base).is_some() {
        ...
        let base = match self.request.replay.as_ref().and_then(|replay| replay.tls_base) {
            Some(base) => base,
    ...
    ///
    /// An IMMUTABLE global export is a data address, relative to the module's
    /// own base; a mutable one is instance state and passes through.
    /// `__tls_size` / `__tls_align` are scalar ABI facts, not addresses.
        ...
        for export in &self.exports {
            match export.kind {
                ExternKind::Global => {
                    ...
                    if mutable
                        || export.name == "__tls_size"
                        || export.name == "__tls_align"
                    {
                        self.relocated
                            .insert(export.name.clone(), SymbolValue::Data { address: raw.as_u64() });
                        continue;
                    }
    ...
    fn relocate_step(&mut self) -> DylinkResult<Option<PlanStep>> {
        if self.relocation_cursor < self.relocations.len() {
            let index = self.relocation_cursor;
            let relocation = &mut self.relocations[index];
            ...
            let global = GlobalId(u32::MAX - index as u32);
            relocation.global = Some(global);
            self.pending = Some(Pending::RelocatedGlobal(index));
            return Ok(Some(PlanStep::Act(LinkAct::NewGlobal {
                global,
                ty: self.width.val_type(),
                mutable: false,
    ...
    fn publish_step(&mut self, linker: &mut Linker) -> DylinkResult<Option<PlanStep>> {
        // Functions need a table slot before anything can take their address.
        while self.publish_cursor < self.publish.len() {
            let index = self.publish_cursor;
            let site = self.publish[index].clone();
            if site.kind == ExternKind::Func && site.table_index.is_none() {
                self.pending = Some(Pending::ExportTableGrow(index));
                return Ok(Some(PlanStep::Act(LinkAct::GrowTable { delta: 1 })));
            }
            if site.kind == ExternKind::Func {
                if let Some(pending) = self.pending.clone() {
                    return Ok(match pending {
                        Pending::ExportTableWrite(_) => Some(PlanStep::Act(LinkAct::WriteTable {
                            index: site.table_index.unwrap_or_default(),
                            value: TableValue::Export {
                                instance: self.instance,
                                name: site.name.clone(),
                            },
                        })),
                        Pending::ExportJournal(_) => {
                            Some(PlanStep::Host(HostRequest::JournalTableMutation {
                                first_index: site.table_index.unwrap_or_default(),
                                length: 1,
                            }))
                        }
                        _ => None,
        ...
        if self.pending_got_writes.is_empty() {
            self.collect_got_writes(linker)?;
        }
        while self.got_write_cursor < self.pending_got_writes.len() {
            let (kind, symbol, global, value, placement) =
                self.pending_got_writes[self.got_write_cursor].clone();
            match placement {
                GotPlacement::Shared => linker.got.set_shared_value(&symbol, value),
                GotPlacement::Local => {
                    if let Some(cell) = self.local_got.get_mut(&(kind, symbol.clone())) {
                        cell.value = value;
                    }
                }
            }
            self.pending = Some(Pending::GotWrite);
            return Ok(Some(PlanStep::Act(LinkAct::WriteGlobal { global, value })));
        }
    ...
    fn collect_got_writes(&mut self, linker: &Linker) -> DylinkResult<()> {
        for site in &self.publish {
            ...
            // A shared cell is only republished when this object is the symbol's
            // first definition. Overwriting an existing global definition would
            // break ELF interposition: the first definition wins for the whole
            // process, and the guest already holds pointers derived from it.
    ...
    fn commit_symbols(&mut self, linker: &mut Linker) -> DylinkResult<()> {
        ...
        for site in &self.publish {
            let value = match (site.kind, site.address) {
                (ExternKind::Func, _) => SymbolValue::Func {
                    instance: self.instance,
                    export: site.name.clone(),
                },
                (ExternKind::Global, Some(address)) => SymbolValue::Data { address },
                _ => continue,
            };
    ...
    /// Take the loaded object out of the plan once it reports `Finished`.
...
fn shape_is_fork_instrumented(shape: &ModuleShape) -> bool {
    wasm_posix_shared::abi::WPK_FORK_REQUIRED_EXPORTS
        .iter()
        .all(|required| shape.exports_function(required.name))
}

fn shape_has_partial_fork_instrumentation(shape: &ModuleShape) -> bool {
    wasm_posix_shared::abi::WPK_FORK_REQUIRED_EXPORTS
        .iter()
        .any(|required| shape.exports_function(required.name))
}

/// A `GOT.*` import's declared global type must be a mutable pointer-width
/// global. Exposed so an executor can assert the module it is handed agrees
/// with the process's pointer width before anything is constructed.
pub fn check_got_import_type(ty: &ImportType, width: PointerWidth) -> bool {
    matches!(
        ty,
