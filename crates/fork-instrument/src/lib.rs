...
/// `kernel.kernel_fork`) do not have user functions rewritten, but still
/// receive the uniform activation-state helpers and imports. They may be
/// loaded beside a fork-capable main module, so their mutable globals, tables,
/// and segment lifetimes remain part of the child process image even when no
/// activation from that module is on the fork stack.
pub fn instrument(input: &[u8], opts: &Options) -> Result<Vec<u8>> {
    ...
    // Every loaded module is part of the process image even when none of its
    // functions can be on the active fork stack. Give no-memory modules the
    // shared process-memory staging contract before deriving the pointer ABI,
    // and give every artifact the same linked imports and state helpers.
    //
