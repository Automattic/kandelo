...
| **AF_UNIX datagram overflow is lossy** | socket | AF_UNIX datagrams preserve message boundaries and order under ordinary load, but their fixed 128-message receive queue currently evicts the oldest message on overflow instead of blocking the sender (or returning EAGAIN for a nonblocking send) as a reliable Unix datagram transport should. |
