...
| `sendto()` / `recvfrom()` | Partial | AF_INET UDP loopback and local virtual-network send/receive, connected and unconnected sendto, source address reporting, and connected receive filtering are implemented. Loopback (`127.0.0.1`) datagrams are delivered across processes on the same machine — not just within one process — matching the cross-process loopback path already provided for TCP `connect()`. External raw UDP routes return ENETUNREACH and are narrowly xfailed in the Sortix UDP suite. |
