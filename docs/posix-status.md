...
| `execve()` | Partial | Delegates to the in-place `exec()` path and has the same remaining descriptor/signal/mapping limitations described above. `#!` scripts are resolved by the kernel (`resolve_shebang`): the interpreter line is scanned to at most 4,096 bytes, yields the interpreter plus exactly one optional argument, and the script's own path becomes the interpreter's first argument. Exactly ONE interpreter retarget is performed. Remaining limitation: a script whose interpreter is itself a `#!` script fails `ENOEXEC`, whereas Linux permits roughly four binfmt rewrites before failing `ELOOP`. The `posix_spawn()` preflight enforces the same one-level limit with the same errno, so a nested chain fails before a child or its file actions exist. |
...
| `select()` | Partial | Wrapper around poll(). Converts fd_set bitmasks to pollfd array. A finite wait holds one `CLOCK_MONOTONIC` deadline in the kernel for the life of the call, so host retries share it and a system clock change cannot move it; the call then finishes with a zero-time kernel pass, except for the descriptor-free sleep form. A caught signal interrupts a would-block retry, including the no-fd sleep path, with EINTR; ignored signals leave it parked and a concurrently ready result is preserved. |
| `poll()` | Partial | Checks readiness for regular files, pipes, and sockets. UDP poll reports queued datagrams, connected-peer filtering, EOF-like read shutdown, write-shutdown hangup, and pending socket errors. A finite wait holds one `CLOCK_MONOTONIC` deadline in the kernel for the life of the call, so targeted wakeups and safety retries share it and a system clock change cannot move it; the call then finishes with a zero-time kernel pass that clears `revents`. Returns EINTR on pending signals. |
| `ppoll()` | Partial | Wraps poll() with atomic signal mask swap: save → set → poll → restore. If the replacement mask makes a caught signal deliverable, its catcher observes that replacement mask together with sa_mask and the delivered signal. Per-TID LIFO contexts keep nested waits distinct; terminal completion, exact final-EINTR cancellation, or `longjmp`/`siglongjmp` abandonment retires each context once. A caught signal ends the wait with `EINTR` regardless of `SA_RESTART`, matching Linux's `ERESTARTNOHAND` for the select/poll family and the rest of that family here — `poll()`, `select()`, `pselect6()`, and `epoll_pwait()` all make the same choice. `tests/sortix/os-test/signal/ppoll-block-raise` and `ppoll-block-sleep-raise` cover it: each has no wake other than the signal, so a resubmitted `ppoll` would block forever, and both now return `ppoll: EINTR` as `signal.expect/` requires. The glue layer converts the timespec to milliseconds, so a sub-millisecond timeout is rounded; that rounding, and the readiness coverage `poll()` documents above, are why this stays Partial rather than Full. A finite call keeps its `CLOCK_MONOTONIC` deadline for that libc call frame across host retries and catcher syscalls, so nested or later identical-argument calls cannot inherit it; expiry still reaches the kernel so `revents` is copied back and the temporary mask is restored. |
| `pselect6()` | Partial | Wraps select() with an atomic signal-mask swap across the host retry loop. The pselect6-style `{sigset_t *, size_t}` argument supplies the mask; timeout precision is rounded to host milliseconds. A catcher observes the replacement mask while it runs, and exact final-EINTR cancellation restores the Rust-owned pre-wait mask once after the catcher. A finite wait preserves its `CLOCK_MONOTONIC` deadline and expires through a zero-time kernel pass. Caught signals interrupt a would-block retry with EINTR; with SA_RESTART, Kandelo deliberately selects POSIX's implementation-defined EINTR result instead of restart. |
| `epoll_create1()` | Full | Creates an epoll instance owned by the **open file description** the returned descriptor names, held in the kernel-global OFD-keyed backing table beside eventfd, timerfd, signalfd and memfd. EPOLL_CLOEXEC supported. `dup`, `fork`, and a non-CLOEXEC `exec` all reach the same instance; the instance is destroyed when the last descriptor for that description anywhere on the machine is closed, not when one process closes its copy. |
| `epoll_ctl()` | Partial | EPOLL_CTL_ADD, EPOLL_CTL_MOD, EPOLL_CTL_DEL; stores the interest set with events and data. A null `event` pointer is accepted only for EPOLL_CTL_DEL, which does not read it; EPOLL_CTL_ADD and EPOLL_CTL_MOD return `EFAULT` rather than registering an interest with zero events and zero data. Each interest is keyed on the pair (registered descriptor number, **OFD identity**), mirroring Linux's `(struct file *, fd)` key: a `dup` registered at another number is a second interest in the same description, a descriptor number reused by a later `open` is a different registration rather than `EEXIST`, and `EPOLL_CTL_DEL` on a reused number is `ENOENT` rather than a removal of the earlier description's interest. Because the instance belongs to the description, a `fork()` child's `epoll_ctl` is visible to the parent and the parent's is visible to the child. Remaining gaps: an interest naming a description the calling process can no longer reach is skipped rather than removed, so it is inert here where Linux would keep delivering it while a sibling process still holds that description; `EPOLLET`, `EPOLLONESHOT`, and `EPOLLRDHUP` are not modelled; and an epoll fd cannot be added to another epoll instance. |
| `epoll_pwait()` | Partial | Builds a pollfd set from the shared interest list, delegates to poll, and maps results back to `epoll_event`. Each interest is resolved to a descriptor the **calling** process currently holds — preferring the number used at registration, otherwise any descriptor naming the same open file description — so an inherited epoll fd works in a `fork()` child, a registration survives `dup` plus close of the original number, and a number a later `open` reused is never polled on the old registration's behalf. Remaining gaps: an interest whose description the caller cannot reach contributes nothing (see `epoll_ctl()`); the edge-triggered and one-shot modes are not modelled; the `sigmask` argument is accepted and validated but never applied, so `epoll_pwait()` behaves as `epoll_wait()` with respect to signals — the kernel implements the mask swap, but the runtime cannot supply it while it owns the wait as a series of zero-timeout readiness probes, because a mask applied per probe would be off in between, which is the exact race `epoll_pwait()` exists to close; closing this gap requires the kernel to own the wait. An empty interest list returns zero events immediately (it used to sleep the whole machine for the timeout). |
...
| `nanosleep()` | Partial | The kernel validates `tv_sec >= 0` and `tv_nsec < 1e9` and returns; the runtime holds the caller's channel for the requested duration and completes it on a timer. No thread blocks for the sleep, so one process sleeping does not stall the others. Interrupted by a caught signal with EINTR. The remaining time is not written back to `rem`. |
| `usleep()` | Full | Same shape as `nanosleep()`: the kernel validates and returns, and the runtime holds the caller for the requested microseconds. |
...
| Function | Status | Notes |
|----------|--------|-------|
| `msgget()` / `msgsnd()` / `msgrcv()` / `msgctl()` | Full | Host-side SysV message queues via SharedIpcTable. Key-based creation, blocking send/recv with message types, IPC_STAT/IPC_SET/IPC_RMID control. IPC_STAT/IPC_SET read and write the caller-width `msqid_ds` directly in the caller's memory: 96 bytes for wasm32 time64 and 120 bytes for wasm64 LP64. `msgsnd`/`msgrcv` likewise read and write `struct msgbuf` in place, including its caller-width `long mtype` prefix; a blocked `msgsnd` sends the bytes the caller had at entry, matching Linux's `load_msg()`-before-wait ordering. |
| `semget()` / `semop()` / `semctl()` / `semtimedop()` | Full | Host-side SysV semaphore sets. Atomic multi-semaphore operations, SEM_UNDO support, IPC_STAT/SETVAL/GETVAL/SETALL/GETALL. The kernel reads and writes the `union semun` in the caller's memory directly. GETALL/SETALL are sized from the set's own `nsems`, under the requested command's permission check — never through a read-only `IPC_STAT`, which would impose a read permission POSIX does not require for SETALL. Musl's wasm32 time64 structure is 72 bytes and its wasm64 LP64 structure is 88 bytes; the process pointer width, not the kernel Wasm width, selects the layout. |
| `shmget()` / `shmat()` / `shmdt()` / `shmctl()` | Partial | Host-side SysV shared-memory segments support IPC_STAT/IPC_SET/IPC_RMID, fork inheritance, and exact attach/detach accounting. IPC_STAT/IPC_SET read and write the caller-width `shmid_ds` directly in the caller's memory: 88 bytes for wasm32 time64 and 112 bytes for wasm64 LP64. Separate process memories merge changed attachment bytes and import peer changes at syscall boundaries. Direct stores are not immediately visible and cross-process futex synchronization over an attachment is unsupported. |
...
| `mq_timedsend()` / `mq_timedreceive()` | Full | Priority-ordered message delivery. Blocking with timeout support. O_NONBLOCK returns EAGAIN. The queue's `mq_msgsize` is resolved before the caller's buffer is read, so a message larger than it — or a receive capacity smaller than it — reports EMSGSIZE rather than a transport-sized ENOMEM. |
...
| `nanosleep()` | A host timer holds the caller's channel; no thread blocks | Requires the worker/channel runtime, like every other parked syscall |
