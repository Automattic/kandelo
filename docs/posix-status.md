...
| `/dev/input/event0`, `/dev/input/event1` | Partial | Linux `evdev` character devices: `event0` is the keyboard, `event1` the pointer. `read()` drains whole 24-byte `struct input_event` records and returns `EAGAIN` when the ring is empty; `poll()` reports `POLLIN` only while records are queued. Each OFD owns its own 1024-record ring, so several readers see the stream independently. On overflow the ring latches a drop and the next `read()` returns a `SYN_DROPPED` record so the client can resynchronise. The host pushes records with `kernel_input_event(device, type, code, value)` and publishes the canvas size with `kernel_set_input_canvas_dims(width, height)`, which is what `EVIOCGABS` reports as the `ABS_X`/`ABS_Y` maxima. ioctls: `EVIOCGVERSION` (reports 1.0.1), `EVIOCGID` (`BUS_VIRTUAL`, vendor `0x1209`), `EVIOCGNAME` (`wpk virtual keyboard` / `wpk virtual pointer`, truncated to the caller's length), `EVIOCGBIT` for `EV_*` capability bitmaps, `EVIOCGABS` on `ABS_X`/`ABS_Y` (pointer only; other axes and the keyboard return `ENOTTY`). `EVIOCGRAB` (exclusive grab) is deliberately **not** supported and returns `ENOTTY`: v1 fans every record out to all readers, so honoring the grab would report success while ignoring the exclusivity a caller asked for — returning `ENOTTY` lets a program that needs an exclusive grab fail loudly instead. Every other `E`-magic request returns `ENOTTY`. Ring state is per OFD: released on last close, carried across `fork()` and `exec()` with the descriptor. The userspace `<linux/input.h>` and `<linux/input-event-codes.h>` are vendored as the **complete** Linux UAPI (verbatim from kernel v6.6), so evdev consumers such as SDL's Linux input backend compile against the full code vocabulary; the kernel implements only this keyboard+pointer subset at runtime — a program can reference joystick/touch/tablet codes, compile, and simply find no device that emits them, exactly like a Linux box with only a keyboard and a mouse. No exclusive grab, no force feedback, no runtime `EVIOCSABS`/`EVIOCGKEY`/`EVIOCGLED` (the macros exist for compilation but return `ENOTTY`), no hotplug, and no input devices beyond these two — joysticks, touch, tablets, grab-exclusivity, and hotplug are deferred, added per-need. |
...
| `getrlimit()` | Full | Returns (soft, hard) resource limits. Defaults: NOFILE=(1024,4096), STACK=(8MB,infinity), others infinity. `RLIMIT_AS` is the exception and is never infinity: a process address space is a bounded Wasm linear memory, so the hard limit is the real ceiling mmap and brk already enforce (the host sets it per process from the memory layout it built — 1 GiB under the desktop memory profile, 256 MiB under the constrained one), and a soft limit set above that ceiling is reported at it. A guest cannot discover the bound any other way, and one told the space is unlimited will size an allocation it cannot have. |
