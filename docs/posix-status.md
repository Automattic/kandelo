...
| `waitpid()` / `wait4()` | Full | Kernel-internal: blocks parent until child exits or, with WUNTRACED, stops. WNOHANG supported. Reaps zombie processes; stopped children remain live and are reported once per stop with WIFSTOPPED/WSTOPSIG. Supports pid>0 (specific child), pid=-1 (any child), pid=0 (same pgid), pid<-1 (specific pgid). wait4 writes a zero-filled rusage struct when requested. |
...
| `getrusage()` / `wait4()` rusage | Partial | Returns zeroed rusage struct (144 bytes). RUSAGE_SELF, RUSAGE_CHILDREN, and wait4 child-rusage output are supported. No actual resource tracking in Wasm. |
