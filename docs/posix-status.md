...
| `/dev/tty0` / `/dev/tty1` | Partial | Linux VT aliases for the controlling terminal. Present for Xfbdev/SDL-style console probes. Minimal no-op VT/KD ioctls are supported: `KDGETMODE`, `KDSETMODE`, `VT_OPENQRY`, `VT_GETMODE`, `VT_SETMODE`, `VT_GETSTATE`, `VT_RELDISP`, `VT_ACTIVATE`, `VT_WAITACTIVE`. |
...
| `/dev/fb0` | Full | Linux fbdev framebuffer. Single-open (`EBUSY` for second opener). Default 640×400 BGRA32 packed-pixel; `FBIOPUT_VSCREENINFO` may select 640×480, 800×600, or 1024×768 before `mmap`. ioctls: `FBIOGET_VSCREENINFO`, `FBIOGET_FSCREENINFO`, `FBIOPAN_DISPLAY` (no-op success), `FBIOPUT_VSCREENINFO`, `FBIOBLANK` (no-op success), `FBIO_WAITFORVSYNC` (no-op success). `mmap` returns a region in process memory and notifies the host (`bind_framebuffer` callback) so the browser canvas can mirror pixels. `munmap`/`close`/`exit`/`exec` clean up. Linux-VT keyboard ioctls (`KDGKBTYPE`/`KDGKBMODE`/`KDSKBMODE`) accepted with sensible defaults so framebuffer software works unmodified. |
| `/dev/input/mice` | Full | Linux `mousedev` PS/2 mouse stream. Single-open (`EBUSY` for second pid). 3-byte packets: byte0 button bits + sign/overflow flags, bytes 1..2 signed dx/dy with positive-up dy. Host pushes events via `kernel_inject_mouse_event(dx, dy, buttons)`; the kernel chunks large motion into multiple signed-8-bit PS/2 packets and buffers up to 4096 packets (whole-packet drop on overflow). `read()` drains queued bytes; returns `EAGAIN` when empty. `poll()` reports `POLLIN` only when bytes are queued. Ownership released on `close`/`exec`/`exit`. No IMPS/2 wheel protocol yet. |
| `/dev/input/event0` | Partial | Linux evdev relative pointer. Single-open (`EBUSY` for second pid). Emits 16-byte `struct input_event` records for `EV_REL` (`REL_X`, `REL_Y`, `REL_WHEEL`), pointer buttons (`BTN_LEFT`, `BTN_RIGHT`, `BTN_MIDDLE`), and `SYN_REPORT`. Supports discovery ioctls `EVIOCGVERSION`, `EVIOCGID`, `EVIOCGNAME`, and `EVIOCGBIT`. |
| `/dev/input/event1` | Partial | Linux evdev keyboard. Single-open (`EBUSY` for second pid). Emits 16-byte `struct input_event` key press/release records plus `SYN_REPORT`. Supports discovery ioctls `EVIOCGVERSION`, `EVIOCGID`, `EVIOCGNAME`, and `EVIOCGBIT` for the currently injected browser key set. Text input remains on terminal/PTY/stdin; this device reports physical key edges. |
