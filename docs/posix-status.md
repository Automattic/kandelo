...
| `tcgetattr()` / `tcsetattr()` | Full | Full termios support on CharDevice and PTY fds. c_iflag, c_oflag, c_cflag, c_lflag, c_cc. On PTYs, TCSANOW and TCSADRAIN preserve unread input across ICANON transitions: completed lines and the current edited partial line become raw-readable in byte order, while unread raw bytes become immediately readable if the mode changes back, matching Linux EOF-push behavior. TCSAFLUSH discards unread input before applying the change. PTY writes synchronously enter the output queue, so there is no deferred device transmission to await. ICANON mode: line buffering with VERASE (backspace), VKILL (^U), VEOF (^D) editing. ICRNL/INLCR/IGNCR input processing. ECHO/ECHOE/ECHOK/ECHONL output. VMIN/VTIME values accessible for raw mode. Uses musl's 60-byte termios layout. |
