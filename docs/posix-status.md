...
| `/dev/dri/card0` | Partial | DRM KMS legacy ioctl surface — enough for upstream libdrm + SDL2's KMSDRM backend. ioctls: VERSION, MODE_GETRESOURCES, MODE_GETCONNECTOR, MODE_GETENCODER, MODE_GETCRTC, MODE_SETCRTC, MODE_GETPLANE, MODE_GETPLANE_RESOURCES, MODE_CURSOR, MODE_ADDFB / RMFB / DIRTYFB, MODE_PAGE_FLIP_FLAGS, MODE_CREATE / DESTROY / MAP_DUMB. One CRTC + one connector + 1024×768 mode. Atomic + properties surface deferred. |
| `/dev/dri/renderD128` | Partial | GL command-buffer transport for the libEGL / libGLESv2 stubs. ioctls: GLIO_INIT (op-version handshake), GLIO_CREATE_CONTEXT, GLIO_CREATE_SURFACE, GLIO_MAKE_CURRENT, GLIO_PRESENT, GLIO_TERMINATE. `mmap` exposes the per-context cmdbuf to userspace. |
