...
| `inotify_add_watch()` / `inotify_rm_watch()` | Stub | Returns ENOSYS. The whole family fails so watchers (Qt's QFileSystemWatcher, glib's GFileMonitor, tail -f) take their polling fallbacks instead of trusting an fd that never delivers events. |
