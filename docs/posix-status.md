...
| `epoll_create1()` | Partial | Creates an epoll instance with a **per-process** interest list. EPOLL_CLOEXEC supported. The instance is owned by the `Process` rather than by the open file description, which is what makes the two gaps below possible. |
| `epoll_ctl()` | Partial | EPOLL_CTL_ADD, EPOLL_CTL_MOD, EPOLL_CTL_DEL; stores the interest set with events and data. Registrations track **numeric fds rather than OFD identity**, so close/dup and same-number replacement are incomplete (the same gap the `exec()` row records). Because the instance is process-owned, a `fork()` child cannot reach it — see `epoll_pwait()`. |
| `epoll_pwait()` | Partial | Builds a pollfd set from the interest list, delegates to poll, maps results back to `epoll_event`, and supports the optional signal-mask swap. **A `fork()` child cannot use an inherited epoll fd: it returns `EBADF`.** On Linux an epoll fd refers to an open file description, so a child's duplicated descriptor names the *same* instance and either process's `epoll_ctl` is visible to both. Kandelo clears the child's instances at `crates/runtime-core/src/fork.rs` instead, and the lookup then fails at `syscalls.rs`. Closing this requires moving instance ownership out of `Process` into an OFD-keyed table — the same change the `epoll_ctl()` row needs, not a separate one. |
