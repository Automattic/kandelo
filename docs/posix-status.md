...
| `/dev/dri/renderD128` | Partial | GL command-buffer transport for the libEGL / libGLESv2 stubs. ioctls: GLIO_INIT (op-version handshake), GLIO_CREATE_CONTEXT, GLIO_CREATE_SURFACE, GLIO_MAKE_CURRENT, GLIO_SUBMIT, GLIO_QUERY, GLIO_PRESENT, GLIO_TERMINATE, plus the WPK extension `DRM_IOCTL_WPK_BIND_FOREIGN_TEXTURE` ((re)upload a CPU-tier bo's pixels as a `WebGLTexture` in the caller's context — the wlcompositor GPU-compositing path; `WPK_CREATE_GPU_BO` still ENOSYS). `mmap` exposes the per-context cmdbuf to userspace. |
