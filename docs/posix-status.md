...
| `fork()` called from a C++/Ruby exception catch handler | Known-broken | B1 stages 1+2 implemented the structural machinery (per-arm scratch space in the save buffer, multi-arm rewind dispatch, capture-block emission for the handler body), but the **synthetic end-to-end fixture** `programs/cpp_eh_fork_from_catch_test.cpp` (landed 2026-05-13 as a `it.fails` regression gate) shows the feature does not actually work — the parent reaches the fork site, never returns from `fork()`, and exits silently. Same failure mode as post-catch fork (`cpp_post_catch_fork_test`). Commits 2.5/2.6/3/4 (2026-05-14) eliminated guard-dispatch entirely (the architectural pivot named in the original plan), but the C1 end-to-end hang remained — fork-from-catch is structurally independent of the carryover / multi-value-params absorbtion that those sub-commits delivered. Carve-outs still apply for additional shapes (ref-typed catch operands, multi-target `*_ref` try_tables — a fork inside them traps with `kernel_fork` returning ENOSYS); A2/A3/A4 follow-ups will close those after the C1 root cause is identified. The SpiderMonkey-port "Gate 1.B" is **not** actually cleared by B1 stages 1+2 alone, and the 2.5/2.6/3/4 pivot did not close it either — the C1 hang investigation is the remaining critical path. See `docs/plans/2026-05-13-fork-instrument-megaPR-eliminate-guard-dispatch-and-modern-EH-plan.md` and `memory/fork-instrument-fierce-wire-paused-pending-423.md` (external memory). |
