...
| `epoll_pwait()` | Partial | Builds a pollfd set from the shared interest list, delegates to poll, maps results back to `epoll_event`, and supports the optional signal-mask swap. Each interest is resolved to a descriptor the **calling** process currently holds — preferring the number used at registration, otherwise any descriptor naming the same open file description — so an inherited epoll fd works in a `fork()` child, a registration survives `dup` plus close of the original number, and a number a later `open` reused is never polled on the old registration's behalf. Remaining gaps: an interest whose description the caller cannot reach contributes nothing (see `epoll_ctl()`); the edge-triggered and one-shot modes are not modelled; and an empty interest list with a signal mask and a positive timeout sleeps through `host_nanosleep` rather than waiting for a signal. |
