...
| `fork()` called from a C++/Ruby exception catch handler | Partial | B1 stages 1+2 (this PR) implement plain-catch fork-from-catch: per-arm scratch space in the save buffer, multi-arm rewind dispatch, capture-block emission for the handler body. Carve-outs apply: functions whose catch arms carry ref-typed operands, and multi-target `*_ref` try_tables, are excluded from fork-path instrumentation (a fork inside them traps with `kernel_fork` returning ENOSYS). The first end-to-end real-world validation will land with the SpiderMonkey port (B1 Stage 3). See [fork-instrumentation.md](fork-instrumentation.md) §Catch-handler resume and §Not guaranteed. |
