...
| **Set-ID exec credential commit** | process | Mount admission defaults to `nosuid`, and the kernel proposal helper ignores set-ID bits there. The one internal trusted mount can preserve those bits as a proposal, but target-aware exec validation and atomic credential commit remain future work. |
