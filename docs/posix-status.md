...
| `ppoll()` | Partial | Wraps poll() with atomic signal mask swap: save → set → poll → restore. If the replacement mask makes a caught signal deliverable, its catcher observes that replacement mask together with sa_mask and the delivered signal. Per-TID LIFO contexts keep nested waits distinct; terminal completion, exact final-EINTR cancellation, or `longjmp`/`siglongjmp` abandonment retires each context once. **Remaining gap: a caught `SA_RESTART` handler resubmits a zero-progress outer ppoll instead of returning `EINTR`.** Linux restarts `ppoll` with `ERESTARTNOHAND`, which restarts only when no handler ran, so a caught signal always yields `EINTR`; resubmitting instead means a `ppoll` whose only possible wake is the signal blocks forever after its handler returns. `tests/sortix/os-test/signal/ppoll-block-raise` and `ppoll-block-sleep-raise` demonstrate it: both print `SIGUSR1` and then hang until the harness kills them, while the six sibling cases that also have a pipe write or close to wake them pass. Note that `pselect6()` below already chooses `EINTR` here, so the two calls currently disagree. The glue layer converts the timespec to milliseconds. A finite call keeps its `CLOCK_MONOTONIC` deadline for that libc call frame across host retries and catcher syscalls, so nested or later identical-argument calls cannot inherit it; expiry still reaches the kernel so `revents` is copied back and the temporary mask is restored. |
