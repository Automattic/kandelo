...
| `/dev/input/event0`, `/dev/input/event1` | Partial | evdev keyboard + mouse devices. Reads return packed `struct input_event` records (4 bytes each on wasm32 time_bits=64 layout). One push-style host import per event (`kernel_input_event`) feeds the kernel's bounded ring; consumers `poll()` / `read()` it. KEY/EV/SYN codes match Linux UAPI byte-for-byte. |
