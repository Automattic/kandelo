...
| `munmap()` | Full | Removes tracked region. Page-aligned address required. Length rounds up to the 64 KB wasm page (Linux semantics: all pages containing a part of the range are unmapped) — matching `mmap`'s rounding so map/unmap cycles with unaligned lengths don't strand tail remnants. Partial munmap supported: front trim, back trim, and middle split. |
...
| `setsockopt()` / `getsockopt()` | Partial | SOL_SOCKET: SO_TYPE, SO_DOMAIN, SO_ERROR, SO_ACCEPTCONN, SO_RCVBUF, SO_SNDBUF readable; SO_PEERCRED returns `struct ucred {pid,uid,gid}` for a connected AF_UNIX socket (single-user model: reports the querying process's own credentials — exact for socketpair/same-process accept); SO_REUSEADDR affects UDP bind conflicts; SO_KEEPALIVE, SO_LINGER, SO_RCVTIMEO, SO_SNDTIMEO, SO_BROADCAST accepted/stored. IPPROTO_TCP: TCP_NODELAY stored. |
...
| `epoll_ctl()` | Full | EPOLL_CTL_ADD, EPOLL_CTL_MOD, EPOLL_CTL_DEL. Stores interest set with events + data. `struct epoll_event` is marshalled with musl's wasm32 layout — 16 bytes, unpacked: `events`@0, pad@4, `data`@8 (musl only applies `__packed__` on x86_64). |
| `epoll_pwait()` | Full | Builds pollfd from interest set, delegates to poll, maps results back to epoll_event structs (16-byte stride, `data`@8 — matching `epoll_ctl`). Nested epoll (epoll-on-epoll) is supported: an epoll fd registered inside another epoll reports readable when any of its own interests is ready (depth-capped at 4 to guard against cycles), which is how compositors register libinput's epoll fd inside `wl_event_loop`. Optional signal mask swap. Host converts epoll_pwait to a non-blocking poll and manages blocking/timeout itself: a finite positive timeout returns 0 after the deadline (per-channel deadline persisted across wakeup-driven retries so a drained fd set parks the full timeout instead of retrying forever); a readable fd wakes it promptly. |
...
| `/dev/ptmx` | Full | PTY master multiplexer. `open()` allocates a new PTY pair, returns master fd. Master `read()` drains buffered slave output *before* reporting EOF: when the slave side has closed (`slave_refs == 0`) but the output buffer is still non-empty — the case where a shell writes its last bytes and exits in one step — the buffered bytes are returned first, and `read()` only returns `0` (EOF) once the buffer is empty. This prevents silently dropping the tail of a program's output (e.g. a terminal emulator losing the final line of a command result). `poll()` reports `POLLIN | POLLHUP` while data remains after slave close. |
...
| `/dev/snd/pcmC0D0p` | Full (write-only) | ALSA PCM playback device. Talks the upstream UAPI struct surface (`<sound/asound.h>`) so alsa-lib and SDL2 link unmodified. PCM ioctls: PVERSION, INFO, HW_REFINE, HW_PARAMS, SW_PARAMS, PREPARE, START, DROP, DRAIN, WRITEI_FRAMES, STATUS, DELAY, SYNC_PTR. mmap-status / mmap-control pages for the lock-free pointer path. S16_LE only, mono/stereo, rate 8000–192000 Hz. See `crates/kernel/src/audio/pcm_ioctl.rs`. |
| `/dev/snd/controlC0` | Partial | ALSA control device gate. Implements PVERSION + PCM_PREFER_SUBDEVICE — the two ioctls alsa-lib fans before opening the PCM fd. Other CTL ioctls (ELEM_LIST, ELEM_INFO, …) return ENOSYS. See `crates/kernel/src/audio/ctl_ioctl.rs`. |
| `/dev/dri/card0` | Partial | DRM KMS legacy ioctl surface — enough for upstream libdrm + SDL2's KMSDRM backend. ioctls: VERSION, MODE_GETRESOURCES, MODE_GETCONNECTOR, MODE_GETENCODER, MODE_GETCRTC, MODE_SETCRTC, MODE_GETPLANE, MODE_GETPLANE_RESOURCES, MODE_CURSOR, MODE_ADDFB / RMFB / DIRTYFB, MODE_PAGE_FLIP_FLAGS, MODE_CREATE / DESTROY / MAP_DUMB. One CRTC + one connector; the preferred mode defaults to 1920×1080@60 and follows the embedder-reported display aspect (width `round(1080 × aspect)` clamped [1440, 3840] at 1080 tall) when the host has one. PAGE_FLIP latches the host scanout at ioctl time; flip-complete events retire at the host's 60 Hz vblank tick. Atomic + properties surface deferred. |
| `/dev/dri/renderD128` | Partial | GL command-buffer transport for the libEGL / libGLESv2 stubs. ioctls: GLIO_INIT (op-version handshake), GLIO_CREATE_CONTEXT, GLIO_CREATE_SURFACE, GLIO_MAKE_CURRENT, GLIO_SUBMIT, GLIO_QUERY, GLIO_PRESENT, GLIO_TERMINATE, plus the WPK extensions `DRM_IOCTL_WPK_BIND_FOREIGN_TEXTURE` ((re)upload a CPU-tier bo's pixels as a `WebGLTexture` in the caller's context — the wlcompositor GPU-compositing path) and `DRM_IOCTL_WPK_CREATE_GPU_BO` (a GPU-tier bo backed by a host `WebGLTexture`+FBO on the compositor's shared multiplexer context — unmappable, rendered into by a routed client and sampled zero-copy; `BIND_FOREIGN_TEXTURE` on a GPU-tier bo returns the texture id with no upload). GPU-tier allocation degrades to a CPU-tier dumb bo when no shared GL context exists (headless Node, or before the compositor's context is up). `GLIO_CREATE_SURFACE`'s `reserved[0]` optionally names a target bo whose FBO the surface renders into (the client's default framebuffer is redirected there). `mmap` exposes the per-context cmdbuf to userspace. |
| `/dev/input/event0`, `/dev/input/event1` | Partial | evdev keyboard + mouse devices. Reads return packed `struct input_event` records (4 bytes each on wasm32 time_bits=64 layout). One push-style host import per event (`kernel_input_event`) feeds the kernel's bounded ring; consumers `poll()` / `read()` it. KEY/EV/SYN codes match Linux UAPI byte-for-byte. `EVIOCG*` introspection: `EVIOCGVERSION`/`GID`/`GNAME`/`GBIT`/`GABS`/`GRAB` supported; `EVIOCGPROP`/`GKEY`/`GLED`/`GSW` return zero-filled current state and `EVIOCGPHYS`/`GUNIQ` return `ENOENT` (unset) so `libevdev_new_from_fd` constructs cleanly; other requests → `ENOTTY`. `stat()` reports a Linux-encoded `st_rdev` (char major 13, minor 64+N — `event0` = 13:64, `event1` = 13:65) so a consumer that identifies a node purely by devnum (libinput's `udev_device_new_from_devnum`) can resolve it (ABI v17; `st_rdev` is otherwise 0 for virtual nodes). |
