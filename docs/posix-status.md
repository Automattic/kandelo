...
| `fork()` called from a C++/Ruby exception catch handler | Known-broken | B1 stages 1+2 (this PR) implement the structural machinery (per-arm scratch space in the save buffer, multi-arm rewind dispatch, capture-block emission for the handler body), but the **synthetic end-to-end fixture** `programs/cpp_eh_fork_from_catch_test.cpp` (landed 2026-05-13 as a `it.fails` regression gate) shows the feature does not actually work — the parent reaches the fork site, never returns from `fork()`, and exits silently. Same failure mode as post-catch fork (`cpp_post_catch_fork_test`). Carve-outs apply for additional shapes (ref-typed catch operands, multi-target `*_ref` try_tables — a fork inside them traps with `kernel_fork` returning ENOSYS). The planned fix is the architectural pivot to eliminate guard-dispatch entirely; see `docs/plans/2026-05-13-fork-instrument-megaPR-eliminate-guard-dispatch-and-modern-EH-plan.md`. The SpiderMonkey-port "Gate 1.B" is **not** actually cleared by B1 stages 1+2 alone — the mega-PR is the gate. |
