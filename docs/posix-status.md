...
| `exit()` / `_exit()` | Partial | Closes all fds and dir streams, releases all fcntl locks, sets ProcessState::Exited. SIGCHLD delivered to a guest parent. Guest-child zombie state is maintained until reaped by waitpid; the host separately reaps only exited direct children of `ppid=0` after their workers can issue no more syscalls. Orphan adoption is not yet implemented when a guest parent exits. |
