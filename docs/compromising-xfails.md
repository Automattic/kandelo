...
- `libc/musl-overlay/src/thread/wasm32posix/pthread_cancel.c` provides `pthread_cancel`, `__cancel`, `__testcancel`, `__syscall_cp_cancel_preflight`, and `__syscall_cp_check` (together the moral equivalent of `__syscall_cp_asm` + `__syscall_cp_c`). The overlay reuses musl's existing per-thread `pthread_t->cancel` atomic field as the pending-cancel flag — no new channel slot or TLS global needed.
...
- `libc/glue/channel_syscall.c::__syscall_cp` calls `__syscall_cp_cancel_preflight()` before each cancellation-point syscall and `__syscall_cp_check(r)` after, handling the three cancel states (ENABLE → pthread_exit, MASKED → synthesize -ECANCELED, DISABLE → pass-through) the way stock musl's asm does. The overlay also exports `__syscall_cp_cancel_wake_allowed()`, which the glue uses to freeze per-request wake authority into the channel's `REQUEST_FLAG_CANCELLATION_WAKE_ALLOWED` bit so a `PTHREAD_CANCEL_DISABLE` target keeps its in-flight operation and its deadline.
...
**Closed:**
- `pthread_create-oom`: **passes** (2026-09-25) — `pthread_create` returns `EAGAIN` once memory is exhausted. It had been XFAIL because the libc-test harness compiled `t_memfill()` to return -1 unconditionally, so the test failed in setup with "memfill failed" before reaching `pthread_create`; see `-fno-builtin-malloc` in `scripts/run-libc-tests.sh`.
...
- **Target 1** (pthread_create): Basic path and deferred cancellation landed, and per-thread signal routing landed — see the status block in section 1. Remaining sub-gaps (asynchronous cancellation, AIO) are separate follow-ups; asynchronous cancellation is a Wasm boundary, not a kernel gap. `pthread_create-oom` passes.
