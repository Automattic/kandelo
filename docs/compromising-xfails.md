...
- **Target 1** (pthread_create): Basic path and deferred cancellation landed, and per-thread signal routing landed — see the status block in section 1. Remaining sub-gaps (asynchronous cancellation, AIO, OOM) are separate follow-ups; asynchronous cancellation and `pthread_create-oom` are not kernel gaps.
