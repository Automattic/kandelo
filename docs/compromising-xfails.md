...
**Closed tests:** libc-test `regression/raise-race` (previously flakey XFAIL; now non-gating flaky because it can pass, fail, or appear as `TIME` depending on runner load/resources), sortix `signal/pthread_kill`, sortix `basic/aio/aio_fsync`, sortix `basic/aio/aio_read`, sortix `basic/aio/aio_error` (after `sys_pread` / `sys_pwrite` reject negative offsets with `EINVAL`, 2026-04-22 — the test's `aio_write(offset=-9000)` used to surface the host's seek error as `EIO`).
