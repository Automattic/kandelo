...
### ABI 40 fork-continuation reserve

ABI 40 increases each fork-continuation save buffer from 16 KiB to 48 KiB.
The buffer's absolute address is passed to every instrumented main module,
pthread worker, and fork-capable side module, so changing its position in the
host-owned control layout requires rebuilding ABI-bound programs and images.
The larger reserve fits within the existing 64 KiB scratch page and covers the
measured 21.5 KiB GTK/GLib continuation while retaining overrun detection for
continuations that exceed the new bound.
