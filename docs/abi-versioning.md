...
Process exit uses the required returning
`kernel_commit_process_exit(status)` export in ABI 43. The host binds the exact
task, calls that transaction directly, compares its returned low-eight-bit
status, and separately requires authoritative `Exited` state before publishing
the lifecycle result. A trap is not a compatibility success path in ABI 43:
the ABI 42 host/kernel rolling compatibility described above remains historical
to that released epoch, while stale ABI 42 artifacts fail the ABI 43
capability/version checks. The disposable guest worker still traps after the
completed channel handshake to preserve `_exit`'s non-returning program
contract without skipping the reusable kernel's shadow-stack epilogue.
