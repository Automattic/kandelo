...
| `networking/virtual-network.ts` | 527 | **SPLIT (K11, partly done)** | **Bind-conflict rule + `EADDRINUSE` removed 2026-09-09** — it was a live second authority over `socket.rs` `udp_can_bind`/`tcp_can_bind` and lost `SO_REUSEADDR` and fork-inherited owners. Still duplicated: per-machine endpoint *selection* (`socket::udp_lookup`'s rule), blocked on a machine-level `NetworkIO` ingress in `kernel-worker.ts`. `VirtualTcpPeer` and address ownership are the wire → **KEEP**. Value plan §2q |
| `networking/` TLS / fetch / tcp / cors backends | 2,241 | **SPLIT** | **Correction (2026-09-09):** `tls-network-backend.ts` contains *no* TLS state machine — the handshake is an injected WebCrypto `TlsMitmConnection`. What it owns is an **HTTP/1.1 parser + MITM proxy**: ≈220 lines of message framing → **MIGRATE**; handshake, WebCrypto, `fetch` → **KEEP**. Value plan §2q |
| `framebuffer/browser-controls.ts` — VT input encoding | ~300 of 732 | **MIGRATE (blocked)** | Linux VT/PS-2 wire encoding is pure computation, but the module runs on the browser **main thread**, which has no kernel instance by contract. Needs the input path to move, not a port. Value plan §2q |
| `webgl/bridge.ts` — command-buffer decode | ~180 of 700 | **MIGRATE (blocked)** | Framing + payload validation is computation; `dispatch`'s `gl.*` calls are the device. Destination is `syscalls.rs:1441-1450`, beside the existing cmdbuf bounds check — not a new export. Value plan §2q |
