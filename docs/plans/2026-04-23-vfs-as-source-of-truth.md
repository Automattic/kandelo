...
>
> **Phase 3.2–3.9 SUPERSEDED (2026-04-24)** — The `OverlayPlatformIO` design below let host filesystem paths fall through implicitly ("the kernel's `/` is also macOS's `/`"), which broke the "VFS is the only lens" principle and created subtle bugs (content-based ownership heuristics, `DEFAULT_HOST_UID` hacks, tests that bridge host↔kernel through shared paths). The corrected design uses an explicit mount table where every VFS path prefix maps to a typed backend (memfs, host-dir, kernel-synthetic) and unmounted paths return `ENOENT`. See [`2026-04-25-vfs-mount-table.md`](2026-04-25-vfs-mount-table.md) for the replacement design.
>
> Phases 0–2 and 3.1 as written are still the correct foundation and landed on `prairie-headlight`. Read those; skip the Phase 3.2–3.9 task descriptions.
