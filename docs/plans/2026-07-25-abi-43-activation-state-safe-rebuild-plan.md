...
Status: development plan only. No canonical package, index, shell, or
...
currently contains 76 ABI-bound packages:
...
- 66 programs, producing 68 architecture generations. The committed program
  projection covers 64 of those packages/66 generations; the special `kernel`
  and `userspace` packages add one wasm32 generation each.

That is 82 ABI-bound `(package, architecture)` archive generations. The
...
Of the 64 projected program packages, 57 packages/59 generations contain an
output whose fork-instrumentation policy is `auto`. These outputs must be
rebuilt from raw linker output with the ABI 43 instrumenter. The seven
all-disabled packages (`nginx-php-vfs`, `nginx-vfs`,
...
candidate. All 14 library generations and 55 of 61 attempted program
...
and `wordpress`); every one failed closed because the available shell VFS
...
therefore contains 69 successful entries and six failed entries, not all 82
...
4. Build all 82 ABI-bound registry generations into isolated staging,
...
7. Report exact successful, failed, and unrun generations/tests. Canonical
   index activation, VFS publication, and merge require
