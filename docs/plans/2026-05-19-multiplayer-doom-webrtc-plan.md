...
> **Goal:** Two browsers on different machines play a deathmatch of fbDOOM. The byte path is an unreliable+unordered `RTCDataChannel`; fbDOOM's chocolate-doom-shaped `d_net.c` is driven by a new POSIX BSD-sockets `i_net_posix.c`; the cross-thread glue is a two-half relay (`RelayChannel` on the page main thread, `RelayHostShim` on the kernel-worker) bridged via a new `kernel_inject_datagram` export and a new `HostIO::send_dgram` host call. This **does** bump `ABI_VERSION`.

## Tech stack

- Rust (kernel) — new `HostIO::send_dgram` trait method in `crates/shared/`, new `kernel_inject_datagram` export, `sys_sendto` dispatches to the new host hook for non-loopback destinations. `sys_recvfrom` unchanged (its existing `dgram_queue` is the inbound landing pad).
- TypeScript (host + browser demo page) — no new npm dependencies. New main-thread `RelayChannel` module in `examples/browser/lib/`; new kernel-worker-side `RelayHostShim` wired alongside `TlsNetworkBackend` in `kernel-worker-entry.ts`.
...
- Reuses the freedoom1.wad fetch already wired into `examples/libs/fbdoom/build-fbdoom.sh`. **No new IWAD distribution.**

## Verification gauntlet

This change touches four subsystems:

1. **Kernel (Rust)** — new HostIO method, new kernel-wasm export, `sys_sendto` dispatch. ABI-affecting.
2. **Host runtime** (kernel-worker-side `RelayHostShim`, host-call dispatch in `kernel-worker.ts`).
3. **fbDOOM port** (new patch under `examples/libs/fbdoom/patches/`).
4. **Browser demo page + main-thread relay** (new `pages/doom-mp/` + `examples/browser/lib/relay-network-backend.ts`).

ABI **does** change. The applicable subset of the CLAUDE.md gauntlet (run in order):
...
# 3. ABI snapshot — REGENERATE it, then verify the bump matches.
#    The new kernel_inject_datagram export + new HostIO::send_dgram trait
#    method WILL change the snapshot; ABI_VERSION must bump in the same
#    commit. See docs/abi-versioning.md.
bash scripts/check-abi-version.sh update   # writes new abi/snapshot.json
git diff abi/snapshot.json                 # inspect — confirm the expected drift
# bump ABI_VERSION in crates/shared/src/lib.rs in the same commit, then:
bash scripts/check-abi-version.sh          # must pass (drift matched by bump)

# 4. Browser bundle builds.
cd examples/browser && npx tsc --noEmit && npx vite build
...
## Task 1 — Kernel: `kernel_inject_datagram` export + `HostIO::send_dgram` + ABI bump

**Goal.** Add the kernel-side surface needed to route UDP datagrams between fbDOOM (running in a process-worker) and the page main thread (where `RTCDataChannel` lives), in both directions. This is the **ABI-affecting** task; it stands alone so the ABI bump is one focused commit.

**Files**

- Modify: `crates/kernel/src/process.rs` — at the `HostIO` trait body (L25-117), add:
  ```rust
  fn send_dgram(
      &mut self,
      src_port: u16,
  ...
  ```
  The default impl returning `ENETUNREACH` means the seven existing concrete impls (`WasmHostIO` in `wasm_api.rs:121` + six test mocks in `syscalls.rs`: `MockHostIO`, `TrackingHostIO`, `NetMock`, `SymlinkMock`, `LoopMock`, `RelSymlinkMock`) compile unchanged; only `WasmHostIO` overrides it. Signature has no per-socket "handle" param — the relay is per-channel, not per-socket; the kernel passes the bound `src_port` so the host can populate the wire envelope.
- Modify: `crates/kernel/src/syscalls.rs` — at L5212 (`sys_sendto`'s `ENETUNREACH` branch for non-loopback), replace with a call into `host.send_dgram(src_port, dst_ip, dst_port, buf)`. The existing loopback branch is unchanged, so `test_udp_loopback` continues to pass. Add a unit test that exercises the non-loopback branch with a mock `HostIO` (override `send_dgram` on `NetMock` or a new mock).
- Modify: `crates/kernel/src/wasm_api.rs`:
  - In the `extern "C"` host-imports block at L38, add `fn host_send_dgram(src_port: u32, dst_a: u32, dst_b: u32, dst_c: u32, dst_d: u32, dst_port: u32, data_ptr: *const u8, data_len: u32) -> i32;` (mirror the parameter-shape of `host_net_listen` at L80).
  - On `impl HostIO for WasmHostIO` (L121), override `send_dgram` to call the new host import (mirror `host_net_listen`'s wrapper at L491).
  - Add the new `kernel_inject_datagram` export next to `kernel_inject_connection` at L8092, with signature:
    ```rust
    #[unsafe(no_mangle)]
    ...
    ```
    Body: look up `proc` via `pid`; linearly scan `proc.sockets` for a `SocketType::Dgram` in `Bound`/`Connected` state with `bind_port == dst_port` (mirroring `sys_sendto`'s loopback scan at L5221-5233); push `Datagram { data: data.to_vec(), src_addr: [src_a, src_b, src_c, src_d], src_port: src_port as u16 }` onto its `dgram_queue`; wake any `sys_recvfrom` blocked on that socket via the kernel's existing wakeup machinery. Returns `0` on success, negative errno on failure (`-ESRCH` no such pid, `-ECONNREFUSED` no bound socket on that port).
- Modify: `crates/shared/src/lib.rs` — bump `ABI_VERSION` (currently 7). **No other edits expected**: there are no new wire-format constants or struct layouts; the structural change is the new wasm import + new wasm export, which the snapshot captures by name.
- Regenerate: `abi/snapshot.json` via `scripts/check-abi-version.sh update`. **Expected drift:** +1 import (`host_send_dgram`), +1 export (`kernel_inject_datagram`), `ABI_VERSION: 7 → 8`. Anything else in the diff is a surprise — investigate before committing.

**Why `(pid, dst_port)`, not `(pid, fd)`.** The TCP analogue `kernel_inject_connection` takes a `listener_fd`, which works because `sys_listen` notifies the host of the (pid, fd, port) tuple via `host_net_listen` (`syscalls.rs:4895`). `sys_bind` has no analogous DGRAM notification, so an fd-based inject would require a *third* new HostIO hook just to teach the host fbDOOM's UDP fd. Port-based addressing sidesteps this — the page already knows the port (5029, baked into the `-server`/`-connect` flag) and the pid (it spawned fbDOOM). The kernel's per-process linear scan is the same one `sys_sendto`'s loopback branch already uses.

**No `sys_recvfrom` change.** Once datagrams land in `dgram_queue` via the new injection path, the existing `sys_recvfrom` drain logic at L5278-5306 already does the right thing. (The unconditional-EAGAIN bug at L5279 is documented in the design's §9 and filed as a follow-up; it doesn't block DOOM because DOOM polls.)

**Verification**

```bash
cargo test -p wasm-posix-kernel --target aarch64-apple-darwin --lib
bash scripts/check-abi-version.sh   # passes after ABI_VERSION bump matches the
                                    # regenerated snapshot
```

This task lands as a single commit: `feat(kernel): UDP host-side relay hooks (ABI bump)`. The new export + trait method + snapshot regeneration + `ABI_VERSION` bump all go in one commit, per CLAUDE.md ABI policy.
...
- New: `examples/browser/lib/relay-network-backend.ts` — the main-thread `RelayChannel` class (~150 LOC). See design §5.1 + §5.3 for the wire envelope.
- Modify: `examples/browser/lib/browser-kernel.ts` (near the existing `injectConnection` at L407) — expose `injectDatagram(pid, dstPort, srcIp, srcPort, data): void`. **This is synchronous fire-and-forget — no `requestId`, no awaited Promise, returns `void`.** Unlike `injectConnection` which routes via `this.request(...)`, `injectDatagram` posts a one-way `{ type: "inject_datagram", pid, dstPort, srcIp, srcPort, data }` message to the kernel-worker via `sendToKernel(...)` (the existing fire-and-forget channel used by `pipeCloseWrite`, `wakeBlockedReaders`, etc., at L449-475). Rationale in design §5.1: at DOOM's ~35 Hz cadence we cannot afford a Promise + thread round-trip per packet, and there is no return value worth consuming (a drop is a drop). Also add a registration API (e.g. `kernel.onHostSendDgram(handler)`) that the `RelayChannel` uses to subscribe to outbound datagrams forwarded from the kernel-worker.
- Modify: `examples/browser/lib/kernel-worker-entry.ts` — near the existing `TlsNetworkBackend` instantiation at L193, add the `RelayHostShim` wiring. Receive the choice "this kernel needs UDP relay" via the init message; instantiate the shim; route its outbound messages to the main thread via `self.postMessage`.
- New: `examples/browser/test/relay-network-backend.test.ts` (or equivalent location matching existing browser-side test conventions) — vitest:
...
- Modify: `host/src/kernel-worker.ts` — add a dispatcher that routes the kernel's `host_send_dgram` host call into the kernel-worker-side `RelayHostShim` (if one is registered) or returns `-ENETUNREACH` (preserving today's behaviour when no relay is present). The exact line to edit is **to be located during implementation** — `kernel-worker.ts` is 6925 lines and the design's first-draft reference to L4531 was the `sendmsg` decompose path, which is unrelated. Search for the existing TCP host-call dispatch and mirror its structure.
- Modify: `examples/browser/lib/browser-kernel.ts` — proxy `injectDatagram(pid, dstPort, srcIp, srcPort, data)` from the page main thread through to the kernel-worker, which calls the kernel-wasm export. The wire-shape template is `sendToKernel(...)` at L449-475 (fire-and-forget), **not** `injectConnection` at L407 (request/response). Already specified in Task 2's `browser-kernel.ts` bullet; Task 3 wires the kernel-worker dispatch side.
...
bash scripts/check-abi-version.sh                                      # no drift since Task 1
```
...
- New: `examples/libs/fbdoom/patches/0002-i_net-posix.patch` — see design §6. Contains:
...
- Modify: `examples/libs/fbdoom/build-fbdoom.sh` — no source changes; the existing patch loop applies `0002-*.patch` automatically. The script does change one thing: after building, print the SHA256 of the resulting `fbdoom.wasm` so the binaries lockfile update is straightforward.
...
2. **Verify `getaddrinfo` round-trips a literal IPv4.** Chocolate-doom's net code typically calls `getaddrinfo()` to fill in a `sockaddr_in` even when given a dotted-quad like `10.99.0.1`. Our `sys_getaddrinfo` (`crates/kernel/src/syscalls.rs:5154`) delegates to the host. If the host doesn't shortcut literal IPs (the browser host's bridge is HTTPS-shaped and may try a real DNS lookup), the `-connect 10.99.0.1` flow will fail at startup. Two acceptable outcomes: (a) the host does shortcut literals — verify and proceed; (b) it doesn't — patch `i_net_posix.c` to call `inet_aton`/`inet_pton` first and only fall back to `getaddrinfo` for non-literal hostnames. Chocolate-doom upstream already takes path (b), so this is mostly a copy-and-confirm.
...
```bash
bash examples/libs/fbdoom/build-fbdoom.sh   # green, produces fbdoom.wasm
```
...
- New: `examples/browser/pages/doom-mp/index.html` — page shell. Layout:
...
- New: `examples/browser/pages/doom-mp/main.ts` — composition: imports `setupSignaling()` (factored shared module from Task 5b below, if needed), `RelayChannel` (from Task 2), `BrowserKernel`, `attachCanvas`, and the `SCANCODE` map from `pages/doom/main.ts`.
- New: `examples/browser/pages/doom-mp/doom-mp.css` — minor overrides on top of `lib/layout.css`.
- New: `examples/browser/pages/doom-mp/README.md` — links to the chat demo's HTTPS-on-LAN setup, describes the user flow from design §7, notes which keys the player uses (Esc / arrows / Ctrl / Space — same as single-player), and adds one sharp-edge note: both peers must run this exact in-tree build and use the same IWAD (chocolate-doom's `InitConnectData` ships a WAD checksum; mismatches fail the netgame sync screen).
- Modify: `examples/browser/vite.config.ts` — add the new page to `build.rollupOptions.input`.
- Modify: `examples/browser/index.html` and each existing `pages/*/index.html` — add `<a href="/pages/doom-mp/">DOOM (multiplayer)</a>` to the sidebar, alphabetical-ish position next to the existing DOOM entry.

**Task 5b — factor signaling out of the chat demo (small, optional)**

The chat-demo's `main.ts` is currently self-contained. If after writing the doom-mp page we find we're duplicating > 80 LOC of handshake plumbing, lift the SDP-handshake state machine into `examples/browser/lib/webrtc-signaling.ts` and import from both. **Only do this if the duplication is significant** — premature abstraction is explicitly worse than two clear files (per CLAUDE.md's general guidance). The chat demo continues to work either way.
...
2. Start the dev server: `cd examples/browser && VITE_HTTPS=1 npx vite --host`.
...
- **Bump `ABI_VERSION` exactly once for this PR**, in the same commit as the `abi/snapshot.json` regeneration (Task 1). The new `kernel_inject_datagram` export and `HostIO::send_dgram` trait method are ABI-affecting per CLAUDE.md; this is the only ABI bump in scope. Other kernel changes (e.g., fixing the unconditional-EAGAIN bug in `sys_recvfrom`) are explicit follow-ups, not in this PR.
