...
- **The other 17 committed binaries are a DIFFERENT category, and should
  ...
  **Checked, because a guard that has quietly become single-language still
  looks like a guard.** Twelve of the fourteen have both a generator in the
  tree and a live `host/src` producer, so they remain genuine cross-language
  checks. Two have no generator at all: `dylink-archive-wasm32.bin` lost its
  writer AND its generator on 2026-09-10 with the TypeScript `ld.so`, and
  `reference-recipes-wasm32.bin` lost its generator on 2026-09-11 with the
  callerless KFRR engine. Those two cannot be regenerated by anything, which
  is the strongest reason they stay.
