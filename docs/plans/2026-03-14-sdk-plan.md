...
**Goal:** Create a general-purpose SDK (`sdk/`) for compiling C/C++ programs targeting Kandelo, with autoconf support.
...
const MACOS_LLVM = '/usr/local/opt/llvm/bin';
...
export async function findLlvmDir(): Promise<string> {
  ...
  // 3. macOS system LLVM
  if (existsSync(join(MACOS_LLVM, 'clang'))) return MACOS_LLVM;
  ...
  throw new Error(
    ...
    'Install LLVM via:\n' +
    '  macOS:  install LLVM 21+ via your package manager\n' +
