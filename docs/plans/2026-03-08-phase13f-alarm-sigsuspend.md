...
- Build Wasm: `cargo build --target wasm32-unknown-unknown -Z build-std=core,alloc -Z build-std-features=panic_immediate_abort -p kandelo --release && cp target/wasm32-unknown-unknown/release/kandelo.wasm host/wasm/`
...
```bash
cargo build --target wasm32-unknown-unknown -Z build-std=core,alloc -Z build-std-features=panic_immediate_abort -p kandelo --release && cp target/wasm32-unknown-unknown/release/kandelo.wasm host/wasm/
```
