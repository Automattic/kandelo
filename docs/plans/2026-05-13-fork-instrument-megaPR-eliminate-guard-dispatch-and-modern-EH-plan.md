...
- **2.4 — Wire top-level carryovers to trampoline.** Flip `instrument_one_function`'s `has_top_level_stack_carryovers` branch (`instrument.rs:303-304`) from `instrument_one_function_guard_dispatch` to `instrument_one_function_trampoline_dispatch`. D-08 (top-level carryovers) flips from `it()` (passing via guard-dispatch) to `it()` (passing via trampoline) — same outcome, different mechanism. Add a regression assertion that the emitted module no longer routes D-08's function to guard-dispatch (parse the wasm, confirm no `wpk_guard_*` markers).
