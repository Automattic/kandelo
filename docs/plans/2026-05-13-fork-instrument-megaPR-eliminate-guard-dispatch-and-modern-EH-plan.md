...
3. **Commits 5–6:** A2 (multi-target `*_ref` rewrite) + A3 (multi-target plain-catch capture-blocks). C-06 and C-07 un-`ignore` and pass. **Blocked on C1/B1 hang root-cause** (B1 stages 1+2 don't actually close fork-from-catch end-to-end; A2/A3 would extend a broken mechanism).
4. **Commit 7:** A4 funcref/externref aux table. C-08 and C-09 un-`ignore` and pass. **Blocked on C1/B1.**
5. **Commit 8:** C3/C4 — fork-from-handler hardening. Two pieces:
   - **C3 conservative-rule discovery:** extend `instrument::discover_fork_path` to treat every address-taken function as a fork-path root. Surprisingly K-01, K-02, K-04 already pass on commit 1 because libc's call-graph reach catches the address-taken handlers transitively; this commit makes the coverage explicit (a future libc / pthread refactor mustn't silently drop the discovery) and emits a clear instrumentation-trace marker the test suite can assert. **Defensive insurance; scope of "every address-taken function" needs clarification — the literal interpretation would massively over-broaden the instrumentation set vs. a subset like sigaction/pthread-cleanup-registered handlers.**
...
7. **Commit 10:** Update `docs/fork-instrumentation.md`. ✅ **Partially landed 2026-05-14 via `5c6f63a7e`, `a781fa64e`, `d2dad5948`.** Dispatch-schemes table updated, §Carryover-spilling rewritten to cover sub-region + direct-call landings, §Not-yet-gated side effects replaced with §Side effects during REWIND — no gating needed, §Per-function transform examples marked as historical guard-dispatch shape, trampoline.rs module docstring refreshed. Outstanding: §Fork from handler context (C3 + K-03 documentation) — depends on commit 8 landing.
8. **Commit 11:** Update `docs/posix-status.md` fork-from-catch entry to "Full" + new ucontext/A5 wording. Update review doc with implementation-complete cross-references. **Blocked on C1/B1** (the entry can only flip to "Full" once fork-from-catch actually works end-to-end). Partial update at `35b2acbae` clarifying the post-2.5/2.6/3/4 state (still "Known-broken"; SpiderMonkey Gate 1.B not cleared).
