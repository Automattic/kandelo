...
- ABI 42 creates a real publication cycle: the strict bottle-backed shell
  ...
  5. publish the complete intended ABI-42 closure and record the resulting tap
     ...
     mirror tag or digest. Merge that cutover only after the anonymous public
     mirror proof is green. Retire the dormant source bridge in a later cleanup
     so the product switch does not also remove recovery code.
