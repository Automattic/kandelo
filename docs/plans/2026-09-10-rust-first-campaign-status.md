...
| B34 | `statx` writes half a device number, and its field offsets are unguarded | **NEW 2026-09-11, in scope for #1350.** `kernel_statx` (`wasm_api.rs:12931`) squeezes the whole 64-bit `st_dev` into `stx_dev_major` and never writes `stx_dev_minor` at offset 140, so a guest reconstructing with `makedev()` gets a different number than `stat()` reports for the same file. Nothing in-tree is a victim -- the statx branch in `fstatat.c` is dead on this target because `time_t` is already 64-bit -- but guest software can call `statx()` directly, and half-writing a field is worse than not implementing it: it returns a plausible wrong answer instead of a refusal. Found while proving the dev-space partition, and it is a blocker for that partition, since `(2^63 + n) as u32 == n` would make the first host device report `stx_dev_major = 0`. **The deeper half is the mechanism:** those offsets (128/132/136/140) are bare magic numbers with no constant in `crates/shared` and no ABI snapshot coverage, so nothing would catch them drifting from the layout musl expects -- which is how a field came to be half-written in the first place |
