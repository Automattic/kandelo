...
| B34 | `statx` writes half a device number, and its field offsets are unguarded | **FIXED 2026-09-11** (the defect), **OPEN** (the mechanism). It was **two** bugs: `stx_dev_minor` at offset 140 was never written, AND the value written as "major" was the low 32 bits of `st_dev` rather than the major half -- so even a small device number reassembled wrong, independent of any base change. `makedev` verified against the real `libc/musl/include/sys/sysmacros.h`, not from memory; the split is lossless across all 64 bits, so the fix is exact. The guard asserts `makedev(major(d), minor(d)) == d` over `u64::MAX`, `2^63` and every kernel device constant, so it fails for *any* lossy split rather than only the one being fixed. **The mechanism half has an answer now, and it is broader than statx:** putting the offsets in `process_layout.rs` buys nothing on its own, because `render_process_layouts_header` in `dump_abi.rs` emits only six modules -- `iovec`, `msghdr`, `cmsghdr`, `multicast_group_request`, `rt_sigqueueinfo`, `sigevent`. Drift protection comes from being in the **generated header**, which gives the C side static asserts. `stat`'s own offsets are equally unguarded today |
| B35 | `st_rdev` is zero for every device node | **NEW 2026-09-11, in scope for #1350.** `WasmStat` has **no `st_rdev` field at all**, while the guest `struct stat` carries one at offset 88 and `process_wire.rs` never writes it. So `st_rdev` reads zero through `stat`, `fstat`, `lstat` and `statx` alike, for every device node. This is a larger POSIX gap than the statx half-write that exposed it: a program distinguishing device nodes, or reconstructing a major/minor to compare against a known device, gets a plausible wrong answer rather than a refusal. Found while fixing B34 and deliberately left visible rather than papered over |
| B36 | A signal-safe wake can complete before a signal the writer has not yet sent | **NEW 2026-09-11, in scope for #1350 as a filed defect; no fix proposed yet.** The underlying race `SIGNAL_SAFE_POLL_WAKE_DELAY_MS` mitigates. A parked `ppoll`/`pselect` that swapped its sigmask is woken by a **host-scheduled task**, not by anything ordered against the writing process's next channel message — so the wait can complete on a ready descriptor before a signal that process is about to send exists at all. On Linux the equivalent ordering falls out of the kernel scheduling the waiter rather than a timer releasing it. **The proposed fix of "wait until the writer's queue is empty" was refuted rather than deferred:** after `write()` returns the child is executing guest code, so the kernel cannot know whether it will ever syscall again, and any bound on the wait is the constant wearing a different hat. **POSIX does not require the ordering** — in the failing shape the signal is generated after `write()` returned, so the descriptor was ready first, and `pselect`'s atomicity guarantee covers a signal arriving in the mask-swap window, not a preference for a not-yet-generated signal over a ready descriptor. So this is a robustness gap rather than a conformance gap, which is why the constant's removal is a judgement call and not a cleanup |
| B37 | The SDK build mutates an input of its own cache key | **NEW 2026-09-11, in scope for #1350.** `kandelo-sdk/wasm32` fails with `cache key changed while building kandelo-sdk (wasm32): before bed8dd16…, after b253e489…`. The build runs `npm` inside the source tree, which changes a file the cache key is computed over, so the key it finishes with is not the key it started with. This is a defect with a name rather than flake: it fails **every** time, it blocks provisioning in a fresh worktree, and it is indistinguishable at the call site from a genuine concurrent-edit race — which is the more dangerous reading, because that one invites a retry loop instead of a fix. Found while trying to produce the eight production VFS images for the lazy-identity validation, which it blocks along with an unrelated upstream `m4` mirror timeout that cascades through `coreutils-docs` to `product/platform-rootfs` |
| W-5 | Deferred-file metadata moves into the SFFS body | **NEW 2026-09-11, supersedes the kernel→host manifest contract (cancelled, never built).** The maintainer's question — why is this in JSON rather than the filesystem format? — dissolved a boundary the coordinator had overstated: `klzy.rs` governs who *decides* a URL may be fetched, not who *stores* the string, and the format already stores symlink targets without that conferring authority over what they resolve to. With no VFS image persistence and images rebuilt per ABI epoch, there is no migration and no compatibility window, and ABI 44 is unreleased — so this is the cheapest moment the change will ever have. **Design settled:** a dedicated section addressed by a hidden inode named from one superblock `u32`, costing zero blocks when absent; the kernel parses `ino`, `size` and `payload_len` and treats the payload as opaque; the host keeps fetch authority entirely. **xattrs were disqualified, not merely costed** — they are guest-visible by definition, so a guest could `setxattr` the URL its own files are fetched from, an authority leak created by the storage choice. **It deletes** the identity join, the mismatch class, the lazy JSON sections and `entries[]` — which is what W-4 was chasing, reached by a better route |
| B38 | Setuid-root binaries ship as lazy refs with no integrity digest | **NEW 2026-09-11. Security-relevant; needs verification of one inferred step before it is acted on.** `images/rootfs/PACKAGES.toml` sets `default_install = "lazy"`. `login` opts out with `install = "eager"`; **`sudo` and `sudo-lite` do not** — and all three are `mode = "4755", uid = 0`, i.e. setuid root. Separately, `LazyFileEntry` carries `{ino, generation, dataSequence, path, paths, url, size}` and **no integrity field**, while the lazy *archive* types do carry `integrity`. **NOW FULLY VERIFIED (2026-09-11).** The inferred step was checked at the emitter: `scripts/generate-rootfs-package-manifest.mjs:624-631` resolves `install` as `pkg.install ?? defaultInstall ?? config.default_install ?? "lazy"`, and the lazy branch emits `${path} f ${mode} ${uid} ${gid} lazy_url=… lazy_size=…` — **mode preserved, URL and size the only attributes, no digest anywhere on that path.** So two setuid-root binaries are fetched by URL with length as the only check. Bytes of the same length from a substituting host, cache or network position execute as root inside the guest, and the platform offers no check of its own — HTTPS is transport security, not artifact integrity, and a third-party host need not use it. **This settles a W-5 design question**: forbidding `setuid` + deferred is not available, because production depends on the combination, so the answer must be mandatory kernel-side digest verification before the setuid bit is honoured. It is also the strongest argument for the maintainer's ruling that the kernel, not the host, must own materialisation integrity |
