...
Each library / program / source dependency under `examples/libs/<name>/` ships two TOML files:
...
- **`build.toml`** — the **project view**: `script_path` (this project's actual build), `repo_url` + `commit` (where the recipe lives in this project), and `[binary]` declaring where binaries are published. Differs per project.

Binary resolution does not look at either of those files for archive URLs. Instead, a per-release `index.toml` ledger hosted at `build.toml`'s `index_url` is the single source of truth — see `docs/plans/2026-05-13-binary-resolution-via-index-ledger-design.md` for the full design.

**Resolver flow** (`xtask build-deps resolve`, called from build scripts):

1. Read `examples/libs/<name>/package.toml` for the recipe.
2. Read `examples/libs/<name>/build.toml` for the binary source.
