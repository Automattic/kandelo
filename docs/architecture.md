...
Regular-file mappings add a backend-qualified stable identity and a
lifetime-stable host handle. Node uses native device/inode identity; VFS
backends scope device/inode identity to the backend object, so hard links and
the same backend mounted at more than one path alias correctly without
colliding with a different backend. Dirty mapped data is published before
...
after the original guest fd is closed or the pathname is later unlinked or
renamed.
