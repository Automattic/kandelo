...
channel dispatch because loading the replacement Worker is asynchronous. The
legacy/direct kernel exports still follow the reusable-instance rule: they
return 0 after the host accepts the replacement instead of deliberately
trapping as successful control flow. In both paths the host retires the old
process Worker, so POSIX `exec` never resumes the old guest image.
...
a missing or rejected transition remains a loud protocol failure. A normal
return from the kernel exit transaction is also insufficient by itself: the
host verifies the Process is actually `Exited` before publishing a clean exit.
