...
1. **FetchNetworkBackend**: Buffers an entire HTTP request from the Wasm process, sends it via `fetch()`, and returns the raw HTTP response bytes. It uses the same validated proxy and browser-credential boundary as `TlsNetworkBackend` when a proxy is configured. Works for simple HTTP clients.

2. **TlsNetworkBackend**: Terminates the guest's TLS connection with a generated, in-VFS CA and sends the decoded HTTP request through browser `fetch()`. Embedders can set `BrowserKernelOptions.corsProxyUrl`; the host validates that deployment setting as an absolute HTTPS prefix (or an HTTP loopback relay for local development) before it crosses the main-thread/worker protocol. The backend then routes both plain-HTTP and terminated-TLS requests through the same proxy path. It preserves the guest method, request body, and ordinary headers, while browser-controlled transport headers are regenerated by `fetch()`. A guest `Authorization` header produces only the proxy's narrow authorization-forwarding opt-in; a guest cannot request forwarding of cookies or other proxy credentials. All backend fetches use `credentials: "omit"`, so cookies, browser HTTP-auth state, and client TLS state from the embedding page never enter the guest network.
...
The browser demo configures `BrowserKernel` explicitly so guest requests keep
their method, body, and headers instead of depending on page-level service
worker rewriting. Development uses a same-origin Vite relay; production uses
the configured public CORS proxy. Node.js continues to use direct socket
transport and does not pass through this browser-only policy.
