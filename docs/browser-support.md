...
- **Exec reads from filesystem by default**: Like a real OS, `exec()` reads binaries from the kernel-side `MemoryFileSystem`. Programs are baked into the VFS image at build time (or written by the page in the legacy path before spawning). Symlinks are used for multicall binaries (e.g., coreutils). Browser callers that need Node-style stable executable mappings can pass `BrowserKernel({ execPrograms })`, using virtual paths mapped to immutable `ArrayBuffer`/`Uint8Array` bytes. Those mappings are resolved before the guest VFS, which is useful for test harnesses that repeatedly exec a tool binary while guest code mutates or replaces filesystem paths.
