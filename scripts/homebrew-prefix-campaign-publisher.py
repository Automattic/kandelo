...
CACHE_ARCHIVE_TOOL = ROOT / "scripts/homebrew_cache_archive.py"
...
CACHE_ARCHIVE = load_tool(
    "homebrew_prefix_campaign_publisher_cache_archive",
    CACHE_ARCHIVE_TOOL,
)
CAMPAIGN_FORMULA = load_tool(
    "homebrew_prefix_campaign_publisher_formula",
...
def formula_identity(path: pathlib.Path) -> str:
    try:
        return CAMPAIGN_FORMULA.formula_identity(
            path,
...
def bind_formula_destination(
    ...
    """Normalize build-only bottle metadata to the campaign destination."""
    try:
...
def prepare(
    *,
    ...
    try:
        ...
        target_record = index[formula]
        target_formula_path = tap_root / f"Formula/{formula}.rb"
        destination_changed = bind_formula_destination(
            target_formula_path,
            target_record["destination"]["bottle_rebuild"],
            target_record["formula_source"][
                "identity_excluding_bottle_sha256"
            ],
            target_record["version"],
            target_record.get("previous_version"),
        )
        destination_commit = target_commit
        destination_tree = target_tree
        if destination_changed:
            run_git(tap_root, "add", f"Formula/{formula}.rb")
            destination_tree = run_git(tap_root, "write-tree")
            destination_commit = deterministic_commit(
                tap_root,
                parent=target_commit,
                tree=destination_tree,
                label=f"{formula} reserved bottle destination",
            )

        fetched: dict[str, pathlib.Path] = {}
        fetched_records: list[dict[str, str]] = []
        prepared_commit = destination_commit
        if arch is not None:
            ...
            prepared_commit = deterministic_commit(
                ...
                tree=prepared_tree,
                label=f"{formula}/{arch} publisher inputs",
            )
        else:
            prepared_tree = destination_tree
...
def bottle_filename(
    ...
    return (
        f"{formula}--{pkg_version}.{tag}.bottle{suffix}.tar.gz"
    )


def verify_built_bottle(
    *,
    tap_root: pathlib.Path,
    campaign_work_root: pathlib.Path,
    receipt_path: pathlib.Path,
    campaign_tag: str,
    bottle_path: pathlib.Path,
    bottle_json_path: pathlib.Path,
    output_path: pathlib.Path,
) -> dict[str, Any]:
    """Verify Homebrew emitted the campaign-reserved bottle identity."""
    verify(tap_root=tap_root, receipt_path=receipt_path)
    receipt, _receipt_payload = EXECUTOR.load_json_bytes(
        receipt_path,
        "campaign publisher checkout receipt",
    )
    campaign_work_root = EXECUTOR.real_directory(
        campaign_work_root,
        "campaign publisher work root",
    )
    campaign_path = campaign_work_root / "campaign.json"
    campaign, campaign_payload, index = EXECUTOR.load_campaign(
        campaign_path
    )
    if campaign_tag != receipt["campaign"]["tag"]:
        fail("campaign tag differs from the publisher receipt")
    campaign_match = CAMPAIGN_TAG.fullmatch(campaign_tag)
    campaign_sha256 = sha256_bytes(campaign_payload)
    if (
        campaign_match is None
        or campaign_match.group(1) != campaign_sha256
    ):
        fail("campaign work root differs from the selected campaign tag")
    if receipt["campaign"]["sha256"] != campaign_sha256:
        fail("campaign receipt SHA-256 differs from the selected campaign")

    formula = receipt["formula"]
    arch = receipt["preparation"]["arch"]
    if formula not in index or arch not in ("wasm32", "wasm64"):
        fail("campaign receipt has no build destination")
    record = index[formula]
    variants = [
        variant
        for variant in record["variants"]
        if variant.get("arch") == arch
    ]
    if len(variants) != 1:
        fail("campaign build architecture is outside the Formula variants")
    disposition = variants[0].get("disposition")
    if (
        not isinstance(disposition, dict)
        or disposition.get("kind")
        not in ("required-build", "required-rebuild")
    ):
        fail("campaign Formula variant is not admitted for a source build")
    authority = campaign["authority"]
    if (
        authority["source_tap_commit"] != receipt["source"]["commit"]
        or authority["guest_layout"] != receipt["campaign"]["guest_layout"]
    ):
        fail("campaign work root differs from the publisher authority")

    value, _payload = EXECUTOR.load_json_bytes(
        bottle_json_path,
        f"{formula}/{arch} built bottle JSON",
        canonical=False,
    )
    ...
    if (
        ...
        fail("built bottle JSON has the wrong Formula identity")
    formula_metadata = value[formula_key].get("formula")
    bottle = value[formula_key].get("bottle")
    if not isinstance(formula_metadata, dict) or not isinstance(bottle, dict):
        fail("built bottle JSON lacks Formula or bottle metadata")
    pkg_version = EXECUTOR.require_string(
        formula_metadata.get("pkg_version"),
        "built bottle package version",
        EXECUTOR.VERSION,
    )
    expected_formula_path = (
        f"Library/Taps/{tap_name.split('/', 1)[0]}/"
        f"homebrew-{tap_name.split('/', 1)[1]}/"
        f"Formula/{formula}.rb"
    )
    if (
        formula_metadata.get("name") != formula
        or formula_metadata.get("path") != expected_formula_path
        or pkg_version != record["version"]
    ):
        fail("built bottle Formula metadata differs from the campaign")
    expected_root_url = (
        "https://ghcr.io/v2/"
        f"{authority['tap_repository'].lower()}"
    )
    if bottle.get("root_url") != expected_root_url:
        fail("built bottle root URL differs from the campaign registry")
    rebuild = EXECUTOR.require_int(
        bottle.get("rebuild"),
        "built bottle rebuild",
    )
    destination = record["destination"]
    destination_rebuild = destination["bottle_rebuild"]
    if rebuild != destination_rebuild:
        fail("built bottle does not use the campaign-reserved rebuild")
    tag = f"{arch}_kandelo"
    ...
    if (
        ...
        fail("built bottle JSON has the wrong architecture")
    tag_metadata = tags[tag]
    digest = EXECUTOR.require_string(
        tag_metadata.get("sha256"),
        "built bottle SHA-256",
        EXECUTOR.SHA256,
    )
    source_filename = bottle_filename(
        formula, pkg_version, tag, rebuild
    )
    if tag_metadata.get("local_filename") != source_filename:
        fail("built bottle local filename differs from its rebuild")
    if not bottle_path.is_absolute():
        fail("built bottle archive path must be absolute")
    try:
        source_size = bottle_path.lstat().st_size
    except OSError as error:
        fail(f"built bottle archive is unavailable: {error}")
    archive = CACHE_ARCHIVE.hash_exact_local_archive(
        bottle_path,
        source_filename,
        digest,
        source_size,
    ...
    expected_reference = (
        ...
        else f"{pkg_version}-{destination_rebuild}"
    )
    if (
        destination["reference"] != expected_reference
        or destination["remote"]
        != f"ghcr.io/{authority['tap_repository'].lower()}/{formula}"
    ):
        fail("campaign destination is not canonical for the built bottle")
    destination_filename = bottle_filename(
        formula,
        pkg_version,
        tag,
        destination_rebuild,
    ...
    if source_filename != destination_filename:
        raise AssertionError("validated destination filename differs")
    if not output_path.is_absolute() or output_path.name in ("", ".", ".."):
        fail("built bottle verification output must be an absolute file")
    requested_parent = output_path.parent
    if not requested_parent.is_dir() or requested_parent.is_symlink():
        fail("built bottle verification parent must be a real directory")
    output_parent = requested_parent.resolve()
    output_path = output_parent / output_path.name
    if output_path.exists() or output_path.is_symlink():
        fail("built bottle verification output must not already exist")
    verification = {
        "arch": arch,
        "archive": {
            "bytes": archive["bytes"],
        ...
        "campaign": {
            "sha256": receipt["campaign"]["sha256"],
            "tag": campaign_tag,
        },
        "destination": {
            "bottle_rebuild": destination_rebuild,
            "reference": destination["reference"],
            "remote": destination["remote"],
        },
        "formula": formula,
        "kind": "kandelo-homebrew-campaign-bottle-verification",
        "schema": 1,
    }
    with output_path.open("xb") as output:
        output.write(pretty_json(verification))
    return verification
...
def parse_args() -> argparse.Namespace:
    ...
    built_parser = commands.add_parser("verify-built-bottle")
    built_parser.add_argument("--tap-root", required=True)
    built_parser.add_argument("--campaign-work-root", required=True)
    built_parser.add_argument("--receipt", required=True)
    built_parser.add_argument("--campaign-tag", required=True)
    built_parser.add_argument("--bottle", required=True)
    built_parser.add_argument("--bottle-json", required=True)
    built_parser.add_argument("--out", required=True)
...
def main() -> int:
    ...
    try:
        ...
        elif arguments.command == "verify-built-bottle":
            verify_built_bottle(
                tap_root=pathlib.Path(arguments.tap_root),
                campaign_work_root=pathlib.Path(
                    arguments.campaign_work_root
                ),
                receipt_path=pathlib.Path(arguments.receipt),
                campaign_tag=arguments.campaign_tag,
                bottle_path=pathlib.Path(arguments.bottle),
                bottle_json_path=pathlib.Path(arguments.bottle_json),
        ...
        CACHE_ARCHIVE.CacheArchiveError,
        OSError,
        UnicodeError,
    ) as error:
        print(
