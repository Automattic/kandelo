...
Build an ABI-current VFS image containing unmodified upstream Homebrew and the
Kandelo programs needed to start it inside NodeKernelHost.
...
if git --git-dir="$BREW_GIT_DIR" remote get-url origin >/dev/null 2>&1; then
    git --git-dir="$BREW_GIT_DIR" remote set-url origin "$BREW_REPOSITORY"
else
    git --git-dir="$BREW_GIT_DIR" remote add origin "$BREW_REPOSITORY"
fi

echo "==> Fetching Homebrew $BREW_REVISION"
git --git-dir="$BREW_GIT_DIR" fetch -q --depth=1 origin "$BREW_REVISION"
RESOLVED_BREW_REVISION="$(git --git-dir="$BREW_GIT_DIR" rev-parse 'FETCH_HEAD^{commit}')"
if [ "$RESOLVED_BREW_REVISION" != "$BREW_REVISION" ]; then
    echo "build-homebrew-bootstrap: fetched Homebrew $RESOLVED_BREW_REVISION, expected $BREW_REVISION" >&2
    exit 1
fi
...
node --input-type=module - "$IMAGE_METADATA" "$ABI_VERSION" "$BREW_REPOSITORY" "$BREW_REVISION" "$BREW_ARCHIVE_SHA256" <<'NODE'
import { writeFileSync } from "node:fs";
const [output, abi, repository, revision, archiveSha256] = process.argv.slice(2);
const metadata = {
  schema: 1,
  ...
  kandelo_abi: Number(abi),
  notes: [
    "The upstream Homebrew checkout is ABI-independent and unpatched.",
...
/etc/homebrew/brew.env f 0644 0 0 src=images/homebrew/brew.env
...
echo "==> Homebrew $BREW_REVISION ($BREW_ARCHIVE_SHA256)"
