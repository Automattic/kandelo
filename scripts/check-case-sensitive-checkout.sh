...
for dir in "$@"; do
    ...
    overall_status=1
    {
        ...
        echo ""
        echo "One spelling per group keeps a directory entry; the others have"
        echo "none. A test discovered by directory listing therefore does not"
        echo "exist under the missing spellings and is SILENTLY NOT RUN, while"
        echo "the total still looks plausible. A test opened by exact path"
        echo "instead reads the surviving file's bytes, not its own."
        echo ""
        echo "Either way the results are not what they claim, and must not be"
        echo "reported as conformance."
        echo ""
