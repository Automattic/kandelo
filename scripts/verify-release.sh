...
if [ -n "$TAG" ]; then
    ...
    curl --retry 5 --retry-delay 2 --retry-all-errors -fsSL --retry 3 -o "$MANIFEST_TMP" "$ARCHIVE_BASE/manifest.json" || {
...
while read -r archive_name archive_sha; do
    if [ "$ARCHIVE_BASE" != "${ARCHIVE_BASE#http}" ]; then
        ...
        actual=$(curl --retry 5 --retry-delay 2 --retry-all-errors -fsSL --retry 3 "$url" 2>/dev/null | shasum -a 256 | awk '{print $1}')
