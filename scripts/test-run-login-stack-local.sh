...
assert_body="$(
  sed -n \
    '/^assert_formula_test_checker() {$/,/^}$/p' \
    "$HARNESS"
)"
[ -n "$assert_body" ] ||
  fail "local login harness lacks its between-Formula checker assertion"
! grep -Fq 'seal-homebrew-formula-checker.sh' <<<"$assert_body" ||
  fail "between-Formula validation must not restage changed checker bytes"
grep -Fq 'assert_formula_test_checker "$formula sidecar generation"' \
  "$HARNESS" ||
  fail "local login harness does not revalidate checker identity between Formulae"
