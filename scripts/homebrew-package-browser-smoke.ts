...
import {
  HOMEBREW_CELLAR,
  browserFormulaUnsupportedReason,
...
async function main(): Promise<void> {
  ...
  try {
    if (unsupportedReason) {
      ...
      try {
        for (const formula of options.formulas) {
          const formulaReason = browserFormulaUnsupportedReason(formula);
          if (formulaReason) {
            await runCase(
              outcomes,
              options,
              tapCommit,
              `browser_smoke_${formula}_unsupported`,
              async () => {
                throw new SkipCase(formulaReason);
              },
              join(formulaDir(options, formula), `${formula}-unsupported.txt`),
            );
            continue;
          }
