...
# name, so this value reaches every child started below -- including the
# `cargo build --release -p xtask` at :411, whose cc-rs build scripts then
# see a wasm cross-compiler as the HOST compiler. The browser runners avoid
# this by assigning WASM32_CC instead (see the comment in
# scripts/run-browser-libc-tests.sh); this file has not been converted.
