...
export async function inspectHomebrewMainShellPublicProduct(input: {
  ...
  if (
    ...
    !bootstrap.descriptor.activation.roots.includes(
      fs.readlink(activationRoot),
    )
  ) {
    throw new Error(
  ...
  const unknownTrees = allPendingTrees.filter(
    (tree) =>
      !bottleTrees.includes(tree) && !bootstrapTrees.includes(tree),
  );
  ...
  const mirroredPackages = new Set(
    plan.assets.map((asset) => asset.package),
  );
...
function parseArgs(args: string[]): {
  ...
  return {
    ...
    homebrewBootstrapSpec: resolve(
      values.get("--homebrew-bootstrap-spec")!,
    ),
    homebrewRuntimeSupport: resolve(
      values.get("--homebrew-runtime-support")!,
    ),
