...
export function createHomebrewBootstrapGuestContractScript(): string {
  ...
  return [
    ...
    "expect_equal \"$(/usr/bin/git --exec-path)\" '/usr/libexec/git-core' 'git exec path'",
    "[ -x /usr/libexec/git-core/git-remote-http ] || fail 'Git HTTP helper is missing from its exec path'",
    "[ -x /usr/libexec/git-core/git-remote-https ] || fail 'Git HTTPS helper is missing from its exec path'",
    ...
    "  abort \"metadata schema mismatch\" unless metadata.fetch(\"schema\") == 2",
    "  abort \"layout schema mismatch\" unless layout.fetch(\"schema\") == 2",
    ...
    "  portable_version = metadata.fetch(\"homebrew_portable_ruby_version\")",
    "  portable = layout.fetch(\"portableRuby\")",
    "  abort \"portable Ruby version mismatch\" unless File.binread(portable.fetch(\"versionFile\")).chomp == portable_version",
    "  abort \"portable Ruby current alias mismatch\" unless File.readlink(portable.fetch(\"current\")) == portable_version",
    "  versioned_runtime = File.join(portable.fetch(\"root\"), portable_version)",
    "  abort \"portable Ruby runtime alias mismatch\" unless File.readlink(versioned_runtime) == portable.fetch(\"initialRuntimeRoot\")",
    "  abort \"portable Ruby executable mismatch\" unless File.realpath(File.join(portable.fetch(\"current\"), \"bin/ruby\")) == \"/usr/bin/ruby\"",
    "  ca = layout.fetch(\"certificateAuthority\")",
    "  abort \"Homebrew CA alias mismatch\" unless File.readlink(ca.fetch(\"homebrewBundle\")) == ca.fetch(\"systemBundle\")",
    "  git = layout.fetch(\"git\")",
    "  abort \"Git HTTP helper mismatch\" unless File.realpath(git.fetch(\"httpHelper\")) == \"/usr/bin/git-remote-http\"",
    "  abort \"Git HTTPS helper mismatch\" unless File.realpath(git.fetch(\"httpsHelper\")) == \"/usr/bin/git-remote-http\"",
    "'",
