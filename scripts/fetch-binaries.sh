...
#
# Packages without `build.toml` (kernel-test-programs, manifest-only
# entries, libraries that ship only as link-time inputs) are skipped
# here, as are all `kind = "source"` packages — those are never
# published as binary archives and cannot source-build under
# `--fetch-only`, even the ones that carry a build.toml purely for its
# `inputs` cache-key list (e.g. wayland-protocols).
