...
#   --prune              Delete binaries/objects/ entries not referenced
#                        by the current manifest.
#   --offline            Fail if an object isn't already in objects/
#                        rather than hitting the network.
#   --pr <N>             Force PR overlay from pr-<N>-staging. Without
#                        this flag, the script auto-detects the PR via
#                        the public api.github.com /commits/{sha}/pulls
#                        endpoint when the current HEAD belongs to an
#                        open PR. Skipped when binaries.lock.pr already
#                        exists locally.
#   --allow-stale <csv>  Comma-separated list of package names whose
#                        cache_key_sha is permitted to differ from the
#                        durable manifest's stamped value. install-release
#                        skips those entries with a warning; mismatches
#                        for any other name still hard-fail. For local
#                        developer use ("I know these packages have
#                        local edits") or as the explicit form behind
#                        --auto-allow-stale.
#   --auto-allow-stale   Compute the allowlist automatically by running
#                        `xtask cache-key-diff` against the just-
#                        downloaded durable manifest. Recommended for
#                        the staging-build CI flow — keeps fetch-binaries'
#                        skip set in lockstep with stage-pr-overlay's
#                        build set (both derive from the same
#                        compute_sha vs. baseline comparison). The
#                        computed list is logged before use so CI logs
#                        record what was permitted to be stale on each
#                        run. Mutually exclusive with --allow-stale.
#
# Exit codes:
#   0  success
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --auto-allow-stale)  AUTO_ALLOW_STALE=1; shift ;;
        -h|--help)
            sed -n '3,52p' "$0"
            exit 0
            ;;
...
if [ -n "$ALLOW_STALE" ] && [ "$AUTO_ALLOW_STALE" = "1" ]; then
    echo "ERROR: --allow-stale and --auto-allow-stale are mutually exclusive" >&2
    exit 2
fi
...
if jq -e '.entries[] | select(.archive_name != null)' "$MANIFEST_OBJ" > /dev/null 2>&1; then
    if [ "$OFFLINE" = "1" ]; then
        ...
        # --auto-allow-stale: compute the allowlist by running
        # `xtask cache-key-diff` against the just-downloaded durable
        # manifest. Same shared compute_sha vs. baseline logic as
        # stage-pr-overlay's build set, so the two cannot drift apart.
        if [ "$AUTO_ALLOW_STALE" = "1" ]; then
            ALLOW_STALE=$(cargo run -p xtask --target "$HOST_TARGET" --quiet -- \
                cache-key-diff \
                    --manifest "$MANIFEST_OBJ" \
                    --arch wasm32 \
                    --arch wasm64)
            echo "fetch-binaries: --auto-allow-stale resolved to: ${ALLOW_STALE:-(empty)}"
        fi

        # When --allow-stale is set (either explicitly or via auto),
        # thread it to the durable install-release call only. The
        # overlay manifest is generated by stage-pr-overlay using the
        # SAME compute_sha as the consumer (see
        # stage_pr_overlay::changed_packages), so its entries are
        # guaranteed to match locally — the allowlist would never
        # apply, and silently passing it would mask any real
        # overlay-side bug.
        DURABLE_ALLOW_STALE_ARGS=()
        if [ -n "$ALLOW_STALE" ]; then
            DURABLE_ALLOW_STALE_ARGS=(--allow-stale "$ALLOW_STALE")
            echo "fetch-binaries: --allow-stale=$ALLOW_STALE (durable mismatches for these names will skip)"
        fi
