...
test(
  "binds the sealed image, deferred brew source, and public mirror plan",
  async () => {
    ...
    assert.deepEqual(result, {
      ...
      bottle_mirror: {
        ...
        plan_url:
          `${fixture.plan.release_root}/${fixture.plan.manifest_asset}`,
...
test(
  "rejects bootstrap bytes that are not the deferred tree in the image",
  async () => {
    const fixture = await productFixture();
...
test(
  "rejects a public mirror outside the runtime-support catalog",
  async () => {
    const fixture = await productFixture({
...
async function productFixture(options?: {
  ...
  const identities = runtimeSupport.additionalFormulaOrder
