...
const PROGRAM_SMOKES: Record<string, ProgramSmokeSpec> = {
  bzip2: {
    argv0: "bzip2",
    guestPath: `${PREFIX}/bin/bzip2`,
    args: ["--help"],
    expected: /bzip2/i,
  },
  dash: {
    argv0: "dash",
    guestPath: `${PREFIX}/bin/dash`,
    args: ["-c", "echo dash-homebrew-smoke"],
    expected: /dash-homebrew-smoke/,
    description: "dash -c executed a shell command",
  },
  git: {
    argv0: "git",
    guestPath: `${PREFIX}/bin/git`,
    args: ["--version"],
    expected: /git version/i,
  },
  less: {
    argv0: "less",
    guestPath: `${PREFIX}/bin/less`,
    args: ["--version"],
    expected: /less/i,
  },
  lsof: {
    argv0: "lsof",
    guestPath: `${PREFIX}/bin/lsof`,
    args: [],
    expected: /COMMAND|PID|lsof/i,
    description: "lsof enumerated visible process file descriptors",
  },
  msmtpd: {
    argv0: "msmtpd",
    guestPath: `${PREFIX}/bin/msmtpd`,
    args: ["--version"],
    expected: /msmtp|msmtpd/i,
  },
  nano: {
    argv0: "nano",
    guestPath: `${PREFIX}/bin/nano`,
    args: ["--version"],
    expected: /GNU nano|nano/i,
  },
  netcat: {
    argv0: "nc",
    guestPath: `${PREFIX}/bin/nc`,
    args: ["--help"],
    expected: /netcat|listen|connect|usage/i,
    description: "nc help path exercised the network CLI binary",
  },
  vim: {
    argv0: "vim",
    guestPath: `${PREFIX}/bin/vim`,
    args: ["--version"],
    expected: /VIM|Vi IMproved/i,
  },
  wget: {
    argv0: "wget",
    guestPath: `${PREFIX}/bin/wget`,
    args: ["--version"],
    expected: /GNU Wget|Wget/i,
  },
  xz: {
    argv0: "xz",
    guestPath: `${PREFIX}/bin/xz`,
    args: ["--version"],
    expected: /xz/i,
  },
};
...
async function runFormulaSmoke(
  ...
  switch (formula) {
    ...
    default: {
      const spec = PROGRAM_SMOKES[formula];
      if (!spec) throw new SkipCase(`no Node smoke is defined for formula ${formula}`);
      return runProgramSmoke(spec, built, options);
    }
  }
}

async function runProgramSmoke(
  spec: ProgramSmokeSpec,
  built: BuiltVfs,
  options: CliOptions,
): Promise<string> {
  const programBytes = readVfsFile(built.fs, spec.guestPath);
  const result = await runWasm(programBytes, [spec.argv0, ...spec.args], built.imageBytes, options);
  if (result.exitCode !== 0) {
    throw new Error(
      `${spec.argv0} ${spec.args.join(" ")} exited ${result.exitCode}; stderr=${JSON.stringify(result.stderr)}`,
    );
  }
  const combined = `${result.stdout}\n${result.stderr}`;
  if (!spec.expected.test(combined)) {
    throw new Error(
      `unexpected ${spec.argv0} ${spec.args.join(" ")} output: ${JSON.stringify(combined)}`,
    );
  }
  return spec.description
    ?? combined.trim().split("\n").find((line) => line.trim() !== "")
    ?? `${spec.argv0} ${spec.args.join(" ")} passed`;
}
