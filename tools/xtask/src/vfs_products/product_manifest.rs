use crate::vfs_products::canonical_json::{
...
pub fn write_or_check_product_catalog(
    ...
    match mode {
        CatalogWriteMode::Generate => {
            if let Some(parent) = output.parent() {
                fs::create_dir_all(parent).map_err(|error| {
                    format!(
                        "cannot create product catalog directory {}: {error}",
                        parent.display()
                    )
                })?;
            }
        ...
        CatalogWriteMode::Check => {
            ...
            if actual != expected {
                return Err(format!(
                    "generated product catalog {} is stale; run `xtask vfs products generate`",
                    output.display()
                ));
            }
            Ok(())
...
mod tests {
    ...
    use crate::vfs_products::canonical_json::canonical_json_bytes;
    ...
    const PRODUCT_FIXTURE: &str =
        include_str!("../../tests/fixtures/vfs-products/canonical/product.toml");
    const PRODUCT_JSON_FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/vfs-products/canonical/product.json");
...
"#
    ...
    fn canonical_product_fixture_round_trips_to_exact_json() {
        let manifest = parse_product_manifest(
            &crate::repo_root(),
            &fixture_path("tools/xtask/tests/fixtures/vfs-products/canonical/product.toml"),
