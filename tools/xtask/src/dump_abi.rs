...
fn ts_syscall_arg_size(size: shared::host_abi::SyscallArgSize) -> String {
    ...
    match size {
        ...
        SyscallArgSize::Ioctl { request_arg_index } => {
            format!("{{ type: \"ioctl\", requestArgIndex: {request_arg_index} }}")
        }
...
fn syscall_arg_size_json(size: shared::host_abi::SyscallArgSize) -> Value {
    ...
    match size {
        ...
        SyscallArgSize::Ioctl { request_arg_index } => {
            m.insert("type".into(), json!("ioctl"));
            m.insert("requestArgIndex".into(), json!(request_arg_index));
        }
