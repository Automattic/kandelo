...
fn main() -> ExitCode {
    ...
    let sub = match args.next() {
        ...
        None => {
            ...
            eprintln!(
                "subcommands: dump-abi, bundle-program, build-deps, compute-cache-key-sha, sort-package-matrix, package-dependency-artifacts, archive-stage, build-index, set-build-commit, set-package-binary, index-update, homebrew-validate"
            );
