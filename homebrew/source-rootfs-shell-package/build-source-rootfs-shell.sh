...
#
# This non-published recipe intentionally composes only resolver-owned source
# package outputs. The canonical `packages/registry/shell` recipe remains the
# final bottle-backed product package.
set -euo pipefail
...
# injection, credentials, network configuration, and package-manager state
# before the first Node process, not only before the image composer.
