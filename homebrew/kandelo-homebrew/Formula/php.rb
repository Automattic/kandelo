...
class Php < Formula
  ...
  def install
    out_dir = kandelo_build_package("php", "build-php.sh", SOURCE_URL, SOURCE_SHA256,
      script_env: {
        ...
        "WASM_POSIX_DEP_LIBXML2_DIR" => Formula["automattic/kandelo-homebrew/libxml2"].opt_prefix.to_s,
        "WASM_POSIX_DEP_OPENSSL_DIR" => Formula["automattic/kandelo-homebrew/openssl"].opt_prefix.to_s,
        "WASM_POSIX_DEP_SQLITE_DIR" => Formula["automattic/kandelo-homebrew/sqlite"].opt_prefix.to_s,
  ...
  test do
    output = kandelo_run_wasm(bin/"php", ["-r", "echo 2 + 3;"])
