...
class SpidermonkeyNode < Formula
  ...
  def install
    odie "spidermonkey-node currently supports wasm32 only" unless kandelo_arch == "wasm32"
    ...
    bin.install out_dir/"node.wasm" => "spidermonkey-node"
    chmod 0755, bin/"spidermonkey-node"
  end
  ...
  test do
    ...
    runtime = bin/"spidermonkey-node"
    assert_equal "\0asm".b, File.binread(runtime, 4)
    output = run_kandelo_wasm(root, runtime, "--version")
    assert_equal "v22.0.0", output.strip
  end
end
