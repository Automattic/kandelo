...
class Erlang < Formula
  ...
  def install
    ...
    kandelo_install_bin_aliases("erlang", %w[erl beam.smp])
    (lib/"erlang").mkpath
    system "tar", "--zstd", "-xf", out_dir/"erlang-otp.tar.zst", "-C", lib/"erlang"
  end

  test do
    assert_equal "\0asm".b, File.binread(bin/"erlang", 4)
    assert_path_exists lib/"erlang/releases/28/start_clean.boot"
  end
end
