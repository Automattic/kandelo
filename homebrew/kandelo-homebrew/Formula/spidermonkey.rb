...
class Spidermonkey < Formula
  desc "SpiderMonkey JavaScript shell for Kandelo"
  ...
  def install
    odie "spidermonkey currently supports wasm32 only" unless kandelo_arch == "wasm32"
    ...
    ENV["WASM_POSIX_DEP_SOURCE_URL"] = "https://ftp.mozilla.org/pub/firefox/releases/#{version}/source/firefox-#{version}.source.tar.xz"
    ENV["WASM_POSIX_DEP_SOURCE_SHA256"] = "1b034d2117356fda24807a151055132315c6ba58ad2bdf7ec71ee707fac5e028"
    ...
    system "bash", "#{root}/packages/registry/spidermonkey/build-spidermonkey.sh"
    chmod 0755, out_dir/"js.wasm"
    bin.install out_dir/"js.wasm" => "js"
    if (out_dir/"node.wasm").exist?
      libexec.install out_dir/"node.wasm"
    end
    chmod 0755, bin/"js"
  end
  ...
  test do
    ...
    js = bin/"js"
    assert_equal "\0asm".b, File.binread(js, 4)
    output = run_kandelo_wasm(root, js, "-e", "print(1 + 1)")
    assert_equal "2", output.strip
    assert_path_exists libexec/"node.wasm"
  end
end
