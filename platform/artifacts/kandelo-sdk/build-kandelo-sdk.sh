...
mkdir -p \
  "$REPO_ROOT/local-binaries/programs/wasm32" \
  "$REPO_ROOT/local-binaries/platform"
cp "$VFS" "$REPO_ROOT/local-binaries/programs/wasm32/kandelo-sdk.vfs.zst"
cp "$VFS" "$REPO_ROOT/local-binaries/platform/kandelo-sdk.vfs.zst"
echo "build-kandelo-sdk: installed local-binaries/programs/wasm32/kandelo-sdk.vfs.zst"
echo "build-kandelo-sdk: installed local-binaries/platform/kandelo-sdk.vfs.zst"

if [ -n "${WASM_POSIX_DEP_OUT_DIR:-}" ]; then
    mkdir -p "$WASM_POSIX_DEP_OUT_DIR"
    cp "$VFS" "$WASM_POSIX_DEP_OUT_DIR/kandelo-sdk.vfs.zst"
    echo "build-kandelo-sdk: installed $WASM_POSIX_DEP_OUT_DIR/kandelo-sdk.vfs.zst"
fi
