...
function a(n, r = {}) {
	...
	const k = new DataView(n.buffer, n.byteOffset, n.byteLength).getUint32(n.byteLength - 8, !0);
	if (function(e) {
		let t = 4294967295;
		for (const n of e) t = o[255 & (t ^ n)] ^ t >>> 8;
		return (4294967295 ^ t) >>> 0;
	}(A) !== k) throw new s(`${a}: gzip CRC32 mismatch`);
	return function(e, n, r) {
		if (e.byteLength % t !== 0) throw new s(`${n}: TAR byte count is not block-aligned`);
		const o = [];
		let i = 0, a = 0, p = 0, x = null, A = {}, k = !1;
		for (; i + t <= e.byteLength;) {
			const L = e.subarray(i, i + t);
			if (i += t, g(L)) {
			...
			w(L, n);
			const T = l(L, 156, 1, n) || "0", R = $(L, 124, 12, `${n}: TAR entry size`), v = 4095 & $(L, 100, 8, `${n}: TAR entry mode`), z = d(L, n, r.maxPathBytes), P = l(L, 157, 100, n);
			if ("x" === T || "g" === T) {
			...
			if ("." !== O) switch (T) {
				...
				if ("5" !== T) throw new s(`${n}: TAR root marker is not a directory`);
				b(N, n, "directory", O);
			}
		}
		if (!k) throw new s(`${n}: TAR is missing its two-block end marker`);
