...
function Fe(e, t, n) {
	...
	return 253 === e ? 12 === i || 13 === i ? n + 16 : i >= 21 && i <= 34 ? $e(t, n) : 84 === i || i >= 92 && i <= 99 || i >= 112 && i <= 123 || i >= 124 && i <= 131 || i >= 156 && i <= 159 ? n + 1 : n : 254 === e ? 0 === i || 1 === i || 2 === i ? $e(t, n) : 3 === i ? n : i >= 16 && i <= 79 ? $e(t, n) : null : null;
}
function Ve(e, t, n) {
	const [i, r] = Ke(e, t);
	t += r + i;
...
function je(e, t) {
	...
	return a += l, function e(t, r = 0) {
		...
		for (; l < h;) {
			...
			if (11 !== t) {
				if (65 === t) {
					const [e] = ze(n, l), [, t] = ze(n, l), i = l + t;
					if (15 === n[i] || 11 === n[i] && i + 1 === h) return e;
					l = i;
				} else if (16 === t) {
					const [t, i] = Ke(n, l), s = l + i;
...
function tt(e) {
	...
	return a && a.toLowerCase().includes("chunked") && (o = function(e) {
		...
		for (; n < e.length;) {
			...
			if (i < 0) break;
			const r = et.decode(e.subarray(n, i)).trim(), s = parseInt(r, 16);
...
function nt(e, t, n = function() {
	...
	const i = function(e, t) {
		const n = it(e, "kernel_host_adapter_manifest_ptr"), i = it(e, "kernel_host_adapter_manifest_len"), r = rt(n(), "kernel_host_adapter_manifest_ptr"), s = rt(i(), "kernel_host_adapter_manifest_len");
	...
	at("channel header size", i.channelHeaderSize, 72), at("channel data offset", i.channelDataOffset, 72), at("channel data size", i.channelDataSize, g), at("channel minimum size", i.channelMinSize, m);
...
function _t(e) {
	...
	var o;
	const a = s / We, c = void 0 !== e.threadSlots ? gt(e.threadSlots, "process thread slot count") : mt(e.programBytes, e.defaultThreadSlots), l = a + 1, h = l * We, d = l + dt, f = d + 2, u = d + (e.preallocateThreadSlots ? 4 * c : 0), p = Math.max(i, u);
...
var Bn = class {
	...
	async init(e) {
		...
		try {
			const e = await import("./__vite-browser-external-0rkbrCr9.js").then((e) => a(e.default, 1));
	...
	readBytesPreview(e, t, n, i = 160) {
		if (0 === t || n <= 0) return "";
		const r = new Uint8Array(e.buffer), s = Math.max(0, Math.min(n, i, r.length - t));
		if (s <= 0) return "";
		const o = new Uint8Array(s);
		return o.set(r.subarray(t, t + s)), new TextDecoder("utf-8", { fatal: !1 }).decode(o);
	}
	formatSyscallEntry(e, t, n) {
		const i = Tn[t] ?? `syscall_${t}`, r = e.pid, s = this.channelTids.get(`${r}:${e.channelOffset}`), o = void 0 !== s ? `:t${s}` : "";
		switch (t) {
			...
			case E: return `[${r}${o}] write(${n[0]}, ${n[2]}, ${JSON.stringify(this.readBytesPreview(e.memory, n[1], n[2]))})`;
			...
			case z: return `[${r}${o}] brk(0x${(n[0] >>> 0).toString(16)})`;
	...
	_handleSyscallInner(e) {
		...
		s || (s = [], this.syscallRing.set(r, s)), s.push(`  ${this.formatSyscallEntry(e, n, i)}`), s.length > 30 && s.shift(), this.syscallTraceEnabled && (this.syscallTraceRing.length >= this.syscallTraceCap && this.syscallTraceRing.shift(), this.syscallTraceRing.push({
			t: performance.now(),
			pid: e.pid,
			nr: n,
			args: [
		...
		let l = "";
		if (c && (l = this.formatSyscallEntry(e, n, i)), n === Wt || n === Dt) return c && console.error(l), void this.handleFork(e, i);
		if (n === Kt) return c && console.error(l), void this.handleSpawn(e, i);
		if (n === Lt) return c && console.error(l), void this.handleExec(e, i);
		if (n === Rt) return c && console.error(l), void this.handleExecveat(e, i);
		if (n === zt) return c && console.error(l), void this.handleClone(e, i);
		if (n === Ot || n === Nt) return c && console.error(l), void this.handleExit(e, n, i);
		if (n === Vt) return c && console.error(l), void this.handleWaitpid(e, i);
		if (n === qt) return c && console.error(l), void this.handleWaitid(e, i);
		if (n === At) {
			if (c) {
				const t = {
					0: "WAIT",
		...
		if (n === Gt) return c && console.error(l), void this.handleThreadCancel(e, i);
		if (n === gn || n === yn) return c && console.error(l), void this.handleWritev(e, n, i);
		if (n === mn || n === _n) return c && console.error(l), void this.handleReadv(e, n, i);
		if ((n === tn || n === sn) && i[2] > 65536) return void this.handleLargeWrite(e, n, i);
		if ((n === nn || n === rn) && i[2] > 65536) return void this.handleLargeRead(e, n, i);
		if (n === hn) return void this.handleSendmsg(e, i);
		if (n === dn) return void this.handleRecvmsg(e, i);
		if (n === Xt) {
			const t = i[1] >>> 0;
		...
		if (n === Tt || n === Pt) return void this.handleEpollCreate(e, n, i);
		if (n === Bt) return void this.handleEpollCtl(e, i);
		if (n === xt || n === Ht) return void this.handleEpollPwait(e, n, i);
		if (n === bn) return void this.handleIpcShmat(e, i);
		if (n === kn) return void this.handleIpcShmdt(e, i);
		if (n === Sn) return void this.handleSemctl(e, i);
		if (n === Et) return void this.handlePselect6(e, i);
		if (n === vt) return void this.handleSelect(e, i);
		...
		try {
			...
			c && console.error(l + " = KERNEL THROW"), console.error(`[handleSyscall] kernel threw for pid=${e.pid} syscall=${n} args=[${i}]:`, Fn), this.completeChannelRaw(e, -5, 5), this.relistenChannel(e);
			return;
		} finally {
		...
		n === en && 0 === y && this.flushSharedMappings(e, i), n === Jt && 0 === y && (this.flushSharedMappings(e, i), this.cleanupSharedMappings(e.pid, i[0] >>> 0, i[1] >>> 0));
		const b = this.kernelInstance.exports.kernel_get_process_exit_status;
		if (b && b(e.pid) >= 128) this.handleProcessTerminated(e);
		else {
			if (n === An && 0 === y && this.drainMqueueNotification(), this.dequeueSignalForDelivery(e), -1 === y && w === wt) return c && console.error(l + " = -1 (EAGAIN, will retry)"), void this.handleBlockingRetry(e, n, i);
			this.handleSleepDelay(e, n, i, y, w) || (0 !== w || n !== $t && n !== Ft || this.recheckDeferredWaitpids(), 0 === w && n === Mt && (this.scheduleWakeBlockedRetries(), this.reapKilledProcessesAfterSyscall()), c && console.error(l + this.formatSyscallReturn(n, y, w)), this.completeChannel(e, n, i, f, y, w));
		}
	}
	...
	handleBlockingRetry(e, t, n) {
		...
		if (En.has(t) || vn.has(t) || t === fn || t === un || t === pn) {
			...
			if (r && 1 === r(e.pid, i)) return void this.completeChannel(e, t, n, Re[t], -1, wt);
		}
		if (t === An || t === In) {
			const i = n[0], r = this.kernelInstance.exports.kernel_is_fd_nonblock;
			if (r && 1 === r(e.pid, i)) return void this.completeChannel(e, t, n, Re[t], -1, wt);
		...
		if (En.has(t) || vn.has(t)) {
			...
			if (r && !this.socketTimeoutTimers.has(e)) {
				const s = En.has(t) ? 1 : 0, o = Number(r(e.pid, i, s));
				if (o > 0) {
					const i = setTimeout(() => {
	...
	handleFcntlLock(e, t) {
		...
		0 !== n && r.set(i.subarray(n, n + 32), o), s.setUint32(4, wn, !0), s.setBigInt64(8, BigInt(t[0]), !0), s.setBigInt64(16, BigInt(t[1]), !0), s.setBigInt64(24, BigInt(0 !== n ? o : 0), !0);
	...
	handleSelect(e, t) {
		...
		0 !== r ? h.set(l.subarray(r, r + n), f) : h.fill(0, f, f + n), 0 !== s ? h.set(l.subarray(s, s + n), f + n) : h.fill(0, f + n, f + 256), 0 !== o ? h.set(l.subarray(o, o + n), f + 256) : h.fill(0, f + 256, f + 384), d.setUint32(4, vt, !0), d.setBigInt64(8, BigInt(i), !0), d.setBigInt64(16, BigInt(0 !== r ? f : 0), !0), d.setBigInt64(24, BigInt(0 !== s ? f + n : 0), !0), d.setBigInt64(32, BigInt(0 !== o ? f + 256 : 0), !0), d.setBigInt64(40, BigInt(c), !0);
	...
	handleEpollCreate(e, t, n) {
		const i = new DataView(this.kernelMemory.buffer, this.scratchOffset), r = n[0], s = t === Pt ? 0 : r;
	...
	handleEpollCtl(e, t) {
		...
		c.setUint32(4, Bt, !0), c.setBigInt64(8, BigInt(n), !0), c.setBigInt64(16, BigInt(i), !0), c.setBigInt64(24, BigInt(r), !0), c.setBigInt64(32, BigInt(0 !== s ? h : 0), !0), c.setBigInt64(40, BigInt(0), !0), c.setBigInt64(48, BigInt(0), !0);
	...
	handleWritev(e, t, n) {
		...
		if (p <= g - m) {
			...
			l.setUint32(4, t, !0), l.setBigInt64(8, BigInt(i), !0), l.setBigInt64(16, BigInt(h), !0), l.setBigInt64(24, BigInt(s), !0), t === yn && (l.setBigInt64(32, BigInt(n[3]), !0), l.setBigInt64(40, BigInt(n[4]), !0));
			const a = this.kernelInstance.exports.kernel_handle_channel;
			this.currentHandlePid = e.pid, this.bindKernelTidForChannel(e);
			try {
				a(this.toKernelPtr(this.scratchOffset), e.pid);
			...
			const r = this.kernelInstance.exports.kernel_handle_channel, s = t === yn;
	...
	handleLargeWrite(e, t, n) {
		const i = n[0], r = n[1], s = n[2], o = t === sn;
		let a = o ? n[3] : 0;
		...
		if (p <= 65528 && s <= Math.floor(8192)) {
			...
			for (const t of u) {
				...
				for (; n < t.len;) {
					...
					new DataView(c.buffer).setUint32(h, g, !0), new DataView(c.buffer).setUint32(h + 4, u, !0), c.fill(0, g, g + u), s ? (l.setUint32(4, _n, !0), l.setBigInt64(8, BigInt(i), !0), l.setBigInt64(16, BigInt(h), !0), l.setBigInt64(24, BigInt(1), !0), l.setBigInt64(32, BigInt(4294967295 & a), !0), l.setBigInt64(40, BigInt(Math.floor(a / 4294967296)), !0)) : (l.setUint32(4, mn, !0), l.setBigInt64(8, BigInt(i), !0), l.setBigInt64(16, BigInt(h), !0), l.setBigInt64(24, BigInt(1), !0)), this.currentHandlePid = e.pid, this.bindKernelTidForChannel(e);
					try {
						r(this.toKernelPtr(this.scratchOffset), e.pid);
					} finally {
						this.currentHandlePid = 0;
	...
	handleRecvmsg(e, t) {
		...
		if (p > 0 && 0 !== u) {
			...
			for (let t = 0; t < p; t++) {
				...
				if (8 === h ? (n = Number(o.getBigUint64(u + t * A, !0)), i = Number(o.getBigUint64(u + t * A + 8, !0))) : (n = o.getUint32(u + 8 * t, !0), i = o.getUint32(u + 8 * t + 4, !0)), i > 0 && w + i <= 65536) {
					const r = l + w;
					a.fill(0, r, r + i), y.setUint32(e + 8 * t, r, !0), y.setUint32(e + 8 * t + 4, i, !0), k.push({
						base: n,
						len: i,
						kernelBase: r
					}), w += i, w = w + 3 & -4;
		...
		c.setUint32(4, dn, !0), c.setBigInt64(8, BigInt(n), !0), c.setBigInt64(16, BigInt(_), !0), c.setBigInt64(24, BigInt(r), !0);
		...
		8 === h ? (o.setUint32(i + 8, v, !0), o.setUint32(i + 40, x, !0), o.setUint32(i + 44, T, !0)) : (o.setUint32(i + 4, v, !0), o.setUint32(i + 20, x, !0), o.setUint32(i + 24, T, !0)), this.completeChannel(e, dn, t, void 0, C, E);
	}
	handleFork(e, t) {
		...
		if (r < 0) return void this.completeChannel(e, Wt, t, void 0, -1, -r >>> 0);
		...
		if (h) try {
			...
			const r = Fn instanceof Error ? Fn.message : String(Fn);
			console.error(`[kernel-worker] fork child slot reservation failed: ${r}`), this.completeChannel(e, Wt, t, void 0, -1, 12);
			return;
		}
		this.callbacks.onFork(n, i, e.memory, h).then((r) => {
			if (this.processes.has(n)) {
				...
				this.completeChannel(e, Wt, t, void 0, i, 0);
			}
		}).catch(() => {
			(0, this.kernelInstance.exports.kernel_remove_process)(i), this.completeChannel(e, Wt, t, void 0, -1, 12);
		});
	}
	handleSpawn(e, t) {
		...
		(async () => {
			...
			if (!i) return void this.completeChannel(e, Kt, t, void 0, -1, 2);
			if (!((r = i) instanceof ArrayBuffer) && "errno" in r && "number" == typeof r.errno) return void this.completeChannel(e, Kt, t, void 0, -1, i.errno >>> 0);
			var r;
			const s = i instanceof ArrayBuffer ? i : i.programBytes, c = i instanceof ArrayBuffer ? f : i.argv;
			this.handleSpawnAfterResolve(e, t, n, a, d, o, c, u, s);
		}).catch((i) => {
			console.error(`[kernel] spawn resolve error for parent ${n}:`, i), this.completeChannel(e, Kt, t, void 0, -1, 5);
		});
	}
	handleSpawnAfterResolve(e, t, n, i, r, s, o, a, c) {
		...
		d >= this.nextChildPid && (this.nextChildPid = d + 1), this.callbacks.onSpawn(d, c, o, a).then((n) => {
			...
			0 !== i && new DataView(e.memory.buffer).setInt32(i, d, !0), this.completeChannel(e, Kt, t, void 0, 0, 0);
		}).catch((i) => {
	...
	handleExecveat(e, t) {
		...
		if (4096 & i && "" === o) {
			...
			if (!i) return void this.completeChannel(e, Rt, t, void 0, -1, 38);
	...
	handleClone(e, t) {
		...
		if (r < 0) return void this.completeChannel(e, zt, t, void 0, r, s);
	...
	completeWaitpid(e, t, n, i) {
		this.dequeueSignalForDelivery(e), this.completeChannel(e, Vt, t, void 0, n, i);
	}
	wakeWaitingParent(e) {
		let t, n = -1;
		...
		this.waitingForChild.splice(n, 1), i.syscallNr === qt ? (this.writeSignalInfo(i.channel, i.origArgs[2], t.childPid, t.waitStatus), i.options & jt || this.consumeExitedChild(e, t.childPid), this.dequeueSignalForDelivery(i.channel), this.completeChannel(i.channel, qt, i.origArgs, void 0, 0, 0)) : (this.consumeExitedChild(e, t.childPid), this.writeWaitStatus(i.channel, i.origArgs[1], t.waitStatus), this.completeWaitpid(i.channel, i.origArgs, t.childPid, 0));
	}
	...
	handleWaitid(e, t) {
		...
		if ("error" !== c.kind) {
			if ("exited" === c.kind) return this.writeSignalInfo(e, r, c.childPid, c.waitStatus), s & jt || this.consumeExitedChild(o, c.childPid), void this.completeChannel(e, qt, t, void 0, 0, 0);
			if (1 & s) {
				if (0 !== r) {
					const t = new DataView(e.memory.buffer);
	...
	handleFutex(e, t) {
		...
		if (0 === s || 9 === s) {
			...
			if (Atomics.load(o, a) !== r) return this.completeChannelRaw(e, -11, wt), void this.relistenChannel(e);
			let n;
			const i = t[3];
			if (0 !== i) {
	...
	ensureProcessMemoryCovers(e, t, n, i, r) {
		let s = 0, o = 0, a = 0;
		n === Zt ? i >= 0 && (s = i) : n === Yt ? i >= 0 && (o = i, a = r[1], s = o + a) : n === Qt && i >= 0 && (o = i, a = r[2], s = o + a);
	...
	populateMmapFromFile(e, t, n) {
		...
		for (; h < r;) {
			...
			a.setUint32(4, rn, !0), a.setBigInt64(8, BigInt(i), !0), a.setBigInt64(16, BigInt(l), !0), a.setBigInt64(24, BigInt(n), !0), a.setBigInt64(32, BigInt(4294967295 & s), !0), a.setBigInt64(40, BigInt(0 | Math.floor(s / 4294967296)), !0), a.setBigInt64(48, BigInt(0), !0), this.currentHandlePid = e.pid, this.bindKernelTidForChannel(e);
			try {
				o(this.toKernelPtr(this.scratchOffset), e.pid);
			} catch {
				break;
	...
	handleIncomingTcpConnection(e, t, n, i) {
		...
		const b = this.tcpScratchOffset, k = this.kernelInstance.exports.kernel_pipe_is_write_open, A = () => {
			...
			for (; _.length > 0;) {
				...
				const i = u(0, d, this.toKernelPtr(b), n);
				if (i <= 0) break;
			...
			let t = 0;
			for (;;) {
				const i = p(0, f, this.toKernelPtr(b), 65536);
				if (i <= 0) break;
				t += i;
				const r = Buffer.from(e.slice(b, b + i));
	...
	handleSemctl(e, t) {
		...
		if (2 === o && 0 !== s) {
			a.setUint32(4, Sn, !0), a.setBigInt64(8, BigInt(n), !0), a.setBigInt64(16, BigInt(i), !0), a.setBigInt64(24, BigInt(r), !0), a.setBigInt64(32, BigInt(h), !0), a.setBigInt64(40, BigInt(0), !0), a.setBigInt64(48, BigInt(0), !0), l.fill(0, h, h + 72), this.currentHandlePid = e.pid, this.bindKernelTidForChannel(e);
			try {
		...
		if (13 === o && 0 !== s) {
			const t = 1024;
			a.setUint32(4, Sn, !0), a.setBigInt64(8, BigInt(n), !0), a.setBigInt64(16, BigInt(i), !0), a.setBigInt64(24, BigInt(r), !0), a.setBigInt64(32, BigInt(h), !0), a.setBigInt64(40, BigInt(0), !0), a.setBigInt64(48, BigInt(0), !0), l.fill(0, h, h + t), this.currentHandlePid = e.pid, this.bindKernelTidForChannel(e);
			try {
				c(this.toKernelPtr(this.scratchOffset), e.pid);
			} finally {
				this.currentHandlePid = 0;
		...
		if (17 === o && 0 !== s) {
			...
			l.set(o.subarray(s, s + t), h), a.setUint32(4, Sn, !0), a.setBigInt64(8, BigInt(n), !0), a.setBigInt64(16, BigInt(i), !0), a.setBigInt64(24, BigInt(r), !0), a.setBigInt64(32, BigInt(h), !0), a.setBigInt64(40, BigInt(0), !0), a.setBigInt64(48, BigInt(0), !0), this.currentHandlePid = e.pid, this.bindKernelTidForChannel(e);
			try {
				c(this.toKernelPtr(this.scratchOffset), e.pid);
			} finally {
				this.currentHandlePid = 0;
		...
		a.setUint32(4, Sn, !0), a.setBigInt64(8, BigInt(n), !0), a.setBigInt64(16, BigInt(i), !0), a.setBigInt64(24, BigInt(r), !0), a.setBigInt64(32, BigInt(s), !0), a.setBigInt64(40, BigInt(0), !0), a.setBigInt64(48, BigInt(0), !0), this.currentHandlePid = e.pid, this.bindKernelTidForChannel(e);
		try {
			c(this.toKernelPtr(this.scratchOffset), e.pid);
		} finally {
			this.currentHandlePid = 0;
...
var Mn = class {
	...
	if (Rn.prototype.fill) return Rn.prototype.fill.call(e, t, n, i);
	for ((null == n || n < 0) && (n = 0), (null == i || i > e.length) && (i = e.length); n < i; ++n) e[n] = t;
	return e;
}, Nn = function(e, t, n, i) {
	if (Rn.prototype.copyWithin) return Rn.prototype.copyWithin.call(e, t, n, i);
	...
	var i = new Error(t || $n[e]);
	if (i.code = e, Error.captureStackTrace && Error.captureStackTrace(i, Fn), !n) throw i;
	return i;
}, Vn = function(e, t, n) {
	...
	r > n && Fn(3);
	for (var s = 1 << r, o = s, a = -1, c = -1, l = -1, h = s, d = new Ln(512 + (s << 2)), f = new Dn(d, 0, 256), u = new Wn(d, 0, 256), p = new Wn(d, 512, s), g = 512 + (s << 1), m = new Rn(d, g, s), _ = new Rn(d, g + s); a < 255 && o > 0;) {
		var y = Gn(o + 1), w = i >> 3, S = (1 << y + 1) - 1, b = (e[w] | e[w + 1] << 8 | e[w + 2] << 16) >> (7 & i) & S, k = (1 << y) - 1, A = S - o - 1, I = b & k;
		if (I < A ? (i += y, b = I) : (i += y + 1, b > k && (b -= A)), f[++a] = --b, -1 == b ? (o += b, m[--h] = a) : o -= b, !b) do {
			var C = i >> 3;
	...
	for (var n = e.length, i = new Kn(n), r = 0; r < n; ++r) i[r] = t, t += 1 << e[r];
	return i;
}, Qn = new Rn(new Kn([
	0,
	0,
	...
	202050057,
	269422093
]).buffer, 0, 36), ei = Zn(Qn, 0), ti = new Rn(new Kn([
	0,
	0,
	0,
	0,
	...
	252579084,
	16
]).buffer, 0, 53), ni = Zn(ti, 3), ii = function(e, t, n) {
	var i = e.length, r = t.length, s = e[i - 1], o = (1 << n.b) - 1, a = -n.b;
	s || Fn(0);
	for (var c = 0, l = n.b, h = (i << 3) - 8 + Gn(s) - l, d = -1; h > a && d < r;) {
		var f = h >> 3;
	...
	ii(e.subarray(i, i += e[0] | e[1] << 8), t.subarray(0, r), n), ii(e.subarray(i, i += e[2] | e[3] << 8), t.subarray(r, s), n), ii(e.subarray(i, i += e[4] | e[5] << 8), t.subarray(s, o), n), ii(e.subarray(i), t.subarray(o), n);
}, si = function(e, t, n) {
	...
	if (1 == o) {
		...
		return t.b = r + 1, n ? (On(n, e[r], t.y, t.y += a), n) : On(new Rn(a), e[r]);
	}
	if (!(c > e.length)) {
		if (0 == o) return t.b = c, n ? (n.set(e.subarray(r, c), t.y), t.y += a, n) : zn(e, r, c);
		if (2 == o) {
			...
			if (0 == h) g.set(e.subarray(r, r += f), m);
				...
				if (2 == h) {
					var y = function(e, t) {
						var n = 0, i = -1, r = new Rn(292), s = e[t], o = r.subarray(0, 256), a = r.subarray(256, 268), c = new Wn(r.buffer, 268);
						if (s < 128) {
						...
						var b = 0;
						for (n = 0; n < i; ++n) (C = o[n]) > 11 && Fn(0), b += C && 1 << C - 1;
						var k = Gn(b) + 1, A = 1 << k, I = A - b;
						for (I & I - 1 && Fn(0), o[i++] = Gn(I) + 1, n = 0; n < i; ++n) {
							var C = o[n];
							++a[o[n] = C && k + 1 - C];
						}
				...
				(p ? ri : ii)(e.subarray(r, r += u), g.subarray(m), _);
			}
			var w = e[r++];
			if (w) {
				...
				for (var b = [
					...
					} else 2 == A ? (r = (i = jn(e, r, 9 - (1 & k)))[0], b[k] = i[1]) : 3 == A && (t.t || Fn(0), b[k] = t.t[k]);
				}
				...
				for (++w; --w;) {
					...
					B = (P -= ti[D]) >> 3;
					var F = ni[D] + ((e[B] | e[B + 1] << 8 | e[B + 2] << 16) >> (7 & P) & (1 << ti[D]) - 1);
					B = (P -= Qn[R]) >> 3;
					var V = ei[R] + ((e[B] | e[B + 1] << 8 | e[B + 2] << 16) >> (7 & P) & (1 << Qn[R]) - 1);
...
function oi(e, t) {
	...
	return function(e, t) {
		if (1 == e.length) return e[0];
		for (var n = new Rn(t), i = 0, r = 0; i < e.length; ++i) {
			var s = e[i];
...
const ai = 4096, ci = 1024, li = Math.floor(160), hi = 12, di = 16, fi = 32, ui = 48, pi = 100, gi = -2147483648, mi = {
...
var _i = class extends Error {
	code;
	constructor(e, t) {
		super(t || mi[e] || `Error ${e}`), this.code = e, this.name = "SFSError";
	}
};
const yi = new TextEncoder(), wi = new TextDecoder();
function Si(e) {
	return e.buffer instanceof SharedArrayBuffer ? wi.decode(new Uint8Array(e)) : wi.decode(e);
}
function bi(e) {
	return e + 3 & -4;
}
var ki = class e {
	...
	static mkfs(t, n) {
		...
		if (i < 65536) throw new _i(-22);
		const r = Math.floor(i / ai), s = n ? Math.floor(n / ai) : 4 * r;
		let o = Math.floor(s / 4);
		o < 32 && (o = 32), o = 32 * Math.ceil(o / 32);
		const a = Math.ceil(o / 32768), c = Math.ceil(s / 32768), l = Math.ceil(128 * o / ai), h = 1 + a, d = h + c, f = d + l;
		if (f >= r) throw new _i(-28);
		new Uint8Array(t).fill(0);
		const u = new e(t);
		u.w32(0, 1397114451), u.w32(4, 1), u.w32(8, ai), u.w32(12, r), u.w32(16, o), u.w32(28, 1), u.w32(32, h), u.w32(36, d), u.w32(40, f), u.w32(44, a), u.w32(48, c), u.w32(52, l), u.w32(68, s), u.w32(72, 256);
		const p = h * ai;
		for (let e = 0; e < f; e++) {
			const t = (p >> 2) + (e >> 5);
			u.i32[t] |= 1 << (31 & e);
		...
		u.w32(m + 8, 16877), u.w32(m + hi, 2);
		const _ = u.blockAlloc();
		if (_ < 0) throw new _i(-28);
		u.w32(m + ui, _);
		const y = _ * ai, w = bi(9), S = bi(10);
		u.w32(y, 1), u.view.setUint16(y + 4, w, !0), u.view.setUint16(y + 6, 1, !0), u.u8[y + 8] = 46;
		const b = y + w;
		return u.w32(b, 1), u.view.setUint16(b + 4, S, !0), u.view.setUint16(b + 6, 2, !0), u.u8[b + 8] = 46, u.u8[b + 8 + 1] = 46, u.w64(m + di, w + S), Atomics.store(u.i32, 14, 1), u;
	}
	static mount(t) {
		const n = new e(t);
		if (1397114451 !== n.r32(0)) throw new _i(-22, "Bad magic");
		if (1 !== n.r32(4)) throw new _i(-22, "Bad version");
		if (4096 !== n.r32(8)) throw new _i(-22, "Bad block size");
		return n;
	}
	statfs() {
	...
	blockAlloc() {
		const e = this.r32(12), t = this.r32(32) * ai, n = Math.ceil(e / 32);
		for (let i = 0; i < n; i++) {
			...
			if (-1 !== r) for (let t = 0; t < 32; t++) {
				...
				if (Atomics.compareExchange(this.i32, n, r, o) === r) {
					...
					const e = s * ai;
					return this.u8.fill(0, e, e + ai), s;
				}
				i--;
				break;
			}
	...
	blockFree(e) {
		const t = (this.r32(32) * ai >> 2) + (e >> 5), n = 31 & e;
		for (;;) {
	...
	inodeOffset(e) {
		return (this.r32(36) + Math.floor(e / 32)) * ai + e % 32 * 128;
	}
	inodeAlloc() {
		const e = this.r32(16), t = this.r32(28) * ai, n = Math.ceil(e / 32);
		for (let i = 0; i < n; i++) {
			const n = (t >> 2) + i, r = Atomics.load(this.i32, n);
			if (-1 !== r) for (let t = 0; t < 32; t++) {
				const s = 32 * i + t;
	...
	inodeFree(e) {
		const t = (this.r32(28) * ai >> 2) + (e >> 5), n = 31 & e;
		for (;;) {
			const e = Atomics.load(this.i32, t), i = e & ~(1 << n);
			if (Atomics.compareExchange(this.i32, t, e, i) === e) break;
		}
	...
	inodeReadLock(e) {
		...
		for (;;) {
			...
			if (e & gi) Atomics.wait(this.i32, t, e);
	...
	inodeWriteLock(e) {
		...
		for (;;) {
			const e = Atomics.load(this.i32, t);
			if (0 === e) {
				if (0 === Atomics.compareExchange(this.i32, t, 0, gi)) return;
	...
	inodeBlockMap(e, t, n) {
		...
		if (t < 10) {
			const e = this.r32(i + ui + 4 * t);
			if (0 !== e) return e;
			if (!n) return 0;
			const r = this.blockAllocWithGrow();
			return r < 0 || this.w32(i + ui + 4 * t, r), r;
		}
		if ((t -= 10) < 1024) {
			let e = this.r32(i + 88);
			if (0 === e) {
		...
		if ((t -= ci) < 1048576) {
			const e = Math.floor(t / ci), r = t % ci;
			let s = this.r32(i + 92);
			if (0 === s) {
				if (!n) return 0;
	...
	inodeReadData(e, t, n, i) {
		const r = this.inodeOffset(e), s = this.r64(r + di);
		if (t >= s) return 0;
		t + i > s && (i = s - t);
		let o = 0, a = 0;
		for (; i > 0;) {
			const r = Math.floor(t / ai), s = t % ai;
			let c = ai - s;
			c > i && (c = i);
	...
	inodeWriteData(e, t, n, i) {
		...
		let s = 0, o = 0;
		for (; i > 0;) {
			const r = Math.floor(t / ai), a = t % ai;
			let c = ai - a;
			c > i && (c = i);
		...
		return t > this.r64(r + di) && this.w64(r + di, t), s;
	}
	freeBlocksFrom(e, t) {
		const n = this.inodeOffset(e);
		for (let s = t; s < 10; s++) {
			const e = this.r32(n + ui + 4 * s);
			e && (this.blockFree(e), this.w32(n + ui + 4 * s, 0));
		}
		const i = this.r32(n + 88);
		if (i) {
			const e = t > 10 ? t - 10 : 0;
			for (let t = e; t < ci; t++) {
				const e = i * ai + 4 * t, n = this.r32(e);
		...
		if (r) {
			const e = t > 1034 ? t - 10 - ci : 0, i = Math.floor(e / ci);
			for (let t = i; t < ci; t++) {
				const n = r * ai + 4 * t, s = this.r32(n);
				if (!s) continue;
				const o = t === i ? e % ci : 0;
				for (let e = o; e < ci; e++) {
					const t = s * ai + 4 * e, n = this.r32(t);
	...
	inodeTruncate(e, t) {
		const n = this.inodeOffset(e);
		if (t >= this.r64(n + di)) return void this.w64(n + di, t);
		const i = Math.ceil(t / ai);
		this.freeBlocksFrom(e, i), this.w64(n + di, t);
	}
	dirLookup(e, t) {
		const n = this.inodeOffset(e), i = this.r64(n + di);
		let r = 0;
		for (; r < i;) {
			const n = Math.floor(r / ai), s = r % ai, o = this.inodeBlockMap(e, n, !1);
			if (o <= 0) return -5;
			const a = o * ai;
			let c = i - r;
			c > 4096 - s && (c = ai - s);
	...
	dirAddEntry(e, t, n) {
		const i = this.inodeOffset(e), r = this.r64(i + di), s = bi(8 + t.length);
		let o = -1, a = 0;
		for (; a < r;) {
			const i = Math.floor(a / ai), c = a % ai, l = this.inodeBlockMap(e, i, !1);
			if (l <= 0) return -5;
			const h = l * ai;
			let d = r - a;
			d > 4096 - c && (d = ai - c);
		...
		let c, l = r, h = Math.floor(l / ai), d = l % ai;
		if (0 !== d && d + s > 4096) {
			const t = ai - d;
			if (t >= 8) {
		...
		const f = c * ai + d;
		return this.w32(f, n), this.view.setUint16(f + 4, s, !0), this.view.setUint16(f + 6, t.length, !0), this.u8.set(t, f + 8), this.w64(i + di, l + s), 0;
	}
	dirRemoveEntry(e, t) {
		const n = this.inodeOffset(e), i = this.r64(n + di);
		let r = 0;
	...
	dirIsEmpty(e) {
		const t = this.inodeOffset(e), n = this.r64(t + di);
		let i = 0;
		for (; i < n;) {
			const t = Math.floor(i / ai), r = i % ai, s = this.inodeBlockMap(e, t, !1);
			if (s <= 0) return !0;
			const o = s * ai;
			let a = n - i;
			a > 4096 - r && (a = ai - r);
	...
	pathResolve(e, t) {
		...
		for (let s = 0; s < i.length; s++) {
			...
			const a = yi.encode(e), c = this.dirLookup(n, a);
			if (c < 0) return c;
			const l = this.inodeOffset(c);
			if (40960 == (61440 & this.r32(l + 8)) && (s !== i.length - 1 || t)) {
				if (++r > 8) return -40;
				const e = this.r64(l + di);
				let t;
				if (e <= 40) t = Si(this.u8.subarray(l + ui, l + ui + e));
				else {
					const n = new Uint8Array(e);
					this.inodeReadData(c, 0, n, e), t = wi.decode(n);
				}
				if (t.startsWith("/")) {
					n = 1;
	...
	pathResolveParent(e) {
		if (!e.startsWith("/")) throw new _i(-22, "Path must be absolute");
		const t = e.split("/").filter((e) => e.length > 0);
		if (0 === t.length) throw new _i(-22, "Cannot operate on /");
		const n = t.pop();
		if (n.length > 255) throw new _i(-36);
		const i = "/" + t.join("/"), r = this.pathResolve(i, !0);
		if (r < 0) throw new _i(r);
		const s = this.inodeOffset(r);
		if (16384 != (61440 & this.r32(s + 8))) throw new _i(-20);
		return {
			parentIno: r,
			name: n
		};
	...
	open(e, t, n = 420) {
		...
		if (s < 0 && -2 === s && r) {
			...
			try {
				const e = yi.encode(i), r = this.dirLookup(t, e);
				if (r >= 0) s = r;
				else {
					const i = this.inodeAlloc();
					if (i < 0) throw new _i(-28);
					const r = this.inodeOffset(i);
					this.w32(r + 8, 32768 | 4095 & n), this.w32(r + hi, 1), this.w64(r + di, 0);
					const o = Date.now();
					this.w64(r + 40, o), this.w64(r + 24, o), this.w64(r + fi, o);
					const a = this.dirAddEntry(t, e, i);
					if (a < 0) throw this.inodeFree(i), new _i(a);
					s = i;
				}
			} finally {
		...
		if (s < 0) throw new _i(s);
		const o = this.inodeOffset(s), a = this.r32(o + 8);
		if (16384 == (61440 & a) && 0 !== i) throw new _i(-21);
		if (65536 & t && 16384 != (61440 & a)) throw new _i(-20);
		if (512 & t) {
			if (16384 == (61440 & a)) throw new _i(-21);
			this.inodeWriteLock(s), this.inodeTruncate(s, 0), this.inodeWriteUnlock(s);
		}
		const c = this.fdAlloc(s, t, !1);
		if (c < 0) throw new _i(c);
		if (1024 & t) {
			const e = 256 + 24 * c;
			this.w64(e + 8, this.r64(o + di));
		}
		return c;
	}
	close(e) {
		if (!this.fdGet(e)) throw new _i(-9);
		this.fdFree(e);
	}
	read(e, t) {
		...
		try {
			const i = this.inodeReadData(n.ino, n.offset, t, t.length), r = 256 + 24 * e;
	...
	write(e, t) {
		...
		try {
			...
			const r = this.inodeWriteData(n.ino, i, t, t.length), s = 256 + 24 * e;
	...
	lseek(e, t, n) {
		...
		if (0 === n) r = t;
			...
			if (2 !== n) throw new _i(-22);
			{
				const e = this.inodeOffset(i.ino);
				r = this.r64(e + di) + t;
			}
		}
		if (r < 0) throw new _i(-22);
		const s = 256 + 24 * e;
	...
	unlink(e) {
		const { parentIno: t, name: n } = this.pathResolveParent(e), i = yi.encode(n);
		this.inodeWriteLock(t);
		try {
			const e = this.dirLookup(t, i);
			if (e < 0) throw new _i(e);
			const n = this.inodeOffset(e);
			if (16384 == (61440 & this.r32(n + 8))) throw new _i(-21);
			const r = this.dirRemoveEntry(t, i);
			if (r < 0) throw new _i(r);
			this.inodeWriteLock(e);
			const s = this.r32(n + hi);
			s <= 1 ? (this.inodeTruncate(e, 0), this.w32(n + hi, 0), this.inodeWriteUnlock(e), this.inodeFree(e)) : (this.w32(n + hi, s - 1), this.inodeWriteUnlock(e));
		} finally {
			this.inodeWriteUnlock(t);
		}
	}
	rename(e, t) {
		const { parentIno: n, name: i } = this.pathResolveParent(e), { parentIno: r, name: s } = this.pathResolveParent(t), o = yi.encode(i), a = yi.encode(s), c = Math.min(n, r), l = Math.max(n, r);
		...
		try {
			...
			if (t >= 0) {
				...
				if (16384 == (61440 & this.r32(e + 8))) throw new _i(-21);
				this.dirRemoveEntry(r, a), this.inodeWriteLock(t), this.inodeTruncate(t, 0), this.w32(e + hi, 0), this.inodeWriteUnlock(t), this.inodeFree(t);
			}
			const i = this.dirAddEntry(r, a, e);
			if (i < 0) throw new _i(i);
			this.dirRemoveEntry(n, o);
			const s = this.inodeOffset(e);
			if (16384 == (61440 & this.r32(s + 8)) && n !== r) {
				const e = this.inodeOffset(n);
				this.w32(e + hi, this.r32(e + hi) - 1);
				const t = this.inodeOffset(r);
				this.w32(t + hi, this.r32(t + hi) + 1);
			}
		} finally {
	...
	mkdir(e, t = 493) {
		const { parentIno: n, name: i } = this.pathResolveParent(e), r = yi.encode(i);
		this.inodeWriteLock(n);
		try {
			if (this.dirLookup(n, r) >= 0) throw new _i(-17);
			const e = this.inodeAlloc();
			if (e < 0) throw new _i(-28);
			const i = this.inodeOffset(e);
			this.w32(i + 8, 16384 | t), this.w32(i + hi, 2), this.w64(i + di, 0);
			const s = Date.now();
			this.w64(i + 40, s), this.w64(i + 24, s), this.w64(i + fi, s);
			const o = this.blockAllocWithGrow();
			if (o < 0) throw this.inodeFree(e), new _i(-28);
			this.w32(i + ui, o);
			const a = o * ai, c = bi(9), l = bi(10);
			this.w32(a, e), this.view.setUint16(a + 4, c, !0), this.view.setUint16(a + 6, 1, !0), this.u8[a + 8] = 46;
			const h = a + c;
			this.w32(h, n), this.view.setUint16(h + 4, l, !0), this.view.setUint16(h + 6, 2, !0), this.u8[h + 8] = 46, this.u8[h + 8 + 1] = 46, this.w64(i + di, c + l);
			const d = this.dirAddEntry(n, r, e);
			if (d < 0) throw this.blockFree(o), this.inodeFree(e), new _i(d);
			const f = this.inodeOffset(n);
			this.w32(f + hi, this.r32(f + hi) + 1);
		} finally {
	...
	rmdir(e) {
		...
		try {
			...
			if (e < 0) throw new _i(e);
			const n = this.inodeOffset(e);
			if (16384 != (61440 & this.r32(n + 8))) throw new _i(-20);
			this.inodeWriteLock(e);
			try {
				if (!this.dirIsEmpty(e)) throw new _i(-39);
				this.dirRemoveEntry(t, i), this.inodeTruncate(e, 0), this.w32(n + hi, 0);
			} finally {
				this.inodeWriteUnlock(e);
			}
			this.inodeFree(e);
			const r = this.inodeOffset(t);
			this.w32(r + hi, this.r32(r + hi) - 1);
		} finally {
			this.inodeWriteUnlock(t);
		}
	}
	symlink(e, t) {
		const { parentIno: n, name: i } = this.pathResolveParent(t), r = yi.encode(i), s = yi.encode(e);
		this.inodeWriteLock(n);
		try {
			if (this.dirLookup(n, r) >= 0) throw new _i(-17);
			const e = this.inodeAlloc();
			if (e < 0) throw new _i(-28);
			const t = this.inodeOffset(e);
			if (this.w32(t + 8, 41471), this.w32(t + hi, 1), s.length <= 40) this.u8.set(s, t + ui), this.w64(t + di, s.length);
			else {
				this.w64(t + di, 0);
				const n = this.inodeWriteData(e, 0, s, s.length);
				if (n < 0) throw this.inodeFree(e), new _i(n);
			}
			const i = this.dirAddEntry(n, r, e);
			if (i < 0) throw this.inodeFree(e), new _i(i);
		} finally {
			this.inodeWriteUnlock(n);
		}
	}
	chmod(e, t) {
		...
		if (n < 0) throw new _i(n);
		this.inodeWriteLock(n);
		try {
			const e = this.inodeOffset(n), i = this.r32(e + 8);
			this.w32(e + 8, 61440 & i | 4095 & t), this.w64(e + fi, Date.now());
		} finally {
			this.inodeWriteUnlock(n);
		}
	}
	...
	chown(e, t, n) {
		...
		try {
			...
			this.w32(e + 96, t), this.w32(e + pi, n), this.w64(e + fi, Date.now());
		} finally {
	...
	link(e, t) {
		...
		if (16384 == (61440 & this.r32(i + 8))) throw new _i(-1);
		const { parentIno: r, name: s } = this.pathResolveParent(t), o = yi.encode(s);
		this.inodeWriteLock(r);
		try {
			if (this.dirLookup(r, o) >= 0) throw new _i(-17);
			const e = this.dirAddEntry(r, o, n);
			if (e < 0) throw new _i(e);
			this.inodeWriteLock(n);
			try {
				const e = this.r32(i + hi);
				this.w32(i + hi, e + 1), this.w64(i + fi, Date.now());
			} finally {
				this.inodeWriteUnlock(n);
			}
		} finally {
	...
	readlink(e) {
		...
		if (40960 != (61440 & this.r32(n + 8))) throw new _i(-22);
		const i = this.r64(n + di);
		if (i <= 40) return Si(this.u8.subarray(n + ui, n + ui + i));
		this.inodeReadLock(t);
		try {
			const e = new Uint8Array(i);
			return this.inodeReadData(t, 0, e, i), wi.decode(e);
		} finally {
			this.inodeReadUnlock(t);
		}
	}
	...
	readdirEntry(e) {
		const t = this.fdGet(e);
		if (!t || !t.isDir) throw new _i(-9);
		const n = this.inodeOffset(t.ino), i = this.r64(n + di);
		for (; t.offset < i;) {
			const n = t.offset, i = Math.floor(n / ai), r = n % ai, s = this.inodeBlockMap(t.ino, i, !1);
			if (s <= 0) return null;
			const o = s * ai + r, a = this.r32(o), c = this.view.getUint16(o + 4, !0), l = this.view.getUint16(o + 6, !0);
			if (0 === c) return null;
			t.offset = n + c;
			const h = 256 + 24 * e;
			if (this.w64(h + 8, n + c), 0 !== a) return {
				name: Si(this.u8.subarray(o + 8, o + 8 + l)),
	...
	writeFile(e, t) {
		const n = "string" == typeof t ? yi.encode(t) : t, i = this.open(e, 577);
		try {
			this.write(i, n);
		} finally {
			this.close(i);
	...
	readFileText(e) {
		return wi.decode(this.readFile(e));
	}
};
const Ai = [
	40,
...
function xi(e) {
	return e.byteLength >= Ai.length && e[0] === Ai[0] && e[1] === Ai[1] && e[2] === Ai[2] && e[3] === Ai[3] ? function(e) {
		return oi(e);
	}(e) : e;
}
function Ti(e) {
	const t = xi(e);
	if (t.byteLength < Ci) throw new Error("VFS image too small");
	const n = new DataView(t.buffer, t.byteOffset, t.byteLength), i = n.getUint32(0, !0);
	if (i !== Ii) throw new Error(`Bad VFS image magic: 0x${i.toString(16)} (expected 0x${Ii.toString(16)})`);
	...
	if (t.byteLength < Ci + o + 4) throw new Error("VFS image truncated");
	return {
		image: t,
		view: n,
		flags: s,
...
var Pi = class e {
	...
	rebaseToNewFileSystem(t) {
		...
		const n = Math.min(t, Math.max(this.sharedBuffer.byteLength, 16777216)), i = new SharedArrayBuffer(n, { maxByteLength: t }), r = e.create(i, t);
		r.setImageMetadata(this.imageMetadata);
		...
		return this.copyPathToFreshFileSystem("/", r, o, c), r.importLazyEntries(s.map((e) => ({
			...e,
			ino: r.lstat(e.path).ino
		}))), r.importLazyArchiveEntries(a.map((e) => ({
			...e,
			entries: e.entries.map((e) => ({
				...e,
				ino: e.deleted ? 0 : r.lstat(e.vfsPath).ino
			}))
		}))), r;
	}
	getImageMetadata() {
	...
	setImageMetadata(e) {
		this.imageMetadata = null === e ? null : vi(e);
	}
	...
	async saveImage(e) {
		...
		const t = new Uint8Array(this.fs.buffer), n = this.exportLazyEntries(), i = n.length > 0, r = i ? new TextEncoder().encode(JSON.stringify(n)) : new Uint8Array(0), s = this.exportLazyArchiveEntries(), o = s.length > 0, a = o ? new TextEncoder().encode(JSON.stringify(s)) : new Uint8Array(0), c = function(e) {
			...
			const t = vi(e), n = new TextEncoder().encode(JSON.stringify(t));
			if (n.byteLength > Ei) throw new Error("VFS image metadata exceeds 65536 bytes");
			return n;
		}(void 0 === e?.metadata ? this.imageMetadata : e.metadata), l = c.byteLength > 0, h = o ? 4 + a.byteLength : 0, d = l ? 4 + c.byteLength : 0, f = Ci + t.byteLength + 4 + r.byteLength + h + d, u = new Uint8Array(f), p = new DataView(u.buffer);
		p.setUint32(0, Ii, !0), p.setUint32(4, 1, !0), p.setUint32(8, (i ? 1 : 0) | (o ? 2 : 0) | (l ? 4 : 0), !0), p.setUint32(12, t.byteLength, !0);
		const g = new Uint8Array(t.byteLength);
		g.set(t), u.set(g, Ci);
		const m = Ci + t.byteLength;
	...
	static readImageMetadata(e) {
		...
		const { metadataOffset: n } = function(e, t, n, i) {
			const r = Ci + i, s = t.getUint32(r, !0), o = r + 4 + s;
			let a = o;
			if (2 & n) {
		...
		if (i > Ei) throw new Error("VFS image metadata exceeds 65536 bytes");
		if (t.image.byteLength < n + 4 + i) throw new Error("VFS image truncated (metadata payload)");
		return 0 === i ? null : function(e) {
			if (e.byteLength > Ei) throw new Error("VFS image metadata exceeds 65536 bytes");
			let t;
			try {
	...
	static fromImage(t, n) {
		...
		new Uint8Array(c).set(t.subarray(Ci, Ci + o));
		let l = null;
		4 & s && (l = e.readImageMetadata(t));
		const h = new e(ki.mount(c), l), d = Ci + o, f = r.getUint32(d, !0);
		if (1 & s && f > 0) {
	...
	copyPathToFreshFileSystem(t, n, i, r) {
		...
		if (16384 === o) {
			...
			try {
				for (;;) {
					...
					if (!e) break;
					"." !== e.name && ".." !== e.name && this.copyPathToFreshFileSystem("/" === t ? `/${e.name}` : `${t}/${e.name}`, n, i, r);
				}
			} finally {
				this.closedir(o);
			}
		...
		if (40960 !== o) {
			...
			if (i.has(t) || r.has(t)) return n.createFileWithOwner(t, a, s.uid, s.gid, new Uint8Array(0)), void e.applyTimes(n, t, s);
			this.copyRegularFileToFreshFileSystem(t, n, s, a);
		} else n.symlinkWithOwner(this.readlink(t), t, s.uid, s.gid);
	}
	copyRegularFileToFreshFileSystem(t, n, i, r) {
		...
		try {
			...
			let a = i.size;
			for (; a > 0;) {
				const i = Math.min(e.byteLength, a), r = this.read(s, e, null, i);
				if (r <= 0) throw new Error(`Unexpected EOF while rebasing VFS file: ${t}`);
				let c = 0;
				for (; c < r;) {
					const i = n.write(o, e.subarray(c, r), null, r - c);
		...
		n.chown(t, i.uid, i.gid), n.chmod(t, r), e.applyTimes(n, t, i);
	}
	static applyTimes(e, t, n) {
		const i = Math.floor(n.atimeMs / 1e3), r = Math.floor(1e6 * (n.atimeMs - 1e3 * i)), s = Math.floor(n.mtimeMs / 1e3), o = Math.floor(1e6 * (n.mtimeMs - 1e3 * s));
		e.utimensat(t, i, r, s, o);
	}
	access(e, t) {
		this.fs.stat(e);
	}
...
var Wi = class {
	...
	constructor() {
		...
		this.devices.set("null", Bi), this.devices.set("zero", Hi), this.devices.set("urandom", e), this.devices.set("random", e), this.devices.set("console", Ui), this.devices.set("tty", Ui), this.deviceNames = [...this.devices.keys()];
	}
	getDevice(e) {
	...
	statfs(e) {
		...
		return t.files = this.devices.size + Mi.length + Li.length, t;
	}
	mkdir(e, t) {
	...
	opendir(e) {
		...
		let n;
		if (Ri(e)) n = [...this.deviceNames.map((e, t) => ({
			name: e,
			type: 2,
			ino: t + 1
		})), ...Li.filter((e) => !this.devices.has(e.name))];
		else {
...
function Oi(e, t, n = {}) {
	...
	for (const r of e) if ("image" === r.source) {
		const e = Pi.fromImage(t, { maxByteLength: 1073741824 });
		zi(e), i.push({
			mountPoint: r.path,
			backend: e,
			readonly: r.readonly
		...
		const e = n.scratchSabBytes?.[r.path] ?? 16777216, t = new SharedArrayBuffer(e), s = Pi.create(t);
...
function Fi(e) {
	return $i(e.map((e) => ArrayBuffer.isView(e) ? new Uint8Array(e.buffer, e.byteOffset, e.byteLength) : new Uint8Array(e))).buffer;
}
const Vi = (...e) => console.warn(...e);
function qi(e) {
...
}, Qi = qi(Zi), er = { host_name: 0 }, tr = qi(er);
var nr = class {
	static decodeFromClient(e) {
		...
		for (; n < i + 2;) {
			...
			if (n += s, i !== er.host_name) throw new Error(`Unsupported name type ${i}`);
			r.push({
				name_type: tr[i],
	...
	static encodeForClient(e) {
		...
		const t = new Ji(4);
		return t.writeUint16(Zi.server_name), t.writeUint16(0), t.uint8Array;
	}
};
const ir = {
	uncompressed: 0,
...
var sr = class {
	...
	static encodeForClient(e) {
		const t = new Ji(6);
		return t.writeUint16(Zi.ec_point_formats), t.writeUint16(2), t.writeUint8(1), t.writeUint8(ir[e]), t.uint8Array;
	}
};
const or = {
	decodeFromClient: (e) => ({}),
	encodeForClient() {
		const e = Zi.extended_master_secret;
		return new Uint8Array([
			e >> 8 & 255,
			255 & e,
			0,
	...
	encodeForClient() {
		const e = Zi.renegotiation_info, t = new Uint8Array([0]);
		return new Uint8Array([
			e >> 8 & 255,
			255 & e,
			0,
...
const mr = {
		...
		static decodeFromClient(e) {
			...
			for (; !t.isFinished();) {
				...
				ur[i] && (gr[e] ? n.push({
					algorithm: ur[i],
					hash: gr[e]
				}) : Vi(`Unknown hash algorithm: ${e}`));
			}
			return n;
		}
		static encodeforClient(e, t) {
			const n = new Ji(6);
			return n.writeUint16(Zi.signature_algorithms), n.writeUint16(2), n.writeUint8(pr[e]), n.writeUint8(fr[t]), n.uint8Array;
		}
	},
	supported_groups: class {
		...
		static encodeForClient(e) {
			const t = new Ji(6);
			return t.writeUint16(Zi.supported_groups), t.writeUint16(2), t.writeUint16(hr[e]), t.uint8Array;
		}
	},
	ec_point_formats: sr,
	renegotiation_info: ar,
	extended_master_secret: or
};
async function _r(e, t, n, i) {
	const r = Fi([t, n]), s = await crypto.subtle.importKey("raw", e, {
		name: "HMAC",
		hash: { name: "SHA-256" }
	}, !1, ["sign"]);
	let o = r;
	const a = [];
	for (; Fi(a).byteLength < i;) {
		o = await yr(s, o);
		const e = await yr(s, Fi([o, r]));
		a.push(e);
	}
	return Fi(a).slice(0, i);
}
...
}, Ar = qi(kr), Ir = 20, Cr = 21, Er = 22, vr = 23, xr = 0, Tr = 1, Pr = 2, Br = 11, Hr = 12, Ur = 14, Mr = 16, Lr = 20, Rr = 3, Wr = 23;
var Dr = class extends Error {};
const Kr = new Uint8Array([3, 3]), zr = crypto.subtle.generateKey({
	name: "ECDH",
	namedCurve: "P-256"
...
var Or = class {
	...
	serverEnd = {
		upstream: new TransformStream(),
		downstream: $r(this.MAX_CHUNK_SIZE)
	};
	...
	async TLSHandshake(e, t) {
		const n = await this.readNextHandshakeMessage(Tr);
		...
		await this.writeTLSRecord(Er, Fr.serverHello(n.body, i, wr)), await this.writeTLSRecord(Er, Fr.certificate(t));
		const r = await zr, s = n.body.random, o = await Fr.ECDHEServerKeyExchange(s, i, r, e);
		await this.writeTLSRecord(Er, o), await this.writeTLSRecord(Er, Fr.serverHelloDone());
		const a = n.body.extensions.some((e) => "extended_master_secret" === e.type), c = await this.readNextHandshakeMessage(Mr);
		await this.readNextMessage(Ir), this.sessionKeys = await this.deriveSessionKeys({
			clientRandom: s,
			serverRandom: i,
			...
			useEMS: a
		}), await this.readNextHandshakeMessage(Lr), await this.writeTLSRecord(Ir, Fr.changeCipherSpec()), await this.writeTLSRecord(Er, await Fr.createFinishedMessage(this.handshakeMessages, this.sessionKeys.masterSecret)), this.handshakeMessages = [], this.pollForClientMessages();
	}
	async deriveSessionKeys({ clientRandom: e, serverRandom: t, serverPrivateKey: n, clientPublicKey: i, useEMS: r = !1 }) {
		...
		let o;
		if (r) {
			const e = new Uint8Array(await crypto.subtle.digest("SHA-256", $i(this.handshakeMessages)));
			o = new Uint8Array(await _r(s, new TextEncoder().encode("extended master secret"), e, 48));
		} else o = new Uint8Array(await _r(s, new TextEncoder().encode("master secret"), $i([e, t]), 48));
		const a = new Yi(await _r(o, new TextEncoder().encode("key expansion"), $i([t, e]), 40)), c = a.readUint8Array(16), l = a.readUint8Array(16), h = a.readUint8Array(4), d = a.readUint8Array(4);
		return {
			masterSecret: o,
	...
	async readNextHandshakeMessage(e) {
		const t = await this.readNextMessage(Er);
	...
	async readNextMessage(e) {
		...
		while (!1 === n);
		const i = Nr.TLSMessage(t.type, n);
		return t.type === Er && this.handshakeMessages.push(t.fragment), i;
	}
	async readNextTLSRecord(e) {
		for (;;) {
			...
			const t = await this.pollBytes(5), n = t[3] << 8 | t[4], i = t[0], r = await this.pollBytes(n), s = {
				...
				length: n,
				fragment: this.sessionKeys && i !== Ir ? await this.decryptData(i, r) : r
			};
			if (s.type === Cr) {
				const e = s.fragment[0], t = s.fragment[1], n = br[e], i = Ar[t];
				if (e === Sr.Warning && t === kr.CloseNotify) throw new Dr("TLS connection closed by peer (CloseNotify)");
	...
	async pollBytes(e) {
		for (; this.receivedBytesBuffer.length < e;) {
			...
			if (n) throw await this.close(), new Dr("TLS connection closed");
			if (this.receivedBytesBuffer = $i([this.receivedBytesBuffer, t]), this.receivedBytesBuffer.length >= e) break;
	...
	async pollForClientMessages() {
		try {
			for (;;) {
				const e = await this.readNextMessage(vr);
	...
	async decryptData(e, t) {
		const n = this.sessionKeys.clientIV, i = t.slice(0, 8), r = new Uint8Array([...n, ...i]), s = await crypto.subtle.decrypt({
			...
			additionalData: new Uint8Array([
				...Xi(this.receivedRecordSequenceNumber),
				e,
				...Kr,
				...Gi(t.length - 8 - 16)
			]),
	...
	async accumulateUntilMessageIsComplete(e) {
		this.partialTLSMessages[e.type] = $i([this.partialTLSMessages[e.type] || new Uint8Array(), e.fragment]);
	...
	async writeTLSRecord(e, t) {
		e === Er && this.handshakeMessages.push(t), this.sessionKeys && e !== Ir && (t = await this.encryptData(e, t));
		const n = Kr, i = t.length, r = new Uint8Array(5);
	...
	async encryptData(e, t) {
		const n = this.sessionKeys.serverIV, i = crypto.getRandomValues(new Uint8Array(8)), r = new Uint8Array([...n, ...i]), s = new Uint8Array([
			...Xi(this.sentRecordSequenceNumber),
			e,
			...Kr,
			...Gi(t.length)
		...
		return ++this.sentRecordSequenceNumber, $i([i, new Uint8Array(o)]);
	}
}, Nr = class e {
	static TLSMessage(t, n) {
		switch (t) {
			case Er: return e.clientHandshake(n);
			case Cr: return e.alert(n);
			case Ir: return e.changeCipherSpec();
			case vr: return e.applicationData(n);
	...
	static clientHelloPayload(t) {
		...
		return i.extensions = function(e) {
			...
			for (; !t.isFinished();) {
				const i = t.offset, r = Qi[t.readUint16()], s = t.readUint16(), o = t.readUint8Array(s);
				if (!(r in mr)) continue;
				const a = mr[r];
				n.push({
					type: r,
...
var Fr = class {
	static certificate(e) {
		const t = [];
		for (const r of e) t.push(ji(r.byteLength)), t.push(new Uint8Array(ArrayBuffer.isView(r) ? r.buffer : r));
		const n = $i(t), i = new Uint8Array([...ji(n.byteLength), ...n]);
		return new Uint8Array([
			Br,
			...ji(i.length),
			...i
	...
	static async ECDHEServerKeyExchange(e, t, n, i) {
		const r = new Uint8Array(await crypto.subtle.exportKey("raw", n.publicKey)), s = new Uint8Array([
			...
			...t,
			...s
		])), a = new Uint8Array(o), c = new Uint8Array([pr.sha256, fr.rsa]), l = new Uint8Array([
			...s,
			...c,
			...Gi(a.length),
			...a
	...
	static serverHello(e, t, n) {
		const i = e.extensions.map((e) => {
			switch (e.type) {
				case "server_name": return nr.encodeForClient();
				case "ec_point_formats": return sr.encodeForClient("uncompressed");
				case "renegotiation_info": return ar.encodeForClient();
				case "extended_master_secret": return or.encodeForClient();
			}
		}).filter((e) => void 0 !== e);
		i.length > 0 && e.extensions.some((e) => "renegotiation_info" === e.type) || i.push(ar.encodeForClient());
		const r = $i(i), s = new Uint8Array([
			...Kr,
			...t,
			e.session_id.length,
			...e.session_id,
			...Gi(cr.TLS1_CK_ECDHE_RSA_WITH_AES_128_GCM_SHA256),
			n,
			...Gi(r.length),
			...r
		]);
	...
	static serverHelloDone() {
		return new Uint8Array([Ur, ...ji(0)]);
	}
	static async createFinishedMessage(e, t) {
		const n = await crypto.subtle.digest("SHA-256", $i(e)), i = new Uint8Array(await _r(t, new TextEncoder().encode("server finished"), n, 12));
		return new Uint8Array([
			Lr,
			...ji(i.length),
			...i
...
function Vr(e, t) {
	return Gr.generateCertificate(e, t);
}
function qr(e) {
...
var Gr = class {
	...
	static async sign(e, t) {
		...
		return Jr.sequence([
			...
			Jr.bitString(new Uint8Array(n))
		]);
	}
	static async signingRequest(e, t) {
		const n = [];
		return e.keyUsage && n.push(this.keyUsage(e.keyUsage)), e.extKeyUsage && n.push(this.extKeyUsage(e.extKeyUsage)), e.subjectAltNames && n.push(this.subjectAltName(e.subjectAltNames)), e.nsCertType && n.push(this.nsCertType(e.nsCertType)), e.basicConstraints && n.push(this.basicConstraints(e.basicConstraints)), Jr.sequence([
	...
	static version(e = 2) {
		return Jr.ASN1(160, Jr.integer(new Uint8Array([e])));
	}
	static serialNumber(e = crypto.getRandomValues(new Uint8Array(4))) {
		return Jr.integer(e);
	}
	static signatureAlgorithm(e = "sha256WithRSAEncryption") {
		return Jr.sequence([Jr.objectIdentifier(Xr(e)), Jr.null()]);
	}
	static async subjectPublicKeyInfo(e) {
		return new Uint8Array(await crypto.subtle.exportKey("spki", e));
	}
	static extensions(e) {
		return Jr.ASN1(163, Jr.sequence(e));
	}
	static distinguishedName(e) {
		const t = [];
		for (const [n, i] of Object.entries(e)) {
			const e = [Jr.objectIdentifier(Xr(n))];
			if ("countryName" === n) e.push(Jr.printableString(i));
			else e.push(Jr.utf8String(i));
			t.push(Jr.set([Jr.sequence(e)]));
		}
		return Jr.sequence(t);
	}
	static validity(e) {
		return Jr.sequence([Jr.ASN1(Yr.UTCTime, new TextEncoder().encode(Zr(e?.notBefore ?? /* @__PURE__ */ new Date()))), Jr.ASN1(Yr.UTCTime, new TextEncoder().encode(Zr(e?.notAfter ?? es(/* @__PURE__ */ new Date(), 10))))]);
	}
	static basicConstraints({ ca: e = !0, pathLenConstraint: t }) {
		const n = [Jr.boolean(e)];
		return void 0 !== t && n.push(Jr.integer(new Uint8Array([t]))), Jr.sequence([Jr.objectIdentifier(Xr("basicConstraints")), Jr.octetString(Jr.sequence(n))]);
	}
	static keyUsage(e) {
		const t = new Uint8Array([0]);
		return e?.digitalSignature && (t[0] |= 128), e?.nonRepudiation && (t[0] |= 64), e?.keyEncipherment && (t[0] |= 32), e?.dataEncipherment && (t[0] |= 16), e?.keyAgreement && (t[0] |= 8), e?.keyCertSign && (t[0] |= 4), e?.cRLSign && (t[0] |= 2), e?.encipherOnly && (t[0] |= 1), Jr.sequence([
			Jr.objectIdentifier(Xr("keyUsage")),
			Jr.boolean(!0),
			Jr.octetString(Jr.bitString(t))
		]);
	}
	static extKeyUsage(e = {}) {
		return Jr.sequence([
			Jr.objectIdentifier(Xr("extKeyUsage")),
			Jr.boolean(!0),
			Jr.octetString(Jr.sequence(Object.entries(e).map(([e, t]) => t ? Jr.objectIdentifier(Xr(e)) : Jr.null())))
		]);
	}
	static nsCertType(e) {
		const t = new Uint8Array([0]);
		return e.client && (t[0] |= 1), e.server && (t[0] |= 2), e.email && (t[0] |= 4), e.objsign && (t[0] |= 8), e.sslCA && (t[0] |= 16), e.emailCA && (t[0] |= 32), e.objCA && (t[0] |= 64), Jr.sequence([Jr.objectIdentifier(Xr("nsCertType")), Jr.octetString(t)]);
	}
	static subjectAltName(e) {
		const t = e.dnsNames?.map((e) => {
			const t = new TextEncoder().encode(e);
			return Jr.contextSpecific(2, t);
		}) || [], n = e.ipAddresses?.map((e) => {
			const t = new TextEncoder().encode(e);
			return Jr.contextSpecific(7, t);
		}) || [], i = Jr.octetString(Jr.sequence([...t, ...n]));
		return Jr.sequence([
			Jr.objectIdentifier(Xr("subjectAltName")),
			Jr.boolean(!0),
			i
		]);
	}
...
function Xr(e) {
	for (const [t, n] of Object.entries(jr)) if (n === e) return t;
...
var Jr = class e {
	...
	static sequence(t) {
		return e.ASN1(Yr.Sequence, $i(t));
	}
	static set(t) {
		return e.ASN1(Yr.Set, $i(t));
	}
	static ia5String(t) {
		const n = new TextEncoder().encode(t);
...
function Zr(e) {
	return `${e.getUTCFullYear().toString().substr(2)}${Qr(e.getUTCMonth() + 1)}${Qr(e.getUTCDate())}${Qr(e.getUTCHours())}${Qr(e.getUTCMinutes())}${Qr(e.getUTCSeconds())}Z`;
}
function Qr(e) {
...
var cs = class {
	...
	async init() {
		this.initialized || (this.caCert = await Vr({
			subject: {
		...
		}), this.caKeyPair = this.caCert.keyPair, this.caCertPEM = qr(this.caCert.certificate), this.initialized = !0);
	}
	getCACertPEM() {
	...
	tryProcessHttpRequest(e) {
		...
		if (-1 === t) return;
		const n = is(new TextDecoder().decode(e.plaintextBuf.subarray(0, t))), i = t + 4, r = e.plaintextBuf.length - i;
		if (n > 0 && r < n) return;
		...
		(async () => {
			try {
				...
				const n = `Error fetching ${d}: ${Fn}`, i = os(502, "Bad Gateway", new Headers({ "Content-Type": "text/plain" }), new TextEncoder().encode(n).buffer);
				try {
	...
	httpSend(e, t) {
		...
		const i = ns(e.sendBuf);
		if (-1 === i) return t.length;
		const r = is(new TextDecoder().decode(e.sendBuf.subarray(0, i))), s = i + 4, o = e.sendBuf.length - s;
		if (r > 0 && o < r) return t.length;
		const { method: a, path: c, headers: l, body: h } = rs(e.sendBuf, i), d = l.get("host"), f = 443 === e.port ? "https" : "http", u = 80 === e.port || 443 === e.port ? "" : `:${e.port}`, p = d || `${e.hostname}${u}`, g = this.dnsAliases[e.hostname], m = void 0 !== g ? `${g}${c}` : `${f}://${p}${c}`, _ = this.corsProxyUrl ? as(this.corsProxyUrl, m) : m, y = "https://registry.npmjs.org" === g, w = new Headers();
		for (const [b, k] of l) {
...
var us = class {
	...
	allocate(e) {
		...
		return yt(e, (t + 4) * We, this.ptrWidth), new Uint8Array(e.buffer, r, m).fill(0), new Uint8Array(e.buffer, n, We).fill(0), new Uint8Array(e.buffer, i, We).fill(0), new Uint8Array(e.buffer, i, ht).fill(0), this.activeCount++, {
			slotStartPage: t,
			basePage: t,
			tlsOffset: n,
			forkSaveOffset: i,
...
if (void 0 === globalThis.setImmediate) {
	const co = [];
	let lo = 0, ho = !1, fo = !1;
	const uo = /* @__PURE__ */ new Set(), po = new MessageChannel();
	function go() {
		ho = !1, fo = !0;
		const e = co.length;
		for (let n = 0; n < e && co.length > 0; n++) {
			const e = co.shift();
			if (uo.has(e.id)) uo.delete(e.id);
			else try {
				e.fn(...e.args);
			} catch (t) {
		...
		fo = !1, co.length > 0 && !ho && (ho = !0, po.port2.postMessage(null));
	}
	po.port1.onmessage = go, globalThis.setImmediate = (e, ...t) => {
		const n = ++lo;
		return co.push({
			id: n,
			fn: e,
			args: t
		}), ho || fo || (ho = !0, po.port2.postMessage(null)), n;
...
function Ls(e, t, n) {
	...
	Us.add(e);
	const i = As.get(e), r = ms.dumpLastSyscalls(e) || "<none>", s = function(e) {
	...
	}(i?.argv), o = `[kernel-worker] nonzero process exit pid=${e} status=${t} source=${n} argv=${JSON.stringify(i?.argv ?? [])}` + (s ? `\n${s}` : "") + `\n${r}`;
	console.warn(o), qs({
		type: "stderr",
		pid: e,
...
let zs = null, Os = null, Ns = null, $s = null, Fs = 1;
...
function Zs(e, t, n, i = Ss, r) {
	...
	try {
		...
		throw console.error("[kernel-worker] process memory allocation failed", JSON.stringify(function(e, t, n, i, r) {
			let s = 0;
...
async function eo(e) {
	...
	const t = Pi.fromExisting(e.shmSab), n = new Wi();
	let i;
	if (e.vfsImage) {
		const r = Oi(Ki, e.vfsImage), s = r.find((e) => "/" === e.mountPoint);
		if (!s) throw new Error("DEFAULT_MOUNT_SPEC missing / mount");
		ys = s.backend, e.lazyUrlBase && (ys.rewriteLazyFileUrls((t) => Qs(e.lazyUrlBase, t)), ys.rewriteLazyArchiveUrls((t) => Qs(e.lazyUrlBase, t))), i = [
			{
				mountPoint: "/dev/shm",
				backend: t
			},
		...
		if (ys = Pi.fromExisting(e.fsSab), e.rootfsImage) try {
			(function(e, t) {
				const n = Pi.fromImage(t);
				try {
					e.mkdir("/etc", 493);
				} catch {}
				let i;
	...
	ws = new Mn(i, new Di());
	const r = new cs({ dnsAliases: e.config.dnsAliases });
	await r.init(), ws.network = r;
	...
	_s = new Hn(e.workerEntryUrl), ms = new Bn({
		...
		onFork: (e, t, n, i) => (qs({
			...
			const s = new Uint8Array(n.buffer), o = Math.ceil(s.byteLength / ps), a = r.ptrWidth, c = r.layout, l = function(e, t, n) {
				return 8 === e ? new WebAssembly.Memory({
					initial: BigInt(t),
					maximum: BigInt(n),
					shared: !0,
			...
			const d = i ? i.forkBufAddr : h - gs, f = {
		...
		onExec: async (e, t, n, i) => {
			const r = await async function(e, t, n, i) {
				...
				if (!r) return -2;
				if ("errno" in r) return -r.errno;
				const { programBytes: s, argv: o } = r, a = ms.kernelExecSetup(e);
				if (a < 0) return a;
				...
				const l = Xe(s), { memory: h, layout: d, threadAllocator: f } = Zs(e, s, l, Ss, {
					operation: "exec",
					path: t,
					argv: o
				}), u = d.channelOffset;
		...
		onClone: (e, t, n, i, r, s, o, a) => async function(e, t, n, i, r, s, o, a) {
			...
			ms.addChannel(e, h.channelOffset, t, n, i);
			const f = {
	...
	o.kernel.callbacks = {
		...
		onNetListen: (e, t, n) => {
			...
			return 0 !== i && o.startTcpListener(i, e, t, n), qs({
				type: "listen_tcp",
				pid: i,
				fd: e,
				port: t
	...
	}), e.bridgePort && (Ns = e.bridgePort, js()), qs({ type: "ready" });
}
async function to(e) {
	try {
		let t;
		if (await Rs(), e.programBytes) t = e.programBytes;
			...
			if (!e.programPath) return void Ys(e.requestId, "No programBytes or programPath");
			{
				const n = await async function(e) {
					return await ys.ensureMaterialized(e), ao(e);
				}(e.programPath);
				if (!n) return void Ys(e.requestId, `ENOENT: ${e.programPath}`);
				t = n;
			}
		}
		if (!De(t)) return void Ys(e.requestId, "ENOEXEC: program is not a WebAssembly module");
		const n = e.pid ?? ms.allocatePid(), i = e.programPath ?? e.argv[0], r = e.maxPages ?? Ss, s = Xe(t), { memory: o, layout: a, threadAllocator: c } = Zs(n, t, s, r, {
			operation: "spawn",
			path: i,
			argv: e.argv
		}), l = a.channelOffset;
		...
		As.set(n, {
			...
			layout: a,
			threadAllocator: c
		}), no(d, n), Xs(e.requestId, n);
	} catch (t) {
		Ys(e.requestId, String(t));
	}
}
function no(e, t) {
	let n = !1;
	const i = (i, r, s) => {
		...
		const o = `[kernel-worker] pid=${t} ${r} -> forcing exit ${i}`;
		0 === i && "worker-main exit message" === r ? console.debug(o) : console.warn(o), Ls(t, i, r), so(t, i, s);
	};
	e.on("error", (n) => {
		if (xs.has(e)) return;
		console.error(`[kernel-worker] Worker error pid=${t}:`, n.message);
		const r = fs(n);
		i(ds(r), "worker.onerror", r);
	}), e.on("exit", (t) => {
		xs.has(e) || i(ds(11), "worker exit event", 11);
	}), e.on("message", (e) => {
		const n = e;
		if ("error" === n.type) {
			console.error(`[kernel-worker] Process error pid=${t}:`, n.message), qs({
				type: "stderr",
				pid: t,
		...
		} else "exit" === n.type && i(n.status ?? 0, "worker-main exit message");
	});
}
...
async function ro(e, t, n, i) {
	...
	const r = Xe(t), { memory: s, layout: o, threadAllocator: a } = Zs(e, t, r, Ss, {
...
function so(e, t, n) {
	(async function(e, t, n = function(e) {
		...
		const r = Es.has(e) ? vs : 0, s = Math.max(r, function(e) {
			const t = Ms(e?.[0] ?? "");
		...
		const o = (async () => {
			...
			await Ds(e), i?.worker && await Ws(i.worker, s), ms.deactivateProcess(e), As.delete(e), Ps.delete(e), Ks.delete(e);
		})();
		Is.set(e, o), qs({
			type: "exit",
			pid: e,
...
async function oo(e, t) {
	if (!zs || !Ns) return;
	const n = Ns, i = function() {
		const e = Fs++;
		return Vs.add(e), Gs(), e;
	}(), r = t.url || "?";
	try {
		let i = $s;
		if (null == i && (i = Array.from(ms.tcpListenerTargets?.keys() ?? [])[0] ?? null), null == i) return console.warn(`[bridge] no listener target for req#${e} ${r}`), void n.postMessage({
			type: "http-error",
			requestId: e,
			error: "No listener target available"
		});
		console.debug(`[bridge] req#${e} ${t.method} ${r} -> port=${i}`);
		const s = await ms.sendHttpRequest(i, {
			method: t.method,
			url: r,
			headers: t.headers ?? {},
			body: t.body ?? null
		...
		console.warn(`[bridge] req#${e} ${r} failed:`, s), n.postMessage({
			type: "http-error",
			requestId: e,
...
function ao(e) {
	try {
		...
		try {
			...
			const n = new Uint8Array(e), i = ys.read(t, n, null, e);
			return ys.close(t), i <= 0 ? null : n.buffer.slice(n.byteOffset, n.byteOffset + i);
		} catch {
	...
	switch (t.type) {
		case "init":
			eo(t).catch((e) => {
				...
				console.error("[kernel-worker] init failed:", e), qs({
					type: "init_error",
					error: t
				});
			});
		...
		case "terminate_process":
			(async function(e) {
				...
				const i = As.get(t);
				i?.worker && await Ws(i.worker);
				try {
					ms.unregisterProcess(t);
				} catch {}
				As.delete(t), Ps.delete(t), Es.delete(t), Ks.delete(t), Xs(e.requestId, !0);
			})(t);
			break;
		...
		case "inject_connection":
			(function(e) {
				if (!zs) return void Xs(e.requestId, -1);
				const t = (0, zs.exports.kernel_inject_connection)(e.pid, e.fd, e.peerAddr[0], e.peerAddr[1], e.peerAddr[2], e.peerAddr[3], e.peerPort);
				t >= 0 && ms.scheduleWakeBlockedRetries(), Xs(e.requestId, t);
			})(t);
			break;
		case "pipe_read":
			(function(e) {
				if (!zs) return void Xs(e.requestId, null);
				const t = zs.exports.kernel_pipe_read, n = ms.tcpScratchOffset || ms.scratchOffset, i = [];
				for (;;) {
					const r = t(e.pid, e.pipeIdx, ms.toKernelPtr(n), ps);
					if (r <= 0) break;
					const s = new Uint8Array(Os.buffer);
					i.push(s.slice(n, n + r));
				}
				if (0 === i.length) return void Xs(e.requestId, null);
		...
		case "pipe_write":
			(function(e) {
				...
				const r = e.data;
				for (; i < r.length;) {
					const s = Math.min(r.length - i, ps);
					new Uint8Array(Os.buffer).set(r.subarray(i, i + s), n);
					const o = t(e.pid, e.pipeIdx, ms.toKernelPtr(n), s);
					if (o <= 0) break;
					i += o;
				}
				ms.notifyPipeReadable(e.pipeIdx), Xs(e.requestId, i);
			})(t);
			break;
		...
		case "pipe_is_write_open":
			(function(e) {
				...
				Xs(e.requestId, 1 === t(e.pid, e.pipeIdx));
			})(t);
			break;
		...
		case "is_stdin_consumed":
			(function(e) {
				const t = ms;
				Xs(e.requestId, t.stdinFinite.has(e.pid) && !t.stdinBuffers.has(e.pid));
			})(t);
			break;
		...
		case "http_request":
			(async function(t) {
				if (zs) try {
					...
					Ys(t.requestId, e instanceof Error ? e.message : String(e));
				}
				else Ys(t.requestId, "Kernel not initialized");
			})(t);
			break;
		case "destroy":
			(async function(e) {
				...
				As.clear(), Ps.clear(), Bs.clear(), Es.clear(), Ks.clear(), await Rs(), Xs(e.requestId, !0);
			})(t);
			break;
		...
		case "get_fork_count":
			try {
				...
				Ys(t.requestId, Fn?.message ?? String(Fn));
			}
			break;
		case "mouse_inject":
			(function(e) {
		...
		case "audio_drain":
			(function(e) {
				const t = Math.min(e.maxBytes, 65536), n = new Uint8Array(t), i = ms.drainAudio(n), r = ms.audioSampleRate(), s = ms.audioChannels(), o = i > 0 ? n.slice(0, i) : new Uint8Array(0);
				qs({
					type: "response",
		...
		case "read_proc_maps":
			try {
				Xs(t.requestId, ms.readProcMaps(t.pid));
			} catch (Fn) {
				Ys(t.requestId, Fn?.message ?? String(Fn));
			}
			break;
		...
		case "drain_syscall_trace":
			try {
				Xs(t.requestId, ms.drainSyscallTrace());
			} catch (Fn) {
				Ys(t.requestId, Fn?.message ?? String(Fn));
			}
			break;
		default: {
			const t = e.data;
			if ("sysprof_start" === t?.type) globalThis.__sysprof = !0, globalThis.__sysprofTable = /* @__PURE__ */ new Map(), globalThis.__sysprofStartedAt = performance.now(), qs({
				type: "stdout",
				pid: 0,
			...
			} else "set_bridge_port" === t?.type && t.bridgePort ? (Ns = t.bridgePort, js(), "number" == typeof t.httpPort && ($s = t.httpPort), Ns && (Ns.onmessage = (e) => {
				const t = e.data;
				"http-request" === t?.type && oo(t.requestId, t);
