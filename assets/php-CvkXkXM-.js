import{B as _,k as g}from"./browser-kernel-3EHx8tzG.js";/* empty css               */const T="/wasm-posix-kernel/assets/php-DWqvTewn.wasm",m=document.getElementById("code"),r=document.getElementById("output"),u=document.getElementById("run"),l=document.getElementById("examples"),o=document.getElementById("status"),f=new TextDecoder,S=new TextEncoder;function s(e,n){const t=document.createElement("span");n&&(t.className=n),t.textContent=e,r.appendChild(t),r.scrollTop=r.scrollHeight}function d(e,n){o.style.display="block",o.textContent=e,o.className=`status ${n}`}function w(){o.style.display="none"}const E={hello:`<?php
...
`};let a=null,c=null;async function P(){a&&c||(d("Loading kernel and PHP wasm...","loading"),[a,c]=await Promise.all([fetch(g).then(e=>e.arrayBuffer()),fetch(T).then(e=>e.arrayBuffer())]),s(`Kernel: ${(a.byteLength/1024).toFixed(0)}KB, PHP: ${(c.byteLength/(1024*1024)).toFixed(1)}MB
`,"info"))}async function y(){u.disabled=!0,r.textContent="";try{await P();const e=m.value;d("Running PHP...","running");const n=new _({onStdout:i=>s(f.decode(i)),onStderr:i=>s(f.decode(i),"stderr")});await n.init(a);const t=n.fs,p=S.encode(e),$="/tmp/script.php",h=t.open($,1|64|512,420);t.write(h,p,null,p.length),t.close(h);const b=await n.spawn(c,["php",$],{env:["HOME=/home","TMPDIR=/tmp","TERM=xterm-256color","PATH=/usr/local/bin:/usr/bin:/bin"]});s(`
Exited with code ${b}
`,"info"),w()}catch(e){s(`
Error: ${e}
