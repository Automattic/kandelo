...
[Python stopped]`),a.dispose(),a=null),k=null,w.disabled=!1,E.disabled=!0}w.addEventListener("click",q);E.addEventListener("click",G);v.addEventListener("change",()=>{const e={hello:'print("Hello, World!")',math:'import math; print(f"pi = {math.pi}")',list:"[x**2 for x in range(10)]",dict:'d = {"a": 1, "b": 2}; print(d)',sys:"import sys; print(sys.version)"},n=v.value;n&&e[n]&&a&&a.write(e[n]+`
`),v.value=""});const I={hello:`print("Hello from CPython 3.13.3 on WebAssembly!")
...
`};function m(e,n){const t=document.createElement("span");n&&(t.className=n),t.textContent=e,y.appendChild(t),y.scrollTop=y.scrollHeight}async function A(){B.disabled=!0,y.textContent="";try{const e=await N();e&&m(e,"info");const n=O.value,t=await W({onStdout:f=>m(T.decode(f)),onStderr:f=>m(T.decode(f),"stderr")}),i="/tmp/script.py",r=Y.encode(n),s=t.fs.open(i,1|64|512,420);t.fs.write(s,r,null,r.length),t.fs.close(s),c("Running Python...","running");const o=await t.spawn(p,["python3",i],{env:j});m(`
Exited with code ${o}
`,"info"),M()}catch(e){m(`
Error: ${e}
`,"stderr"),c(`Error: ${e}`,"error"),console.error(e)}finally{B.disabled=!1}}B.addEventListener("click",A);x.addEventListener("change",()=>{const e=x.value;e&&I[e]&&(O.value=I[e]),x.value=""});O.addEventListener("keydown",e=>{(e.ctrlKey||e.metaKey)&&e.key==="Enter"&&(e.preventDefault(),A())});
