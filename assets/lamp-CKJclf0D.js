...
if (isset($_SERVER['HTTP_HOST'])) {
    ...
    define('WP_HOME', '${E}://' . $_SERVER['HTTP_HOST'] . '${x}');
    define('WP_SITEURL', '${E}://' . $_SERVER['HTTP_HOST'] . '${x}');
}
...
`;async function b(t){try{const a=await fetch(t,{method:"HEAD"});return a.ok&&parseInt(a.headers.get("content-length")||"0",10)||0}catch{return 0}}let n=null,$=null;async function ie(){T.disabled=!0,m.textContent="",d("Loading WordPress (LEMP)...","loading");try{s(`Fetching resources...
...
`,"info"),e==="nginx"&&(d("LEMP stack running! Loading WordPress...","running"),k.disabled=!1,D())}}),await $.boot()}catch(t){const a=(t==null?void 0:t.message)||String(t);s(`
Error: ${a}
`,"stderr"),d(`Error: ${a}`,"error"),console.error(t),T.disabled=!1}}function D(){const t=document.createElement("iframe");t.id="frame",t.src=O,A.replaceWith(t),A=t}T.addEventListener("click",ie);k.addEventListener("click",D);
