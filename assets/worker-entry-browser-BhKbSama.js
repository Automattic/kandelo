...
async function $(e, t) {
	...
	try {
		...
		const S = k(p, n, A, r, void 0, () => _, m), x = new WebAssembly.Instance(p, S);
		_ = x;
		const E = x.exports.__wasm_init_tls, I = d;
		E && I > 0 && E(8 === m ? BigInt(I) : I);
		const U = x.exports.__stack_pointer;
		U && (U.value = 8 === m ? BigInt(c) : c);
		const M = x.exports.__wasm_thread_init;
		M && f > 0 && M(8 === m ? BigInt(f) : f), W(x, p, n, r, t.programBytes, m);
		const $ = x.exports.__indirect_function_table;
		if (!$) throw new Error("No __indirect_function_table export — cannot call thread function");
		const P = 8 === m ? BigInt(i) : i, T = $.get(P);
		if (!T) throw new Error(`Thread function at table index ${i} is null`);
		const O = 8 === m ? BigInt(a) : a;
		let R = 0;
		if (b) {
			const e = x.exports.wpk_fork_state, t = x.exports.wpk_fork_unwind_end, o = x.exports.wpk_fork_rewind_begin;
			let s = !1;
			for (;;) {
				s && (o(h), s = !1);
				try {
					...
					if (g instanceof Error && g.message.includes("unreachable")) {
						R = 0;
						break;
					}
					if (g instanceof Error && g.message.includes("null function or function signature mismatch")) {
						R = 0;
						break;
					}
					throw g;
			...
			if (g instanceof Error && g.message.includes("unreachable")) R = 0;
			else {
				if (!(g instanceof Error && g.message.includes("null function or function signature mismatch"))) throw g;
				R = 0;
			}
		}
		{
			const e = new DataView(n.buffer), t = r;
			e.setInt32(t + 4, l.Exit, !0), e.setInt32(t + 8, R ?? 0, !0);
...
P.onmessage = (e) => {
	const t = e.data;
	if ("__kandelo_worker_shutdown" === t.type) return P.postMessage({ type: "__kandelo_worker_shutdown_ack" }), void P.close?.();
	const n = {
	...
	if ("centralized_init" === t.type) (async function(t, n) {
		try {
			...
			if (p(a)) {
				...
				const { WasiShim: o, WasiExit: l } = await import("./wasi-shim-D_e4hj7W.js"), c = new o(r, s, n.argv || [], n.env || []), f = {
