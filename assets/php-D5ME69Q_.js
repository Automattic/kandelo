import{B as _,k as g}from"./browser-kernel-BPDGIj33.js";const S="/wasm-posix-kernel/assets/php-CDviQDmV.wasm",m=document.getElementById("code"),r=document.getElementById("output"),u=document.getElementById("run"),l=document.getElementById("examples"),a=document.getElementById("status"),f=new TextDecoder,T=new TextEncoder;function s(e,n){const t=document.createElement("span");n&&(t.className=n),t.textContent=e,r.appendChild(t),r.scrollTop=r.scrollHeight}function d(e,n){a.style.display="block",a.textContent=e,a.className=`status ${n}`}function w(){a.style.display="none"}const E={hello:`<?php
...
`};let o=null,c=null;async function P(){o&&c||(d("Loading kernel and PHP wasm...","loading"),[o,c]=await Promise.all([fetch(g).then(e=>e.arrayBuffer()),fetch(S).then(e=>e.arrayBuffer())]),s(`Kernel: ${(o.byteLength/1024).toFixed(0)}KB, PHP: ${(c.byteLength/(1024*1024)).toFixed(1)}MB
`,"info"))}async function y(){u.disabled=!0,r.textContent="";try{await P();const e=m.value;d("Running PHP...","running");const n=new _({onStdout:i=>s(f.decode(i)),onStderr:i=>s(f.decode(i),"stderr")});await n.init(o);const t=n.fs,$=T.encode(e),p="/tmp/script.php",h=t.open(p,1|64|512,420);t.write(h,$,null,$.length),t.close(h);const b=await n.spawn(c,["php",p],{env:["HOME=/home","TMPDIR=/tmp","TERM=xterm-256color","PATH=/usr/local/bin:/usr/bin:/bin"]});s(`
Exited with code ${b}
`,"info"),w()}catch(e){s(`
Error: ${e}
