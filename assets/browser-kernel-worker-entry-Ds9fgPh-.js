...
var Bn = class {
	...
	async init(e) {
		...
		try {
			const e = await import("./__vite-browser-external-u9pDiogR.js").then((e) => a(e.default, 1));
...
if (void 0 === globalThis.setImmediate) {
	const eo = [];
	let to = 0, no = !1, io = !1;
	const ro = /* @__PURE__ */ new Set(), so = new MessageChannel();
	function oo() {
		no = !1, io = !0;
		const e = eo.length;
		for (let n = 0; n < e && eo.length > 0; n++) {
			const e = eo.shift();
			if (ro.has(e.id)) ro.delete(e.id);
			else try {
				e.fn(...e.args);
			} catch (t) {
		...
		io = !1, eo.length > 0 && !no && (no = !0, so.port2.postMessage(null));
	}
	so.port1.onmessage = oo, globalThis.setImmediate = (e, ...t) => {
		const n = ++to;
		return eo.push({
			id: n,
			fn: e,
			args: t
		}), no || io || (no = !0, so.port2.postMessage(null)), n;
...
async function Ls(e) {
	...
	if (t) {
		Bs.delete(e);
		for (const n of t) await (n.termination ?? Ms(n.worker, vs)), Hs.release(e, n.channelOffset);
	}
}
const Rs = /* @__PURE__ */ new Map();
let Ws = null, Ds = null, Ks = null, zs = null;
function Os(e, t) {
...
async function Gs(e) {
	...
	_s = new Hn(e.workerEntryUrl), ms = new Bn({
		...
		onExec: async (e, t, n, i) => {
			const r = await async function(e, t, n, i) {
				...
				const l = Xe(s), { memory: h, layout: d, threadAllocator: f } = Vs(e, s, l), u = d.channelOffset;
		...
		onClone: (e, t, n, i, r, s, o, a) => async function(e, t, n, i, r, s, o, a) {
			...
			}, _ = () => (p.termination || (p.termination = Ms(u, vs).finally(m)), p.termination);
	...
	o.kernel.callbacks = {
		...
		onNetListen: (e, t, n) => {
			...
			return 0 !== i && o.startTcpListener(i, e, t, n), Os({
				type: "listen_tcp",
				pid: i,
				fd: e,
				port: t
	...
	}, await ms.init(e.kernelWasmBytes), Ws = o.kernelInstance, Ds = o.kernelMemory, ms.framebuffers.onChange((e, t) => {
		if ("bind" === t) {
	...
	}), e.bridgePort && (Ks = e.bridgePort), Os({ type: "ready" });
}
async function js(e) {
	try {
		let t;
		if (await Us(), e.programBytes) t = e.programBytes;
		else {
			if (!e.programPath) return void $s(e.requestId, "No programBytes or programPath");
			{
				const n = await async function(e) {
					return await ys.ensureMaterialized(e), Qs(e);
				}(e.programPath);
				if (!n) return void $s(e.requestId, `ENOENT: ${e.programPath}`);
				t = n;
			}
		}
		if (!De(t)) return void $s(e.requestId, "ENOEXEC: program is not a WebAssembly module");
		const n = e.pid ?? ms.allocatePid(), i = e.maxPages ?? Ss, r = Xe(t), { memory: s, layout: o, threadAllocator: a } = Vs(n, t, r, i), c = o.channelOffset;
		if (ms.registerProcess(n, s, [c], {
			ptrWidth: r,
			argv: e.argv,
			brkBase: o.brkBase,
			mmapBase: o.mmapBase,
			...
			Rs.set(n, t), null != e.ptyCols && null != e.ptyRows && ms.ptySetWinsize(t, e.ptyRows, e.ptyCols);
		} else if (e.stdin) {
		...
		As.set(n, {
			...
			layout: o,
			threadAllocator: a
		}), Xs(h, n), Ns(e.requestId, n);
	} catch (t) {
		$s(e.requestId, String(t));
	}
}
function Xs(e, t) {
	let n = !1;
	const i = (i, r, s) => {
		...
		0 === i && "worker-main exit message" === r ? console.debug(o) : console.warn(o), Zs(t, i, s);
	};
	e.on("error", (n) => {
		if (xs.has(e)) return;
		...
		const n = e;
		if ("error" === n.type) {
			console.error(`[kernel-worker] Process error pid=${t}:`, n.message), Os({
				type: "stderr",
				pid: t,
...
async function Js(e, t, n, i) {
	await Us(), Os({
		type: "proc_event",
		kind: "spawn",
		pid: e
	});
	const r = Xe(t), { memory: s, layout: o, threadAllocator: a } = Vs(e, t, r), c = o.channelOffset;
...
function Zs(e, t, n) {
	(async function(e, t, n = function(e) {
		...
		if (Is.has(e)) return;
		const i = As.get(e), r = Es.has(e) ? vs : 0, s = Math.max(r, function(e) {
			const t = function(e) {
		...
		const o = (async () => {
			...
			ms.deactivateProcess(e), As.delete(e), Ps.delete(e), Rs.delete(e), await Ls(e), i?.worker && await Ms(i.worker, s);
		})();
		Is.set(e, o), Os({
			type: "exit",
			pid: e,
	...
	switch (t.type) {
		case "init":
			Gs(t).catch((e) => {
				...
				console.error("[kernel-worker] init failed:", e), Os({
					type: "init_error",
					error: t
				});
			});
		...
		case "terminate_process":
			(async function(e) {
				...
				if (n) {
					for (const e of n) {
						await (e.termination ?? Ms(e.worker, vs));
						try {
				...
				const i = As.get(t);
				i?.worker && await Ms(i.worker);
				try {
					ms.unregisterProcess(t);
				} catch {}
				As.delete(t), Ps.delete(t), Es.delete(t), Rs.delete(t), Ns(e.requestId, !0);
			})(t);
			break;
		...
		case "inject_connection":
			(function(e) {
				if (!Ws) return void Ns(e.requestId, -1);
				const t = (0, Ws.exports.kernel_inject_connection)(e.pid, e.fd, e.peerAddr[0], e.peerAddr[1], e.peerAddr[2], e.peerAddr[3], e.peerPort);
				t >= 0 && ms.scheduleWakeBlockedRetries(), Ns(e.requestId, t);
			})(t);
			break;
		case "pipe_read":
			(function(e) {
				if (!Ws) return void Ns(e.requestId, null);
				const t = Ws.exports.kernel_pipe_read, n = ms.tcpScratchOffset || ms.scratchOffset, i = [];
				for (;;) {
					const r = t(e.pid, e.pipeIdx, ms.toKernelPtr(n), ps);
					if (r <= 0) break;
					const s = new Uint8Array(Ds.buffer);
					i.push(s.slice(n, n + r));
				}
				if (0 === i.length) return void Ns(e.requestId, null);
		...
		case "pipe_write":
			(function(e) {
				if (!Ws) return void Ns(e.requestId, -1);
				const t = Ws.exports.kernel_pipe_write, n = ms.tcpScratchOffset || ms.scratchOffset;
				let i = 0;
				const r = e.data;
				for (; i < r.length;) {
					const s = Math.min(r.length - i, ps);
					new Uint8Array(Ds.buffer).set(r.subarray(i, i + s), n);
					const o = t(e.pid, e.pipeIdx, ms.toKernelPtr(n), s);
					if (o <= 0) break;
					i += o;
				}
				ms.notifyPipeReadable(e.pipeIdx), Ns(e.requestId, i);
			})(t);
			break;
		case "pipe_close_read":
			(function(e) {
				if (!Ws) return;
				(0, Ws.exports.kernel_pipe_close_read)(e.pid, e.pipeIdx);
			})(t);
			break;
		case "pipe_close_write":
			(function(e) {
				if (!Ws) return;
				(0, Ws.exports.kernel_pipe_close_write)(e.pid, e.pipeIdx);
			})(t);
			break;
		case "pipe_is_write_open":
			(function(e) {
				if (!Ws) return void Ns(e.requestId, !1);
				const t = Ws.exports.kernel_pipe_is_write_open;
				Ns(e.requestId, 1 === t(e.pid, e.pipeIdx));
			})(t);
			break;
		...
		case "is_stdin_consumed":
			(function(e) {
				const t = ms;
				Ns(e.requestId, t.stdinFinite.has(e.pid) && !t.stdinBuffers.has(e.pid));
			})(t);
			break;
		...
		case "http_request":
			(async function(t) {
				if (Ws) try {
					...
					$s(t.requestId, e instanceof Error ? e.message : String(e));
				}
				else $s(t.requestId, "Kernel not initialized");
			})(t);
			break;
		case "destroy":
			(async function(e) {
				for (const [t, n] of As) {
					n.worker && (await Ls(t), await Ms(n.worker));
					try {
						ms.unregisterProcess(t);
					} catch {}
				}
				for (const t of Bs.values()) for (const e of t) await (e.termination ?? Ms(e.worker, vs));
				As.clear(), Ps.clear(), Bs.clear(), Es.clear(), Rs.clear(), await Us(), Ns(e.requestId, !0);
			})(t);
			break;
		...
		case "get_fork_count":
			try {
				...
				$s(t.requestId, Fn?.message ?? String(Fn));
			}
			break;
		case "mouse_inject":
			(function(e) {
		...
		case "read_proc_maps":
			try {
				Ns(t.requestId, ms.readProcMaps(t.pid));
			} catch (Fn) {
				$s(t.requestId, Fn?.message ?? String(Fn));
			}
			break;
		...
		case "drain_syscall_trace":
			try {
				Ns(t.requestId, ms.drainSyscallTrace());
			} catch (Fn) {
				$s(t.requestId, Fn?.message ?? String(Fn));
			}
			break;
		default: {
			const t = e.data;
			if ("sysprof_start" === t?.type) globalThis.__sysprof = !0, globalThis.__sysprofTable = /* @__PURE__ */ new Map(), globalThis.__sysprofStartedAt = performance.now(), Os({
				type: "stdout",
				pid: 0,
			...
			} else "set_bridge_port" === t?.type && t.bridgePort ? (Ks = t.bridgePort, "number" == typeof t.httpPort && (zs = t.httpPort), Ks && (Ks.onmessage = (t) => {
				const n = t.data;
				"http-request" === n?.type && async function(t, n) {
					if (!Ws || !Ks) return;
					const i = n.url || "?";
					let r = zs;
					if (null == r && (r = Array.from(ms.tcpListenerTargets?.keys() ?? [])[0] ?? null), null == r) return console.warn(`[bridge] no listener target for req#${t} ${i}`), void Ks.postMessage({
						type: "http-error",
						requestId: t,
					...
					try {
						const e = await ms.sendHttpRequest(r, {
							method: n.method,
							url: i,
							headers: n.headers ?? {},
							body: n.body ?? null
						...
						console.warn(`[bridge] req#${t} ${i} failed:`, e), Ks.postMessage({
							type: "http-error",
							requestId: t,
