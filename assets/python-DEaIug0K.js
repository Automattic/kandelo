...
`};function m(e,n){const t=document.createElement("span");n&&(t.className=n),t.textContent=e,y.appendChild(t),y.scrollTop=y.scrollHeight}async function W(){B.disabled=!0,y.textContent="";try{const e=await D();e&&m(e,"info");const n=O.value,t=await M({onStdout:f=>m(T.decode(f)),onStderr:f=>m(T.decode(f),"stderr")}),i="/tmp/script.py",r=Y.encode(n),s=t.fs.open(i,1|64|512,420);t.fs.write(s,r,null,r.length),t.fs.close(s),c("Running Python...","running");const o=await t.spawn(p,["python3",i],{env:N});m(`
Exited with code ${o}
`,"info"),j()}catch(e){m(`
Error: ${e}
`,"stderr"),c(`Error: ${e}`,"error"),console.error(e)}finally{B.disabled=!1}}B.addEventListener("click",W);v.addEventListener("change",()=>{const e=v.value;e&&I[e]&&(O.value=I[e]),v.value=""});O.addEventListener("keydown",e=>{(e.ctrlKey||e.metaKey)&&e.key==="Enter"&&(e.preventDefault(),W())});
