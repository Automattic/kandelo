 ...
 *
 * Usage:
 *   node --experimental-strip-types packages/examples/wordpress/serve.ts [port]
 *
 * Requires:
 *   1. PHP binary: packages/registry/php/php-src/sapi/cli/php
 *      (build with: cd packages/registry/php && bash build.sh)
 *   2. WordPress files: packages/examples/wordpress/wordpress/
 *      (download with: bash packages/examples/wordpress/setup.sh)
 */
...
if (!existsSync(join(wpDir, "wp-settings.php"))) {
  console.error("Error: WordPress not found. Run: bash packages/examples/wordpress/setup.sh");
  process.exit(1);
}
