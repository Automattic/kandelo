 ...
 *
 * Usage:
 *   npx tsx packages/examples/wordpress/perf/benchmark.ts
 *   npx tsx packages/examples/wordpress/perf/benchmark.ts --rounds=5
 *   npx tsx packages/examples/wordpress/perf/benchmark.ts --headed
 */
...
function getOptions(): Options {
  ...
  if (values.help) {
    console.log(`Usage: npx tsx packages/examples/wordpress/perf/benchmark.ts [options]
...
async function startServer(port: number): Promise<{ proc: ChildProcess; startupMs: number }> {
  ...
  const proc = spawn(
    "npx",
    ["tsx", "packages/examples/wordpress/serve-nginx.ts", String(port)],
    {
      cwd: REPO_ROOT,
