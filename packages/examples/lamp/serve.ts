 ...
 *
 * Usage:
 *   npx tsx packages/examples/lamp/serve.ts [port]
 *
 * Requires:
 *   1. MariaDB:  packages/registry/mariadb/mariadb-install/bin/mariadbd
 *   2. PHP-FPM:  packages/examples/nginx/php-fpm.wasm (with --with-pdo-mysql)
 *   3. nginx:    packages/examples/nginx/nginx.wasm
 *   4. WordPress: packages/examples/lamp/wordpress/
 *      (download with: bash packages/examples/lamp/setup.sh)
 */
...
for (const [name, path, hint] of [
  ...
  ["nginx.wasm", nginxWasmPath, "bash packages/examples/nginx/build.sh"],
] as const) {
  if (!existsSync(path)) {
...
if (!existsSync(join(wpDir, "wp-settings.php"))) {
  console.error("Error: WordPress not found. Run: bash packages/examples/lamp/setup.sh");
  process.exit(1);
}
