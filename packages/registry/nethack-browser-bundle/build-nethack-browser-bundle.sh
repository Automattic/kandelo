...
NETHACK_DIR="${WASM_POSIX_DEP_NETHACK_DIR:-}"
TARGET_ARCH="${WASM_POSIX_DEP_TARGET_ARCH:-}"

fail() {
    echo "build-nethack-browser-bundle: $*" >&2
    exit 2
}
...
require_real_directory WASM_POSIX_DEP_NETHACK_DIR "$NETHACK_DIR"

archive="$WORK_DIR/nethack.zip"
...
install_local_binary nethack-browser-bundle "$archive" nethack.zip
