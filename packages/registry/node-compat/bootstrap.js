...
const timers = (() => {
    ...
    const wrap = (id, type) => {
        const handle = {
            _id: id,
            _resourceType: type,
            _resourceId: _trackActiveResource(type),
            _hasRef: true,
            _destroyed: false,
            unref() {
                if (this._hasRef) {
                    _untrackActiveResource(this._resourceId);
                    this._resourceId = 0;
                    this._hasRef = false;
                }
                return this;
            ...
            ref() {
                if (!this._destroyed && !this._hasRef) {
                    this._resourceId = _trackActiveResource(this._resourceType);
                    this._hasRef = true;
                }
                return this;
            },
            hasRef() { return this._hasRef; },
            refresh() { return this; },
        };
        return handle;
    };
...
const timersPromises = {
    setTimeout: (delay, value) => new Promise(r => timers.setTimeout(() => r(value), delay || 0)),
};
...
function _runEventLoopUntilIdle() {
    let spins = 0;
    while (_activeResources.size > 0) {
        _drainSpiderMonkeyJobs();
        if (typeof __kandeloRunDueTimers === 'function') {
            try {
        ...
        _drainSpiderMonkeyJobs();
        if (_activeResources.size === 0) break;

        const delay = typeof __kandeloNextTimerDelay === 'function'
            ? __kandeloNextTimerDelay()
            : null;
        if (delay === null || delay === undefined) break;
        if (delay > 0 && _eventLoopSleepView) {
            try { Atomics.wait(_eventLoopSleepView, 0, 0, Math.min(delay, 10)); } catch {}
        }
        if (++spins > 100000) {
            process.exitCode = process.exitCode || 1;
            console.error('Node compatibility event loop did not quiesce');
            break;
        }
    }
}
