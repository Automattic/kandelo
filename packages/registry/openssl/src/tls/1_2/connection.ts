...
class MessageEncoder {
	static certificate(certificatesDER: (ArrayBufferLike | ArrayBufferView)[]): Uint8Array {
		const certsBodies: Uint8Array[] = [];
		for (const cert of certificatesDER) {
			certsBodies.push(as3Bytes(cert.byteLength));
			certsBodies.push(new Uint8Array(ArrayBuffer.isView(cert) ? cert.buffer as ArrayBuffer : cert as ArrayBuffer));
		}
	...
	static async createFinishedMessage(
		...
		masterSecret: ArrayBufferLike | ArrayBufferView
	): Promise<Uint8Array> {
