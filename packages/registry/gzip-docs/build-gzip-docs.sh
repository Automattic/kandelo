#!/usr/bin/env bash
# Generate gzip's man page(s) from the REAL gzip wasm binary running
...
DEP_DIR="${WASM_POSIX_DEP_GZIP_DIR:?gzip dependency dir required}"
...
SRC_LABEL="GNU gzip 1.13 (Kandelo)"

GZIP_WASM="$(manpage_docs_find_wasm "$DEP_DIR" gzip.wasm)"

manpage_docs_capture "$CAP" "$GZIP_WASM" gzip

manpage_docs_emit_page gzip 1 "compress or expand files" "$SRC_LABEL" "$CAP" "$STAGE" "$WRAP"

[ -f "$STAGE/share/man/man1/gzip.1" ] || { echo "gzip.1 not generated" >&2; exit 2; }
manpage_docs_finalize "$STAGE" "$WORK" gzip-docs gzip-docs.zip
