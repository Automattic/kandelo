#!/usr/bin/env bash
#
# Build the browser lazy-archive bundle for GNU Midnight Commander.
#
# The package-system archive remains a .tar.zst, but its declared output is
# mc.zip. Resolver consumers see the bare zip at programs/wasm32/mc.zip.
set -euo pipefail
...
MC_DIR="${WASM_POSIX_DEP_MC_DIR:-}"
TARGET_ARCH="${WASM_POSIX_DEP_TARGET_ARCH:-}"

fail() {
    echo "build-mc-browser-bundle: $*" >&2
    exit 2
}
...
require_real_directory WASM_POSIX_DEP_MC_DIR "$MC_DIR"

archive="$WORK_DIR/mc.zip"
bash "$REPO_ROOT/images/vfs/scripts/build-mc-zip.sh" \
    "$MC_DIR" "$archive"
...
install_local_binary mc-browser-bundle "$archive" mc.zip
