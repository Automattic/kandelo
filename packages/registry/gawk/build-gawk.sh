...
GAWK_VERSION="${GAWK_VERSION:-${WASM_POSIX_DEP_VERSION:-5.3.0}}"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
WORK_DIR="${WASM_POSIX_DEP_WORK_DIR:-$SCRIPT_DIR}"
SRC_DIR="${WASM_POSIX_DEP_SOURCE_DIR:-$WORK_DIR/gawk-src}"
...
if [ ! -d "$SRC_DIR" ]; then
    ...
    URL="${WASM_POSIX_DEP_SOURCE_URL:-https://ftpmirror.gnu.org/gnu/gawk/${TARBALL}}"
...
# --- Configure ---
if [ ! -f Makefile ]; then
    echo "==> Configuring gawk for wasm32..."
    ...
    # gawk-specific cross-compilation overrides
    export ac_cv_func_getpgrp_void=yes
    export ac_cv_func_GetSystemTimeAsFileTime=no
...
if [ -f "$SRC_DIR/gawk" ]; then
    cp "$SRC_DIR/gawk" "$BIN_DIR/gawk.wasm"
    echo "==> Built gawk"
    ls -lh "$BIN_DIR/gawk.wasm"
else
    echo "ERROR: gawk binary not found after build" >&2
    exit 1
fi
