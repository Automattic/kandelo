...
# real type classification in ffi_call plus ffi_closure over a
...
# trampoline pool). See include/ffi.h for the design.
#
# The three objects stay separate in the archive on purpose: a consumer
# that never creates closures links ffi_core.o + ffi_dispatch.o and does
# not pay for the trampoline pool.
#
...
if [ -n "${WASM_POSIX_DEP_OUT_DIR:-}" ]; then
    if [ -n "$(find "$INSTALL_DIR" -mindepth 1 -print -quit)" ]; then
        echo "ERROR: libffi resolver output directory must start empty" >&2
        exit 1
    fi
else
    rm -rf "$INSTALL_DIR"
