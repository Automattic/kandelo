...
# Build GNU m4 1.4.19 for wasm32-posix-kernel.
#
# Uses the SDK's wasm32posix-configure wrapper for cross-compilation.
# Output: packages/registry/m4/bin/m4.wasm
...
SOURCE_URL="${WASM_POSIX_DEP_SOURCE_URL:-https://ftpmirror.gnu.org/gnu/m4/m4-${M4_VERSION}.tar.xz}"
...
# --- Stage verified m4 source ---
expected_source_marker="$(printf '%s\n%s\n%s' \
    "$M4_VERSION" "$SOURCE_URL" "$SOURCE_SHA256")"
if [ -d "$SRC_DIR" ] && \
   [ "$(cat "$SOURCE_MARKER" 2>/dev/null || true)" != "$expected_source_marker" ]; then
    rm -rf "$SRC_DIR" "$BIN_DIR"
fi
if [ ! -d "$SRC_DIR" ]; then
    echo "==> Staging verified m4 $M4_VERSION source..."
    kandelo_package_stage_verified_source m4 "$SRC_DIR" \
...
# --- Configure ---
if [ ! -f Makefile ]; then
    echo "==> Configuring m4 for wasm32..."
...
if [ -f "$SRC_DIR/src/m4" ]; then
    cp "$SRC_DIR/src/m4" "$BIN_DIR/m4.wasm"
    echo "==> Built m4"
    ls -lh "$BIN_DIR/m4.wasm"
else
    echo "ERROR: m4 binary not found after build" >&2
    exit 1
fi
...
install_local_binary m4 "$BIN_DIR/m4.wasm"
