...
export function readSourceRootfsShellDependencyContract(path) {
  ...
  const dependencies = value.dependencies.map((entry, index) => {
    ...
    return { name: entry.name, version: entry.version, role: entry.role };
  });
...
export function readLockedPackageManifestDependencies(path) {
  ...
  if (
    Array.from(topLevel.matchAll(/^[ \t]*depends_on[ \t]*=/gm)).length !== 1
  ) {
    throw new Error(
...
export function validateSourceRootfsShellPackageManifest(contract, path) {
  ...
  const mismatched = [...expectedByName].flatMap(([name, expectedVersion]) => {
...
if (
  ...
  if (
    ...
    process.argv[2] !== "--print-resolver-owned"
  ) {
    throw new Error(
  ...
  validateSourceRootfsShellPackageManifest(contract, resolve(process.argv[4]));
