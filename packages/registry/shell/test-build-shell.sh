...
grep -Eq '^revision[[:space:]]*=[[:space:]]*30$' "$BUILD_TOML" ||
    fail "canonical source shell revision must be 30"
...
[ "${#declared_dependencies[@]}" -eq 20 ] ||
    fail "canonical contract must expose 20 lazy resolver dependencies"
