...
# Inject wasm32_uw.o into the bash link line. Idempotent and outside
# the wasm32_uw.c-creation block above so a Makefile regenerated by a
# fresh `configure` (e.g. after `make distclean`) still gets the patch
# even when wasm32_uw.c is already in place from a prior build.
if ! grep -q "wasm32_uw.o" Makefile; then
    sed -i.bak 's/\(OBJECTS[[:space:]]*=[[:space:]]*\)shell\.o /\1wasm32_uw.o shell.o /' Makefile
    rm -f Makefile.bak
fi
...
echo ""
echo "Run with:"
echo "  npx tsx packages/registry/shell/demo/serve.ts -c 'echo hello from bash'"
