...
# shared object file". On macOS, the declared Nix clang's real installed
# directory is outside LLVM_PREFIX, so clang++ does not discover the unified
# tree's libc++ headers automatically. Select those declared headers and
# libraries explicitly rather than falling back to an ambient Xcode toolchain.
# CXXFLAGS/LDFLAGS set here are honored: runConfigureICU re-exports them to
# configure.
HOST_CXXFLAGS="-O2"
case "$(uname -s)" in
    Linux) HOST_LDFLAGS="-static-libstdc++ -static-libgcc" ;;
    Darwin)
        : "${LLVM_PREFIX:?ERROR: LLVM_PREFIX is required for the ICU host build on macOS}"
        [ -f "$LLVM_PREFIX/include/c++/v1/memory" ] || {
        ...
        HOST_CXXFLAGS="$HOST_CXXFLAGS -nostdinc++ -isystem $LLVM_PREFIX/include/c++/v1"
        HOST_LDFLAGS="-L$LLVM_PREFIX/lib"
        ;;
    *) HOST_LDFLAGS="" ;;
esac
