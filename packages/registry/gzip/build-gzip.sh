...
GZIP_VERSION="${GZIP_VERSION:-${WASM_POSIX_DEP_VERSION:-1.13}}"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
WORK_DIR="${WASM_POSIX_DEP_WORK_DIR:-$SCRIPT_DIR}"
SRC_DIR="${WASM_POSIX_DEP_SOURCE_DIR:-$WORK_DIR/gzip-src}"
...
if [ ! -d "$SRC_DIR" ]; then
    ...
    URL="${WASM_POSIX_DEP_SOURCE_URL:-https://ftpmirror.gnu.org/gnu/gzip/${TARBALL}}"
