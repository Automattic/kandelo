...
/// Default ABI version for CLI commands.
///
/// Task A.10 will replace this with a runtime read of
/// `crates/shared/src/lib.rs::ABI_VERSION`. Until then the CLI
/// hardcodes the current value (4) so cache keys generated by the
/// CLI match those any in-tree caller would produce.
const DEFAULT_ABI_VERSION: u32 = 4;
...
fn cmd_resolve(m: &DepsManifest, registry: &Registry, repo: &Path) -> Result<(), String> {
    ...
    let path = ensure_built(m, registry, DEFAULT_ARCH, DEFAULT_ABI_VERSION, &opts)?;
    println!("{}", path.display());
    Ok(())
}
...
"#,
    ...
    /// Test-default ABI version — current `ABI_VERSION` in
    /// `crates/shared/src/lib.rs`. Hardcoded here per A.5 (A.10 wires
    /// the actual constant).
