...
/// Accepts the bare form `binaries-abi-v<N>` and the dated form
/// `binaries-abi-v<N>-YYYY-MM-DD` (where `<N>` is the kernel's
/// compiled-in `ABI_VERSION`). The dated suffix is what we publish to
/// GitHub Releases — see `docs/binary-releases.md` "Release tag
/// convention". The bare form is preserved so legacy callers (and the
/// build_manifest tests) keep working.
fn verify_tag_matches_abi(tag: &str, abi_version: u32) -> Result<(), String> {
    ...
    if suffix_ok {
        ...
        Err(format!(
            "tag {tag:?} does not match {prefix:?} or {prefix:?}-YYYY-MM-DD — \
             refusing to generate a manifest that would claim a different ABI \
             than `wasm_posix_shared::ABI_VERSION` ({abi_version}). \
