 ...
 * good handle in place, stashes the info log in renderer_last_error(),
 * and returns 1. Empty-info-log failures — the headless-GL stub that
 * NodeKernelHost emits because host_gl_query returns -1 — return 0
 * (warned but not "real"), matching Phase 2/3 behavior. */
 ...
 * (vp_x, vp_y, vp_w, vp_h) rectangle iff `renderer_error_visible()`
 * is set. The caller is expected to have the right-pane scissor still
 * active so the strip is clipped within it. */
void renderer_draw_error_strip(int vp_x, int vp_y, int vp_w, int vp_h);

void renderer_set_error_visible(int visible);
int  renderer_error_visible(void);
const char *renderer_last_error(void);
 ...
 * top-left of the window, matching the editor's mental model. The
 * baseline of the first row lives at `y + ascent`. */

int  renderer_text_advance(void);     /* monospace glyph cell width  */
int  renderer_text_line_height(void); /* total line stride (px)      */
int  renderer_text_ascent(void);      /* baseline offset from top (px)*/
