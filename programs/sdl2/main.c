 ...
 * (`sdl2.wasm`, installed at /usr/local/bin/sdl2). Keystrokes typed
 * into the left pane edit a gap buffer (`editor.c`), 250 ms of
 * keystroke-idle triggers a recompile of the user shader, and Ctrl+S
 * writes the editor's contents to `/home/shaders/image/current.frag`
 * and force-reloads.
 *
 * main.c stays at orchestration: SDL/GL/audio init, the main event
 * loop, key routing, the auto-recompile timer, F5/Ctrl+S, ESC quit.
 * All GL drawing (user shader, error strip, editor text, font atlas)
 * lives in `renderer.c`; all gap-buffer + cursor + line navigation
 * lives in `editor.c`.
 *
 * Startup shader-source chain:
 ...
 *
 * Audio runs on SDL's own audio thread; main-loop writes to the state
 * the callback reads hold SDL_LockAudioDevice.
 */
 ...
 * live image shader; the editor takes the rest. The split is 2/3
...
/* Boot default: the image preset leg points at "Trailing the
...
/* The sound-shader source chain mirrors the image chain. Its preset leg
 * points at the Tunnelwisp track; sine.frag stays a loadable preset. */
...
/* Renders the chip synth or the sound-shader buffer, and feeds the FFT
 * analysis ring that drives the iAudio uniform. SDL's dsp backend runs
 * this on its own audio thread, so every main-loop write to the state it
 * reads is bracketed by SDL_LockAudioDevice. */
...
int main(void) {
    /* The stdout breadcrumbs are the test contract (sdl2.test.ts waits
     * on them mid-run) and the harness feeds stdout through a pipe, so
     * musl full-buffers it. Line-buffer explicitly so every breadcrumb
     * leaves at its newline. */
    ...
    /* Stand up the sound-shader pipeline and compile the resolved
     * sound source. The Tunnelwisp track is the boot default: render it
     * and hand the decoded PCM to the audio source immediately, instead
     * of waiting for the first F2. On the
