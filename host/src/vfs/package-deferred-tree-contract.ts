...
export function parsePackageDeferredZipTreeSpec(
  ...
  const activationValue = record.activation;
  const activation = exactRecord(
    ...
    [
      ...
      ...(typeof activationValue === "object" &&
          ...
          Object.hasOwn(activationValue, "atomic_group")
        ? ["atomic_group"]
        : []),
    ],
  ...
  const atomicGroup = activation.atomic_group;
  if (
    atomicGroup !== undefined &&
    (
      activation.mode !== "first-use" ||
      typeof atomicGroup !== "string" ||
      utf8Length(atomicGroup) >
        VFS_DEFERRED_TREE_LIMITS.maxActivationCapabilityBytes ||
      !/^[a-z0-9][a-z0-9:._-]*$/.test(atomicGroup)
    )
  ) {
    throw new Error("package deferred ZIP tree atomic group is invalid");
  }
