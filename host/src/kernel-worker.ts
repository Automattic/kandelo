...
export class CentralizedKernelWorker {
  ...
  private pendingSignalWaits = new Map<string, {
    timer: ReturnType<typeof setTimeout>;
    channel: ChannelInfo;
    origArgs: number[];
  }>();
      ...
      onPosixTimer: (timerId: number, signo: number, valueMs: number, intervalMs: number): number => {
        ...
        if (valueMs > 0 || intervalMs > 0) {
          ...
          const timeout = setTimeout(() => {
            ...
            this.firePosixTimer(pid, timerId, signo, false);
            ...
            if (intervalMs > 0) {
              const iv = setInterval(() => {
                ...
                this.firePosixTimer(pid, timerId, signo, true);
              }, intervalMs);
              const entry = this.posixTimers.get(key);
  ...
  removeChannel(pid: number, channelOffset: number): void {
    const signalWaitKey = `${pid}:${channelOffset}`;
    const signalWait = this.pendingSignalWaits.get(signalWaitKey);
    if (signalWait) {
      clearTimeout(signalWait.timer);
  ...
  private _handleSyscallInner(channel: ChannelInfo): void {
    ...
    try {
      ...
      if (syscallNr === SYS_RT_SIGTIMEDWAIT) {
        this.signalWaitDeadlines.delete(`${channel.pid}:${channel.channelOffset}`);
      }
    ...
    if (syscallNr === SYS_RT_SIGTIMEDWAIT && !(retVal === -1 && errVal === EAGAIN)) {
      this.signalWaitDeadlines.delete(`${channel.pid}:${channel.channelOffset}`);
    }
    ...
    // tkill carries a thread-directed signal. Wake only the matching thread's
    // sigtimedwait so musl helper threads (including POSIX timer helpers) do
    // not wait for their periodic fallback retry.
    if (retVal === 0 && errVal === 0 && syscallNr === SYS_TKILL) {
    ...
    if (syscallNr === SYS_RT_SIGTIMEDWAIT) {
      ...
      if (timeoutPtr === 0) {
        ...
        const timer = setTimeout(() => {
          ...
          if (this.processes.has(channel.pid)) this.retrySyscall(channel);
        }, 500);
  ...
  /**
   * Ask the kernel to queue one host-scheduled POSIX timer expiration, then
   * wake the exact signal-waiting thread selected by the timer's sigevent.
   * Older kernels lack the additive export and retain the process-wide path.
   */
  private firePosixTimer(
    pid: number,
    timerId: number,
    signum: number,
    isInterval: boolean,
    ...
    if (!fire) {
      if (isInterval) {
        const intervalFire = this.kernelInstance?.exports
          ...
          ((pid: number, timerId: number) => number) | undefined;
        if (intervalFire?.(pid, timerId)) return;
      }
  ...
  private wakePendingSignalWaits(
    ...
    const matches = Array.from(this.pendingSignalWaits.entries()).filter(([, entry]) => {
      if (entry.channel.pid !== pid) return false;
      const channelTid = this.channelTids.get(`${pid}:${entry.channel.channelOffset}`) ?? pid;
      if (targetTid !== undefined && channelTid !== targetTid) return false;

      const maskPtr = entry.origArgs[0] >>> 0;
      if (maskPtr === 0 || signum <= 0 || signum > 64) return false;
      const mask = new DataView(entry.channel.memory.buffer).getBigUint64(maskPtr, true);
   ...
   */
  private sendSignalToProcess(targetPid: number, signum: number, queueSignal = true): void {
    ...
    if (queueSignal) {
      ...
      // Host-originated process signals are shared deliveries. Force tid=0
      // so the kernel does not consult state from a prior thread dispatch.
      const setTid = this.kernelInstance.exports.kernel_set_current_tid as
        ((tid: number) => void) | undefined;
      if (setTid) setTid(0);
      try {
