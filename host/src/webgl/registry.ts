...
/** Cross-context GPU-tier bo → WebGLTexture map (plan 3 §B6). */
export class ForeignTextureRegistry {
  private boTextures = new Map<number, WebGLTexture>();
  private perCtxBindings = new Map<number, Map<number, WebGLTexture>>();
  private perCtxNext = new Map<number, number>();
  ...
  bind(boId: number, ctxId: number): number {
    const tex = this.boTextures.get(boId);
    if (!tex) return -1;
    let m = this.perCtxBindings.get(ctxId);
    if (!m) {
      m = new Map();
      this.perCtxBindings.set(ctxId, m);
    }
    const id = this.perCtxNext.get(ctxId) ?? 1;
    this.perCtxNext.set(ctxId, id + 1);
    m.set(id, tex);
    return id;
  }

  resolve(ctxId: number, syntheticId: number): WebGLTexture | undefined {
    return this.perCtxBindings.get(ctxId)?.get(syntheticId);
  }
