...
import { readWasmProcessMemoryLayout } from "./wasm-artifact-driver";
import {
...
/**
 * The bytes handed to the artifact reader when a caller names no program.
 *
 * Not a module, and deliberately so: a layout asked for without a program has
 * no `__heap_base`, no memory import and no pthread declaration, which is
 * exactly what the reader reports for bytes it cannot walk. Passing an empty
 * buffer says that in the one place the reader can answer it, rather than
 * teaching the caller a second set of defaults.
 */
const EMPTY_PROGRAM = new ArrayBuffer(0);
...
/**
 * Where one process's control memory, syscall channel and initial break go.
 *
 * **The arithmetic is not here, and no longer anywhere in this host.** It is
 * `wasm_posix_shared::process_memory::compute_layout`, reached through the
 * artifact reader's `wa_process_memory_layout` export, and `crates/host-native`
 * calls that same function directly. One address space, described once.
 *
 * What that replaced was 200 lines that included a hand-rolled LEB128 walk over
 * the import section to find the imported memory's minimum — a third
 * WebAssembly decoder in a host that already had a Rust one. The program's
 * `__heap_base` and pthread declaration were already being read through that
 * reader; only the memory minimum and the placement rule were still local.
 *
 * `heapBase` stays authoritative when a caller supplies it, which is what this
 * function has always done: `process-lifecycle.ts` extracts it from the same
 * bytes it passes, and a caller placing a layout with no program at all has
 * nowhere else to get it. Omitted, the reader answers from the program's own
 * `__heap_base`, which is what `crates/host-native` does.
 */
export function computeProcessMemoryLayout(
  ...
  const layout = readWasmProcessMemoryLayout(
    ...
    {
      maximumPages: options.maxPages ?? DEFAULT_MAX_PAGES,
      requestedMinimumPages: options.minPages ?? 0,
      defaultThreadSlots: options.defaultThreadSlots ?? DEFAULT_PROCESS_THREAD_SLOTS,
      threadSlots: options.threadSlots,
      heapBase: options.heapBase,
    },
  );
  return { ...layout };
}
