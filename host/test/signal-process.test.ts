...
describe("host-initiated signal delivery", () => {
  it("SIGTERM terminates a process blocked in pause()", { timeout: 30_000 }, async () => {
    ...
    try {
      ...
      // Signal 0 is the POSIX existence probe. It must consult kernel state
      // without applying a disposition or waking the blocked process.
  ...
  it("signalling an unknown pid reports ESRCH rather than pretending", async () => {
    ...
    try {
      ...
      expect(await host.signalProcess(999_999, 0)).toBe(false);
      await expect(host.signalProcess(999_999, 65)).rejects.toThrow(
        "Invalid POSIX signal number 65",
      );
    } finally {
      await host.destroy();
    }
