...
describe("NodePcmDriver", () => {
  ...
  it("wakes a pending machine-teardown drain as soon as the Node clock consumes its tail", async () => {
    const { worker, descriptor } = createPcmKernelWorker({
      ...
      clockUpdate: (requestedFrames, transport) => {
        const frames = Math.min(requestedFrames, 1);
        const transportWords = pcmControlWords(transport);
        writeConsumerPosition(transportWords, BigInt(frames * 4));
        Atomics.add(transportWords, PCM_CONTROL.wakeSeq, 1);
        return frames;
      },
    });
    const words = pcmControlWords(descriptor);
