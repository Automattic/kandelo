...
const SETUP_FAILURE = /wlcube failed|wlcompositor failed/;

const canvasLocator = (page: Page) =>
  page.locator(".kmachine-primary-slot:not(.is-hidden) canvas").first();

/**
 * Browser gate for the raw-libwayland-egl (GL) demo: wlcompositor composites
 * a single client, wlcube, which renders a spinning, back-face-culled cube
 * over a teal clear through the STANDARD libwayland-egl / libEGL entry points
 * a mesa client uses — with NO toolkit in between. Where kandelo-sdl2gl.spec
 * proves the step-10/11/12 GL chain under SDL2, this proves the same chain
 * driven directly: wl_egl_window is our GPU-bo/dmabuf shim, libEGL targets
 ...
 *
 * Gates:
 *   1. The raw client came up (WLCUBE_UP) and connected to the compositor
 *      (CLIENT_CONNECTED count=1). A connect race or an EGL/link failure
 *      fails here.
 ...
 *      near the cube window's top-left) is non-black — i.e. the client's GL
 *      pixels crossed the process boundary as a dmabuf-imported texture.
 *   3. The composited desktop is on the canvas: the frame compresses to a
 *      PNG far larger than a blank one.
 *   4. Liveness: the client keeps swapping frames (WLCUBE_FRAME count
 ...
 */
test("Kandelo raw libwayland-egl GL client composites through wlcompositor", async ({ page }) => {
  test.setTimeout(240_000);

  await gotoOrSkip(page, "/?demo=wlcube");

  // Boot is heavy (compositor + GL client). Watch the syslog for the
  // client's own up-marker.
  await openSurface(page, "Internals");
  await expect
    .poll(() => syslogStream(page), { timeout: 180_000 })
    .toMatch(/WLCUBE_UP w=\d+ h=\d+/);
  expect(await syslogText(page), "wlcube setup reported failure")
  ...
  // Gate 2b: the GL readback proof — the client's pixels reached the
  ...
  // the raw-EGL path; the node smokes never exercise the GL readback.
  await expect
    .poll(() => syslogStream(page), { timeout: 60_000 })
  ...
  // Gate 4: liveness — the client keeps presenting frames. wlcube prints
  // WLCUBE_FRAME every 30 frames (~1 s at 30 fps); the count must keep
  // climbing, proving the single reusable buffer isn't wedged.
  await openSurface(page, "Internals");
  const frameCount = async () => {
    const nums = [...(await syslogStream(page)).matchAll(/WLCUBE_FRAME (\d+)/g)]
      .map((m) => Number(m[1]));
  ...
  expect(await syslogText(page), "wlcube reported failure while running")
    .not.toMatch(SETUP_FAILURE);
});
