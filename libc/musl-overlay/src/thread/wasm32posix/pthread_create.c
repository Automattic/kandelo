...
/* Wasm varargs are stack-backed and require the shadow stack pointer to
 * enter functions 16-byte aligned. The generic musl path aligns only to
 * pointer size before placing start_args, which corrupts i64 varargs in
 * pthread workers on wasm32. */
...
int __pthread_create(pthread_t *restrict res, const pthread_attr_t *restrict attrp, void *(*entry)(void *), void *restrict arg)
{
	 ...
	 * list is unlocked. Keep the entry shadow stack 16-byte aligned
	 * after reserving start_args; wasm varargs depend on it. */
	stack -= sizeof(struct start_args);
	stack -= (uintptr_t)stack % WASM_STACK_ALIGN;
