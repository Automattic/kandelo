...
#define KANDELO_DEFAULT_ST_BLKSIZE 4096

static unsigned long long kandelo_stat_blocks(unsigned long long size)
{
	return (size + 511) / 512;
}

static int kandelo_stat_blksize(int blksize)
{
	return blksize > 0 ? blksize : KANDELO_DEFAULT_ST_BLKSIZE;
}

static unsigned long long kandelo_stat_reported_blocks(unsigned long long size, unsigned long long blocks)
{
	return blocks ? blocks : kandelo_stat_blocks(size);
}
...
static int fstatat_statx(int fd, const char *restrict path, struct stat *restrict st, int flag)
{
	...
	*st = (struct stat){
		...
		.st_blksize = kandelo_stat_blksize(stx.stx_blksize),
		.st_blocks = kandelo_stat_reported_blocks(stx.stx_size, stx.stx_blocks),
...
static int fstatat_kstat(int fd, const char *restrict path, struct stat *restrict st, int flag)
{
	...
	*st = (struct stat){
		...
		.st_rdev = kst.st_rdev,
		.st_size = kst.st_size,
		.st_blksize = kandelo_stat_blksize(kst.st_blksize),
		.st_blocks = kandelo_stat_reported_blocks(kst.st_size, kst.st_blocks),
