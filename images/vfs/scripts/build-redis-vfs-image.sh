#!/usr/bin/env bash
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
cd "$REPO_ROOT"
if [ "$#" -ne 0 ] && [ "${1:-}" = "--vfs-product-manifest" ]; then
  exec node "$REPO_ROOT/node_modules/tsx/dist/cli.mjs" \
    "$SCRIPT_DIR/staged-product-inputs.ts" browser-redis "$@"
fi
echo "==> Building Redis VFS image..."
npx tsx "$SCRIPT_DIR/build-redis-vfs-image.ts"
echo "==> Done."
ls -lh apps/browser-demos/public/redis.vfs.zst

# Mirror into local-binaries/ so the @binaries/ Vite alias resolves for
# pages/redis/main.ts. See sibling build-nginx-vfs-image.sh for rationale.
source "$REPO_ROOT/scripts/install-local-binary.sh"
install_local_binary redis-vfs "$REPO_ROOT/apps/browser-demos/public/redis.vfs.zst"
