...
export async function runHomebrewVfsImageBuilder(
  ...
  if (options.materializationPolicy === undefined) {
    ...
    const collectionFs = (
      await createFs(undefined, maxByteLength, plan.kandeloAbi)
    ).fs;
  ...
  if (materializedBuild !== undefined || packageTree !== undefined) {
    ...
    // WHY: post-save assertions inspect trusted lazy state; authenticate the
    // imported seals before any synchronous assertion exports that state.
  ...
  if (options.lazyLayerOut && options.lazyLayerDescriptor) {
    ...
    const layerFs = (
      await createFs(undefined, options.maxBytes, plan.kandeloAbi)
    ).fs;
