...
kernel_runtime_changed_files() {
  grep -E \
    ...
    -e '^scripts/(build-musl|build-libcxx|build-programs|check-abi-version|check-libcxx-toolchain-version|dev-shell|run-libc-tests|run-posix-tests|run-sortix-tests)\.sh$' \
